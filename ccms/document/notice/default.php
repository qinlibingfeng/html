...
function GetPageCount(a_ctions,doa_ctions){
	...
	var url="action=get_notice&pages=1&actions="+a_ctions+"&do_actions="+doa_ctions+"&sorts="+$('#sorts').val()+"&order="+$('#order').val()+times+"&"+$('#form1').serialize();
...
function get_datalist(page_nums,a_ctions,doa_ctions,pagesize){
	...
	var url="action=get_notice&pages="+page_nums+"&actions="+a_ctions+"&do_actions="+doa_ctions+"&sorts="+$('#sorts').val()+"&order="+$('#order').val()+"&pagesize="+pagesize+times+"&"+$('#form1').serialize();
	//alert(url);
	//return false;
		...
		success: function(json){ 
			...
			if(parseInt(json.counts)>0){
				...
				$.each(json.datalist, function(index,con){
					...
					disabled="";
					dblclick=" ondblclick='edit_notice(\""+con.notice_id+"\")' ";
 					 
					disabled="";
					do_edit="<a href='javascript:void(0)' onclick='edit_notice(\""+con.notice_id+"\")'>修改</a> <a href='javascript:void(0)' onclick='del_(\""+con.notice_id+"\")'>删除</a>";
  					 
					tr_str="<tr align=\"left\" id=\"notice_"+con.notice_id+"\" "+dblclick+" >";
					tr_str+="<td align=\"center\"><input name=\"c_id\" type=\"checkbox\" value=\""+con.notice_id+"\" "+disabled+" /></td>";
					...
					tr_str+="<td title='"+con.notice_des+"'><div class='hide_tit'>"+con.notice_des+"</div></td>";
  					tr_str+="<td>"+con.full_name+" ["+con.user_id+"]"+"</td>";
...
function add_notice(actions){
	var diag =new Dialog("add_notice");
    diag.Width = $(window).width() - 26;
    diag.Height = $(window).height() -60;
 	diag.Title = "新建公告";
	
	diag.URL = '/document/notice/list.php?action=add_notice&tits='+encodeURIComponent("新建公告");
 	
	diag.OKEvent = set_add_notice;
	//diag.CancelEvent = parent_focus;
    diag.show();
}
 
function set_add_notice(){
	Zd_DW.do_add_notice();
}
  
function edit_notice(notice_id){
	var diag =new Dialog("edit_notice_"+notice_id);
    diag.Width = $(window).width() - 26;
    diag.Height = $(window).height() -60;
 	diag.Title = "公告设置";
	
	diag.URL = '/document/notice/list.php?action=edit_notice&notice_id='+notice_id+'&tits='+encodeURIComponent("公告设置");
 	
	diag.OKEvent = set_edit_notice;
	//diag.CancelEvent = parent_focus;
    diag.show();
}
 
function set_edit_notice(){
	Zd_DW.do_edit_notice();
}
  
   
function copy_notice(){
	var diag =new Dialog("copy_notice_");
	diag.Width = 580;
	diag.Height = 240;	
 	diag.Title = "复制公告";
	
	diag.URL = '/document/notice/list.php?action=copy_notice&tits='+encodeURIComponent("复制公告");
 	
	diag.OKEvent = set_copy_notice;
    diag.show();
}
 
function set_copy_notice(){
	Zd_DW.do_copy_notice();
}   
...
$(document).ready(function(){
	...
	$('<input name="a_ctions" type="hidden" id="a_ctions"/> <input name="doa_ctions" type="hidden" id="doa_ctions"/> <input name="recounts" type="hidden" id="recounts"/> <input name="pages" type="hidden" id="pages" value="1"/> <input name="pagecounts" type="hidden" id="pagecounts"/><input name="pagesize" type="hidden" id="pagesize" value="20"/> <input name="sorts" type="hidden" id="sorts" value="notice_id"/> <input name="order" type="hidden" id="order"/>').appendTo("body");
...
<div class="page_main">
     ...
     <tr>
        <td colspan="2"><a href='javascript:void(0);'  class='zPushBtn' hidefocus='true' tabindex='-1' onselectstart='return false' priv="true" onClick="add_notice('');" title="新建公告！"><img src="/images/icons/script_3.png" style="margin-top:4px"/><b>新建公告&nbsp;</b></a></td>
      </tr>
    </table>
            ...
            <table border="0" width="100%" align="center" cellpadding="0" cellspacing="0" class="dataTable" id="datatable" >
                  <thead>
                    <tr align="left" class="dataHead">
                      ...
                      <th sort="notice_id" >公告标题</th>
                      <th sort="notice_title" >公告描述</th>
                      <th sort="user_id">发布人</th>
