...
function GetPageCount(a_ctions,doa_ctions){
	...
	var url="action=get_pause_code_list&pages=1&actions="+a_ctions+"&do_actions="+doa_ctions+"&sorts="+$('#sorts').val()+"&order="+$('#order').val()+times+"&"+$('#form1').serialize();
...
function get_datalist(page_nums,a_ctions,doa_ctions,pagesize){
	...
	var url="action=get_pause_code_list&pages="+page_nums+"&actions="+a_ctions+"&do_actions="+doa_ctions+"&sorts="+$('#sorts').val()+"&order="+$('#order').val()+"&pagesize="+pagesize+times+"&"+$('#form1').serialize();
	//alert(url);
	//return false;
		...
		success: function(json){ 
			...
			if(parseInt(json.counts)>0){
				...
				$.each(json.datalist, function(index,con){
					...
					dblclick=" ondblclick='edit_pause_code(\""+con.pause_code+"\")' ";
					do_edit="<a href='javascript:void(0)' onclick='edit_pause_code(\""+con.pause_code+"\")'>修改</a> <a href='javascript:void(0)' onclick='del_(\""+con.pause_code+"\")'>删除</a>";
  					
					tr_str="<tr align=\"left\" id=\"pause_code_list_"+con.pause_code+"\" "+dblclick+">";
					tr_str+="<td align=\"center\"><input name=\"c_id\" type=\"checkbox\" value=\""+con.pause_code+"\" /></td>";
					tr_str+="<td>"+con.pause_code+"</td>";
					tr_str+="<td title='"+con.pause_code_name+"'><div >"+con.pause_code_name+"</div></td>";
...
function add_pause_code(actions){
	var diag =new Dialog("add_pause_code");
    diag.Width = 580;
    diag.Height = 240;
 	diag.Title = "新建暂停原因";
	diag.URL = '/document/campaign/list.php?action=add_pause_code&tits='+encodeURIComponent("新建暂停原因");
 	diag.OKEvent = set_add_pause_code;
	//diag.CancelEvent = parent_focus;
    diag.show();
}
 
function set_add_pause_code(){
	Zd_DW.do_add_pause_code();
}
  
function edit_pause_code(pause_code){
	var diag =new Dialog("edit_pause_code_"+pause_code);
    diag.Width = 580;
    diag.Height = 240;
 	diag.Title = "暂停原因设置";
	diag.URL = '/document/campaign/list.php?action=edit_pause_code&pause_code='+pause_code+'&tits='+encodeURIComponent("暂停原因设置");
 	diag.OKEvent = set_edit_pause_code;
	//diag.CancelEvent = parent_focus;
    diag.show();
}
 
function set_edit_pause_code(){
	Zd_DW.do_edit_pause_code();
}
...
function del_(c_id){	
	...
	datas="action=del_pause_code&do_actions=pause_code&c_id="+c_id+times;
 	//alert(datas);
...
$(document).ready(function(){
	...
	$('<input name="a_ctions" type="hidden" id="a_ctions"/> <input name="doa_ctions" type="hidden" id="doa_ctions"/> <input name="recounts" type="hidden" id="recounts"/> <input name="pages" type="hidden" id="pages" value="1"/> <input name="pagecounts" type="hidden" id="pagecounts"/><input name="pagesize" type="hidden" id="pagesize" value="20"/> <input name="sorts" type="hidden" id="sorts" value="pause_code"/> <input name="order" type="hidden" id="order"/>').appendTo("body");
...
<div class="page_main">
     ...
     <tr>
        <td colspan="2"><a href='javascript:void(0);'  class='zPushBtn' hidefocus='true' tabindex='-1' onselectstart='return false' priv="true" onClick="add_pause_code('');" title="新建暂停原因"><img src="/images/icons/icons_81.png" style="margin-top:4px"/><b>新建暂停原因&nbsp;</b></a></td>
      </tr>
    </table>
          ...
          <fieldset><legend> <label onClick="show_div('search_list');" title="点击收缩/展开">查询选项</label></legend>
            <form action="" onSubmit=""  method="post" name="form1" id="form1">       
             <table width="100%" border="0" align="center"  cellspacing="0" id="search_list" class="search_table" >
               ...
               <tr>
                 <td width="8%" align="right">原因ID：</td>
                 <td height="">
                 <input name="pause_code" type="text" class="input_text" id="pause_code" title="输入要查询的原因ID，多个以英文“,”分隔"  size="21" onkeyup="this.value=value.replace(/[^\w\/,]/ig,'')" onafterpaste="this.value=value.replace(/[^\w\/,]/ig,'')" />
                 </td>
                 <td width="8%" height="26" align="right" id="">原因名称：</td>
         		 <td nowrap="nowrap"><input name="pause_code_name" type="text" class="input_text" id="pause_code_name" /></td>
            ...
            <table border="0" width="100%" align="center" cellpadding="0" cellspacing="0" class="dataTable" id="datatable" >
                  <thead>
                    <tr align="left" class="dataHead">
                      ...
                      <th sort="pause_code" >暂停原因ID</th>
                      <th sort="pause_code_name" >暂停原因名称</th>
