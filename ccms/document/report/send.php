...
switch($action){
	...
	/*	if($phone_login<>""){
		...
		$data_sql="select campaign_id,user,left(call_date,".$g_type.") as call_date from data_report_call_log_list where ".$wheres."";	 
		//echo $data_sql.'<br><br>';
 		 
		if($drop_opt=="drop_opt_jt"){
			$sql_wrjt=",sum(case when status in('DISPO','XFER','WRJT','PU','NP','NI','N','SALE','XDROP','NA','CBHOLD') then counts else 0 end ) as '无人接听'";
 			$sql_drop_p="+丢弃";
		}else{
			$sql_wrjt=",sum(case when status in('DISPO','XFER','WRJT','PU','NP','NI','N','SALE','XDROP','NA','CBHOLD','DROP') then counts else 0 end ) as '无人接听'";
 			$sql_drop_p="";
		}
		...
		$group_sql=",sum(case when status='CG' then counts else 0 end ) as '成功',sum(case when status='SB' then counts else 0 end ) as '失败',sum(case when status in('QUEUE','DNCL','DNC','DEC','PM','INCALL','CALLBK','QT','ERI') then counts else 0 end ) as '其他',sum(case when status in('QVMAIL','B','AB','ADC','GJ') then counts else 0 end ) as '关机',sum(case when status='TJ' then counts else 0 end ) as '停机' ".$sql_wrjt.",sum(case when status='DROP' then counts else 0 end ) as '丢弃',sum(case when status in('DC','SVYEXT','SVYVM','KHCZ','SVYHU','A','AA','AM','AL','SVYREC','AFAX') then counts else 0 end ) as '空号传真'";
	  ...
	  if($report_type=="report_1"){
			...
			switch($data_type){
				...
				//按工号、业务统计
				case "data_7":
					$sql="select case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',users.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$report_field." from( select case when user is null then 'ZongJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id,SUM(成功) as '成功',sum(停机+空号传真) as '停机或空号',sum(无人接听) as '无人接听',sum(关机+其他) as '关机或其他'".$case_sql." from( select user,campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by user,campaign_id ) datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users users on datas.工号=users.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
...
,sum(成功+失败+其他+关机+停机+无人接听+空号传真".$sql_drop_p.") as '呼叫总数',count(case when user is not null and user!='VDAD' then 1 else 0 end ) as '坐席数',sum(成功+失败".$sql_drop_p.") as '接通量',SUM(成功) as '成功',case when SUM(成功)=0 then concat(0,'%') else concat(ROUND((SUM(成功)/sum(成功+失败".$sql_drop_p."))*100,2),'%') end as '呼通成功率',case when sum(成功+失败".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((sum(成功+失败".$sql_drop_p.")/sum(成功+失败+其他+关机+停机+无人接听+空号传真".$sql_drop_p."))*100,2),'%') end as '接通率'".$sum_drop." from( select left(call_date,".$g_type.") as call_date,user ".$group_sql." from data_report_call_log_list where ".$wheres." group by left(call_date,".$g_type.") ,user ) datas group by call_date ".$with_rollup."";
			...
			switch($data_type){
				...
				case "data_2":
					...
					$sql="select case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$report_field." from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id,sum(成功) as '成功',sum(失败) as '失败',sum(停机) as '停机',sum(关机) as '关机',sum(其他) as '其他',sum(空号传真) as '空号传真',sum(无人接听) as '无人接听' ".$case_sql." from( select campaign_id ".$group_sql." from data_report_call_log_list where ".$wheres." group by campaign_id ) datas group by  campaign_id ".$with_rollup." )datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
...
case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',users.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$report_field." from(select case when user is null then 'ZongJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id,sum(成功) as '成功',sum(失败) as '失败',sum(停机) as '停机',sum(关机) as '关机',sum(其他) as '其他',sum(空号传真) as '空号传真',sum(无人接听) as '无人接听' ".$case_sql." from( select user,campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by user,campaign_id ) datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users users on datas.工号=users.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
			...
			switch($data_type){
				...
				case "data_1":
					...
					$sql="select case when call_date is null then '合计' else call_date end as '日期'".$case_sql." from( select left(call_date,".$g_type.") as call_date ".$group_sql." from data_report_call_log_list where ".$wheres." group by left(call_date,".$g_type.")  ) datas group by call_date ".$with_rollup."";
				
				break;
				//按月份统计
				case "data_1_month":
				
					$sql="select case when call_date is null then '合计' else call_date end as '月份'".$case_sql." from( select left(call_date,".$g_type.") as call_date ".$group_sql." from data_report_call_log_list where ".$wheres." group by left(call_date,".$g_type.")  ) datas group by call_date ".$with_rollup."";
				
				break;
				//按业务统计
				case "data_2":
				
					$sql="select case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$fields_list." from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id ".$case_sql." from( select campaign_id ".$group_sql." from data_report_call_log_list where ".$wheres." group by campaign_id ) datas group by  campaign_id ".$with_rollup." )datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
				...
				case "data_7":
					$sql="select 
case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',users.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$fields_list." from(select case when user is null then 'ZongJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id".$case_sql." from( select user,campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by user,campaign_id ) datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users users on datas.工号=users.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
			...
			if ($row_counts_list_group!=0) {
			  ...
			  while($rs= mysqli_fetch_array($rows_group)){
				  ...
				  if(strpos($agent_list,",")>-1){
					  $group_sql.=",sum(case when status in('".str_replace(",","',''",$status)."') then counts else 0 end ) as '".$status_name."'";
				  }else{
					  $group_sql.=",sum(case when status='".$status."' then counts else 0 end ) as '".$status_name."'"; 
				  }
...
case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',users.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$fields_list." from(select case when user is null then 'ZongJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id".$case_sql." from( select user,campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by user,campaign_id ) datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users users on datas.工号=users.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
	...
	case "timelen_report":
		...
		$group_sql=",sum(case when status ='CG' then counts else 0 end ) as '成功',sum(case when status in('CG','SB','GJ15','HY','BXY','ZJBHG'".$sql_drop_p.") then counts else 0 end ) as '呼通',sum(counts) as '呼叫量',sum(".$time_sum_len_type.") as '时长',max(max_".$time_sum_len_type.") as '最长时',min(".$time_sum_len_type.") as '最短时'";
		...
		switch($data_type){
			...
			//按业务统计
			case "data_3":
				$sql="SELECT case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else b.campaign_name end as '业务',".$time_field."成功,呼通,呼叫量,成功率,呼通成功率,呼通率 from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id".$case_sql." from(select campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by campaign_id )datas group by campaign_id ".$with_rollup." )datas left join vicidial_campaigns b on datas.campaign_id=b.campaign_id";
			...
			//按工号业务统计
			case "data_9":
				$sql="SELECT case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else datas.user end as '工号',b.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',".$time_field."成功,呼通,呼叫量,成功率,呼通成功率,呼通率 from(select case when user is null then 'HeJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id".$case_sql." from(select user,campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by user,campaign_id )datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
	...
	case "fee_report":
		...
		$time_len_type=trim($_REQUEST["time_len_type"]);
		$time_sum_len_type=substr($time_len_type,2,strlen($time_len_type)-2);
		$time_sum_len_type="fee_length_sec";
		...
		$data_sql=" ";	
 		...
 		//'CG','HY','BXY','ZQ','HHMD','ZJBHG'
		$group_sql=",sum(case when status ='CG' then counts else 0 end ) as '成功',sum(case when status in('CG','SB','GJ15','HY','BXY','ZJBHG'".$sql_drop_p.") then counts else 0 end ) as '呼通',sum(counts) as '呼叫量',round(sum(fee),2) as '费用合计',round(sum(case when status ='CG' then fee else 0 end),2) as '成功费用',max(max_".$time_sum_len_type.") as '最长时',max(max_fee) as '最长费用',sum(".$time_sum_len_type.") as '计费时长'";
		
		$case_sql=",max(最长时) as '最长时',max(最长费用) as '最长费用',sum(成功) as '成功',sum(呼通) as '呼通',sum(呼叫量) as '呼叫量',case when SUM(计费时长)=0 or SUM(呼通)=0 then 0 else ROUND((SUM(计费时长)/SUM(呼通)),2) end  as '平均时',case when SUM(费用合计)=0 or SUM(呼通)=0 then 0 else ROUND((SUM(费用合计)/SUM(呼通)),2) end  as '平均费用',case when SUM(成功费用)=0 then 0 else ROUND((SUM(成功费用)/SUM(成功)),2) end  as '平均成功费用',case when SUM(费用合计)=0 then concat(0,'%') else concat(ROUND((SUM(成功费用)/SUM(费用合计))*100,2),'%') end as '成功费用比',sum(计费时长) as '计费时长',sum(费用合计) as '费用合计',case when SUM(成功)=0 then concat(0,'%') else concat(ROUND((SUM(成功)/SUM(呼叫量))*100,2),'%') end as '成功率',case when SUM(成功)=0 then concat(0,'%') else concat(ROUND((SUM(成功)/SUM(呼通))*100,2),'%') end as '呼通成功率'";
		
		if($time_type=="times"){
			$time_field="sec_to_time(最长时) as 最长时,sec_to_time(平均时) as 平均时,最长费用,平均费用,平均成功费用,成功费用比,费用合计,成功,呼通,呼叫量,成功率,呼通成功率";
		}elseif($time_type=="sec"){
			$time_field="最长时,平均时,计费时长,最长费用,平均费用,平均成功费用,成功费用比,费用合计,成功,呼通,呼叫量,成功率,呼通成功率";	
		}else{
			$time_field="ROUND(最长时/60,2) as 最长时,ROUND(平均时/60,2) as 平均时,最长费用,平均费用,平均成功费用,成功费用比,费用合计,成功,呼通,呼叫量,成功率,呼通成功率";	
		}
		...
		switch($data_type){
			...
			case "data_1":
				...
				$sql="SELECT case when 日期='HeJi' then '合计' when 日期='XiaoJi' then '小计' when 日期='ZongJi' then '总计' else 日期 end as '日期',".$time_field." from(select case when call_date is null then 'HeJi' else call_date end as '日期',case when call_date is null then '1' else '0' end as 'sums'".$case_sql." from(select left(call_date,".$g_type.") as call_date".$group_sql." from data_report_fee_log_list where ".$wheres." group by left(call_date,".$g_type.") )datas group by call_date ".$with_rollup." )datas";
			
			break;
 			//按月份统计
			case "data_1_month":
			
				$sql="SELECT case when 月份='HeJi' then '合计' when 月份='XiaoJi' then '小计' when 月份='ZongJi' then '总计' else 月份 end as '月份',".$time_field." from(select case when call_date is null then 'HeJi' else call_date end as '月份',case when call_date is null then '1' else '0' end as 'sums'".$case_sql." from(select left(call_date,".$g_type.") as call_date".$group_sql." from data_report_fee_log_list where ".$wheres." group by left(call_date,".$g_type.") )datas group by call_date ".$with_rollup." )datas";
			
			break;
			//按工号统计
			case "data_2":
			
				$sql="SELECT case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',b.full_name as '姓名',".$time_field." from( select case when user is null then 'HeJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user".$case_sql." from(select user".$group_sql." from data_report_fee_log_list where ".$wheres." group by user )datas group by user ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user";
 			break;
			//按业务统计
			case "data_3":
				$sql="SELECT case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else b.campaign_name end as '业务',".$time_field." from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id".$case_sql." from(select campaign_id".$group_sql." from data_report_fee_log_list where ".$wheres." group by campaign_id )datas group by campaign_id ".$with_rollup." )datas left join vicidial_campaigns b on datas.campaign_id=b.campaign_id";
   
			break;
			//按日期工号统计
			case "data_4":
				$sql="SELECT case when 日期='HeJi' then '合计' when 日期='XiaoJi' then '小计' when 日期='ZongJi' then '总计' else 日期 end as '日期',case when 日期 is not null and datas.user is null and sums='0' then '合计' else datas.user end as '工号',b.full_name as '姓名',".$time_field." from(select case when call_date is null then 'HeJi' else call_date end as '日期',case when call_date is null then '1' else '0' end as 'sums' ,user".$case_sql." from(select left(call_date,".$g_type.") as call_date,user".$group_sql." from data_report_fee_log_list where ".$wheres." group by left(call_date,".$g_type."),user )datas group by call_date,user ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user";
 
			break;
			//按月份工号统计
			case "data_4_month":
				$sql="SELECT case when 月份='HeJi' then '合计' when 月份='XiaoJi' then '小计' when 月份='ZongJi' then '总计' else 月份 end as '月份',case when 月份 is not null and datas.user is null and sums='0' then '合计' else datas.user end as '工号',b.full_name as '姓名',".$time_field." from(select case when call_date is null then 'HeJi' else call_date end as '月份',case when call_date is null then '1' else '0' end as 'sums' ,user".$case_sql." from(select left(call_date,".$g_type.") as call_date,user".$group_sql." from data_report_fee_log_list where ".$wheres." group by left(call_date,".$g_type."),user )datas group by call_date,user ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user";
 
			break;
			//按工号日期统计
			case "data_5":
				$sql="SELECT case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',b.full_name as '姓名' ,case when 工号 is not null and call_date is null and sums='0' then '合计' else call_date  end as '日期',".$time_field." from( select case when user is null then 'HeJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,call_date".$case_sql." from(select user,left(call_date,".$g_type.") as call_date".$group_sql." from data_report_fee_log_list where ".$wheres." group by user,left(call_date,".$g_type.")  )datas group by user,call_date ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user";
 
			break;
			//按工号月份统计
			case "data_5_month":
				$sql="SELECT case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',b.full_name as '姓名' ,case when 工号 is not null and call_date is null and sums='0' then '合计' else call_date  end as '月份',".$time_field." from( select case when user is null then 'HeJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,call_date".$case_sql." from(select user,left(call_date,".$g_type.") as call_date".$group_sql." from data_report_fee_log_list where ".$wheres." group by user,left(call_date,".$g_type.")  )datas group by user,call_date ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user";
 
			break;
			//按日期业务统计
			case "data_6":
				$sql="SELECT case when 日期='HeJi' then '合计' when 日期='XiaoJi' then '小计' when 日期='ZongJi' then '总计' else 日期 end as '日期',case when 日期 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',".$time_field." from(select case when call_date is null then 'HeJi' else call_date end as '日期',case when call_date is null then '1' else '0' end as 'sums' ,campaign_id".$case_sql." from(select left(call_date,".$g_type.") as call_date,campaign_id ".$group_sql." from data_report_fee_log_list where ".$wheres." group by left(call_date,".$g_type.") ,campaign_id )datas group by call_date,campaign_id ".$with_rollup.")datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
 
			break;
			//按月份业务统计
			case "data_6_month":
				$sql="SELECT case when 月份='HeJi' then '合计' when 月份='XiaoJi' then '小计' when 月份='ZongJi' then '总计' else 月份 end as '月份',case when 月份 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',".$time_field." from(select case when call_date is null then 'HeJi' else call_date end as '月份',case when call_date is null then '1' else '0' end as 'sums' ,campaign_id".$case_sql." from(select left(call_date,".$g_type.") as call_date,campaign_id ".$group_sql." from data_report_fee_log_list where ".$wheres." group by left(call_date,".$g_type.") ,campaign_id )datas group by call_date,campaign_id ".$with_rollup.")datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
 
			break;
			//按业务日期统计
			case "data_7":
				$sql="SELECT case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',case when 业务 is not null and call_date is null and sums='0' then '合计' else call_date end as '日期',".$time_field." from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id,call_date".$case_sql." from(select campaign_id,left(call_date,".$g_type.") as call_date".$group_sql." from data_report_fee_log_list where ".$wheres." group by campaign_id,left(call_date,".$g_type.")  )datas group by campaign_id,call_date ".$with_rollup." )datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
				 
			break;
			//按业务月份统计
			case "data_7_month":
				$sql="SELECT case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',case when 业务 is not null and call_date is null and sums='0' then '合计' else call_date end as '月份',".$time_field." from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id,call_date".$case_sql." from(select campaign_id,left(call_date,".$g_type.") as call_date".$group_sql." from data_report_fee_log_list where ".$wheres." group by campaign_id,left(call_date,".$g_type.")  )datas group by campaign_id,call_date ".$with_rollup." )datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
				 
			break;
			//按业务工号统计
			case "data_8":
				$sql="SELECT case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',case when 业务 is not null and datas.user is null and sums='0' then '合计' else datas.user end as '工号',b.full_name as '姓名',".$time_field." from( select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id,user ".$case_sql." from(select campaign_id,user".$group_sql." from data_report_fee_log_list where ".$wheres." group by campaign_id,user )datas group by campaign_id,user ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
				  
			break;
			
			//按工号业务统计
			case "data_9":
				$sql="SELECT case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else datas.user end as '工号',b.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务',".$time_field." from(select case when user is null then 'HeJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id".$case_sql." from(select user,campaign_id".$group_sql." from data_report_fee_log_list where ".$wheres." group by user,campaign_id )datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users b on datas.user=b.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
		...
		if($sql<>""){
			if($do_actions=="") { 
				...
				echo json_encode(save_detail_excel($sql,"通话费用统计报表"));	
			}
	...
	case "quality_report":
			...
			if ($row_counts_list_group!=0) {
			  ...
			  while($rs= mysqli_fetch_array($rows_group)){
				  ...
				  $status=$rs["status"];
 				  ...
 				  $group_sql.=",sum(case when quality_status='".$status."' then counts else 0 end ) as '".$status_name."'"; 
			  ...
			  $group_sql.=",sum(case when quality_status='0' and status='CG' then counts else 0 end ) as '成功:未质检'";
			  ...
			  if($hg_list!=""){
 			  ...
 			  $group_sql.=",sum(case when quality_status>0 then counts else 0 end ) as '质检总量'";
			...
			switch($data_type){
				...
				//按业务统计
				case "data_3":
					$sql="SELECT case when 业务='HeJi' then '合计' when 业务='XiaoJi' then '小计' when 业务='ZongJi' then '总计' else ifnull(b.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$fields_list." from(select case when campaign_id is null then 'HeJi' else campaign_id end as '业务',case when campaign_id is null then '1' else '0' end as 'sums' ,campaign_id".$case_sql." from(select campaign_id".$group_sql." from data_report_call_log_list where ".$wheres." group by campaign_id )datas group by campaign_id ".$with_rollup." )datas left join vicidial_campaigns b on datas.campaign_id=b.campaign_id";
	   
				break;
				//按质检人统计
				case "data_4":
					$sql="SELECT case when 质检人='HeJi' then '合计' when 质检人='XiaoJi' then '小计' when 质检人='ZongJi' then '总计' else concat(userid,'[',b.full_name,']') end as '质检人'".$fields_list." from(select case when userid is null then 'HeJi' else userid end as '质检人',case when userid is null then '1' else '0' end as 'sums' ,userid".$case_sql." from(select userid".$group_sql." from data_report_call_log_list where ".$wheres." group by userid )datas group by userid ".$with_rollup." )datas left join vicidial_users b on datas.userid=b.user";
	...
	case "quality_user_report":
			...
			if($day_part>31||$day_part<-31){
				...
				$json_data.="\"des\":".json_encode($day_part."本查询只可查询时间跨度为31天内数据!").",";
				$json_data.="\"field_name_list\":".json_encode($field_name_list).",";
				$json_data.="\"datalist\":".json_encode($list_arr)."";
				$json_data.="}";
			...
			$sql1=" a.addtime between '".$start_date."' and '".$end_date."' and INSTR(a.vicidial_id,'_b')=0 ";
			...
			if($agent_list<>""){
				...
				$inner_log_sql=" inner join vicidial_log b on a.vicidial_id=b.uniqueid ";
			...
			if ($row_counts_list_group!=0) {
			  ...
			  while($rs= mysqli_fetch_array($rows_group)){
				  ...
				  $status=$rs["status"];
 				  ...
 				  $group_sql.=",sum(case when qualitystatus='".$status."' then 1 else 0 end ) as '".$status_name."'"; 
				  
				  $sum_list.=",sum(".$status_name.") as '".$status_name."'";
				  $fields_list.=",".$status_name."";
			  }
 			  
 			  $group_sql.=",count(*) as '质检总量'";
			  $fields_list.=",质检总量";
			}
			mysqli_free_result($rows_group); 
			
  	
			$case_sql.=$sum_list.",sum(质检总量) as '质检总量'";
			...
			switch($data_type){
				...
				case "data_8":
					...
					$sql="select case when userid is null then '合计' else concat(datas.userid,' [',ifnull(b.full_name,datas.userid),']') end as '质检人' ".$fields_list." from(select a.userid ".$group_sql." from data_quality_log a ".$inner_log_sql." where ".$wheres." group by a.userid ".$with_rollup." )datas left join vicidial_users b on datas.userid=b.user";
				
				break;
				//按月份统计
				case "data_9":
				
					$sql="select case when userid is null and addtime is null then '总计' else concat(datas.userid,' [',ifnull(b.full_name,datas.userid),']')  end as '质检人',case when addtime is null and userid is not null then '合计' else addtime end as '日期' ".$fields_list." from(select a.userid,left(a.addtime,10) as addtime ".$group_sql." from data_quality_log a ".$inner_log_sql." where ".$wheres." group by a.userid,left(a.addtime,10) ".$with_rollup." )datas left join vicidial_users b on datas.userid=b.user ";
 				break;
				
				//按月份统计
				case "data_9_month":
				
					$sql="select case when userid is null and addtime is null then '总计' else concat(datas.userid,' [',ifnull(b.full_name,datas.userid),']')  end as '质检人',case when addtime is null and userid is not null then '合计' else addtime end as '月份' ".$fields_list." from(select a.userid,left(a.addtime,7) as addtime ".$group_sql." from data_quality_log a ".$inner_log_sql." where ".$wheres." group by a.userid,left(a.addtime,7) ".$with_rollup." )datas left join vicidial_users b on datas.userid=b.user ";
 				break;
				
				//按工号统计
				case "data_10":
				
					$sql="select case when userid is null and datas.user is null then '总计' else concat(datas.userid,' [',ifnull(b.full_name,datas.userid),']')  end as '质检人',case when datas.user is null and userid is not null then '合计' else concat(datas.user,' [',ifnull(c.full_name,datas.user),']')  end as '工号' ".$fields_list." from(select a.userid,b.user ".$group_sql." from data_quality_log a inner join vicidial_log b on a.vicidial_id=b.uniqueid where ".$wheres." group by a.userid,b.user ".$with_rollup." )datas left join vicidial_users b on datas.userid=b.user left join vicidial_users c on datas.user=c.user ";
	 
				break;
				//按业务统计
				case "data_11":
					$sql="select case when userid is null and datas.campaign_id is null then '总计' else concat(datas.userid,' [',ifnull(b.full_name,datas.userid),']')  end as '质检人',case when datas.campaign_id is null and userid is not null then '合计' else concat(datas.campaign_id,' [',ifnull(c.campaign_name,datas.campaign_id),']')  end as '业务活动' ".$fields_list." from(select a.userid,a.campaign_id ".$group_sql." from data_quality_log a ".$inner_log_sql." where ".$wheres." group by a.userid,a.campaign_id ".$with_rollup." )datas left join vicidial_users b on datas.userid=b.user left join vicidial_campaigns c on datas.campaign_id=c.campaign_id";
	...
	case "get_work_person":
		...
		if($drop_opt=="drop_opt_jt"){
			$sql_wrjt=",sum(case when status in('DISPO','XFER','WRJT','PU','NP','NI','N','SALE','XDROP','NA','CBHOLD') then counts else 0 end ) as '无人接听',sum(case when status='DROP' then 1 else 0 end ) as '丢弃'";
 			$sql_drop_p="+丢弃";
		}else{
			$sql_wrjt=",sum(case when status in('DISPO','XFER','WRJT','PU','NP','NI','N','SALE','XDROP','NA','CBHOLD','DROP') then counts else 0 end ) as '无人接听'";
 			$sql_drop_p="";
		...
		$sql="select concat(datas.user,' [',users.full_name,']') as gh,cg,zl,jtl,ztcgl,bhg,sc from(select user,sum(成功+失败+其他+关机+停机+无人接听+空号传真) as 'zl',SUM(成功) as 'cg' ,SUM(不合格) as 'bhg' ,sec_to_time(SUM(sc)) as 'sc' ,case when sum(成功+失败)=0 then concat(0,'%') else concat(ROUND((sum(成功+失败)/sum(成功+失败+其他+关机+停机+无人接听+空号传真))*100,2),'%') end as 'jtl',case when SUM(成功)=0 then concat(0,'%') else concat(ROUND((SUM(成功)/sum(成功+失败+其他+关机+停机+无人接听+空号传真))*100,2),'%') end as 'ztcgl' from( select user,sum(case when status='CG' then counts else 0 end ) as '成功',sum(case when status='SB' then counts else 0 end ) as '失败',sum(case when status in('QUEUE','DNCL','DNC','DEC','PM','INCALL','CALLBK','QT','ERI') then counts else 0 end ) as '其他',sum(case when status in('QVMAIL','B','AB','ADC','GJ') then counts else 0 end ) as '关机',sum(case when status='TJ' then counts else 0 end ) as '停机'".$sql_wrjt.",sum(case when status in('DC','SVYEXT','SVYVM','KHCZ','SVYHU','A','AA','AM','AL','SVYREC','AFAX') then counts else 0 end ) as '空号传真',sum(case when quality_status='1' then counts else 0 end ) as '不合格',sum(talk_length_sec) as 'sc' from  data_report_call_log_list where user!='VDAD' and call_date BETWEEN '".$yes_day_s."' and '".$yes_day_e."' group by user ) datas group by user )datas left join vicidial_users users on datas.user=users.user  order by cg+0 desc,left(ztcgl,length(ztcgl)-1)+0 desc";
				...
				$row_counts_list=mysqli_num_rows($rows);			
		 		...
		 		if ($row_counts_list > 0) {					 			
					while($rs=mysqli_fetch_assoc($rows)){//通过循环读取数据内容				
						$list=array("field_name"=>$rs['field_name'],"field_label"=>str_replace("：","",$rs['field_label']));	
						//print_r($list);
					array_push($list_arr,$list);
					}
