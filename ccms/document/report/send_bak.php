...
switch($action){
	...
	case "data_report":
		...
		if($dis_drop=="no"){
			...
			$sum_drop=",case when SUM(丢弃)=0 then concat(0,'%') else concat(ROUND((SUM(丢弃)/SUM(成功+一般+失败+丢弃+预约+非本店))*100,2),'%') end as '丢弃率'";
			$f_drop=",丢弃率";	
		}else{
			$sum_drop=",sum(丢弃) as '丢弃',case when SUM(丢弃)=0 then concat(0,'%') else concat(ROUND((SUM(丢弃)/SUM(成功+一般+失败+丢弃+预约+非本店))*100,2),'%') end as '丢弃率'";
			$f_drop=",丢弃,丢弃率";	
		}
		
		$group_sql=",sum(case when status_name='成功' then 1 else 0 end ) as '成功',sum(case when status_name='一般' then 1 else 0 end ) as '一般',sum(case when status_name='失败' then 1 else 0 end ) as '失败',sum(case when status_name='其他' then 1 else 0 end ) as '其他',sum(case when status_name='关机' then 1 else 0 end ) as '关机',sum(case when status_name='停机' then 1 else 0 end ) as '停机' ".$sql_wrjt.",sum(case when status_name='丢弃' then 1 else 0 end ) as '丢弃',sum(case when status_name='空号传真' then 1 else 0 end ) as '空号传真',sum(case when status_name='预约' then 1 else 0 end ) as '预约',sum(case when status_name='非本店' then 1 else 0 end ) as '非本店'";
		//echo $group_sql.'<br><br>';
		
		$case_sql=",sum(成功+一般+失败+预约+非本店".$sql_drop_p.") as '接通量',sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p.") as '呼叫总数',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般+失败+预约+非本店".$sql_drop_p.")/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '呼通成功率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '整体成功率'".$sum_drop;
	  ...
	  if($report_type=="report_1"){
			...
			switch($data_type){
				...
				case "data_1":
					...
					$sql="select case when call_date is null then '合计' else call_date end as '日期',sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p.") as '呼叫总数',sum(成功+一般) as '成功',sum(成功+一般+失败+预约+非本店".$sql_drop_p.") as '接通量',sum(停机+空号传真) as '停机或空号',sum(无人接听) as '无人接听',sum(关机+其他) as '关机或其他',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般+失败+预约+非本店".$sql_drop_p.")/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '呼通成功率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '整体成功率'".$sum_drop." from( select call_date ".$group_sql." from (".$data_sql.") datas group by call_date  order by null) datas group by call_date ".$with_rollup."";
				...
				case "data_1_month":
					...
					$sql="select case when call_date is null then '合计' else call_date end as '月份',sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p.") as '呼叫总数',sum(成功+一般) as '成功',sum(成功+一般+失败+预约+非本店".$sql_drop_p.") as '接通量',sum(停机+空号传真) as '停机或空号',sum(无人接听) as '无人接听',sum(关机+其他) as '关机或其他',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般+失败+预约+非本店".$sql_drop_p.")/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '呼通成功率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '整体成功率'".$sum_drop." from( select call_date 	".$group_sql." from (".$data_sql.") datas group by call_date  order by null) datas group by call_date ".$with_rollup."";
...
,SUM(成功+一般) as '成功',sum(失败) as '失败',sum(空号传真) as '空号传真',sum(其他) as '其他',sum(无人接听) as '无人接听',sum(关机+停机) as '关机停机',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般+失败+预约+非本店".$sql_drop_p.")/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '整体成功率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '呼通成功率'".$sum_drop." from( select call_date	".$group_sql." from (".$data_sql.") datas group by call_date order by null) datas group by call_date ".$with_rollup."";
...
,SUM(成功+一般) as '成功',sum(失败) as '失败',sum(空号传真) as '空号传真',sum(其他) as '其他',sum(无人接听) as '无人接听',sum(关机+停机) as '关机停机',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般+失败+预约+非本店".$sql_drop_p.")/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '整体成功率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/SUM(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '呼通成功率'".$sum_drop." from( select call_date	".$group_sql." from (".$data_sql.") datas group by call_date order by null) datas group by call_date ".$with_rollup."";
			...
			switch($data_type){
				...
				case "data_1":
					...
					$sql="select case when call_date is null then '合计' else call_date end as '日期'
,sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p.") as '呼叫总数',count(case when user is not null and user!='VDAD' then 1 else 0 end ) as '坐席数',sum(成功+一般+失败+预约+非本店".$sql_drop_p.") as '接通量',SUM(成功+一般) as '成功',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/sum(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '呼通成功率',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((sum(成功+一般+失败+预约+非本店".$sql_drop_p.")/sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率'".$sum_drop." from( select call_date,user ".$group_sql." from (".$data_sql.") datas group by call_date,user order by null) datas group by call_date ".$with_rollup."";
...
case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',users.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$report_field." from(select case when user is null then 'ZongJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id,sum(成功+一般) as '成功',sum(失败) as '失败',sum(停机) as '停机',sum(关机) as '关机',sum(其他) as '其他',sum(空号传真) as '空号传真',sum(无人接听) as '无人接听',sum(预约) as '预约',sum(非本店) as '非本店' ".$case_sql." from( select user,campaign_id".$group_sql." from (".$data_sql.") datas group by user,campaign_id order by null) datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users users on datas.工号=users.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
			...
			$case_sql=",sum(成功+一般) as '定制量',sum(失败) as '拒绝用户',sum(停机+关机+其他+空号传真) as '停关机用户',sum(无人接听) as '无人接听',sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p.") as '外呼总量',sum(成功+一般+失败) as '接触用户数',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/sum(成功+一般+失败+预约+非本店".$sql_drop_p."))*100,2),'%') end as '推荐成功率',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((sum(成功+一般+失败+预约+非本店".$sql_drop_p.")/sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '有效接通率'".$sum_drop;
...
case when 工号='HeJi' then '合计' when 工号='XiaoJi' then '小计' when 工号='ZongJi' then '总计' else 工号 end as '工号',users.full_name as '姓名',case when 工号 is not null and datas.campaign_id is null and sums='0' then '合计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']')) end as '业务'".$fields_list." from(select case when user is null then 'ZongJi' else user end as '工号',case when user is null then '1' else '0' end as 'sums' ,user,campaign_id".$case_sql." from( select user,campaign_id".$group_sql." from (".$data_sql.") datas group by user,campaign_id order by null) datas group by user,campaign_id ".$with_rollup." )datas left join vicidial_users users on datas.工号=users.user left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
	...
	case "get_work_cam":
		...
		$sql="select case when yw='HeJi' then '合计' when yw='XiaoJi' then '小计' when yw='ZongJi' then '总计' else ifnull(cam.campaign_name,concat('未知业务 [',datas.campaign_id,']'))end as 'yw',坐席数 as zxs,成功 as cg,总量 as zl,接通率 as jtl,整体成功率 as ztcgl,未质检 as wzj,不合格率 as bhgl from(select case when campaign_id is null then 'HeJi' else campaign_id end as 'yw',case when campaign_id is null then '1' else '0' end as 'sums' ,sum(case when user is not null and user!='VDAD' then 1 else 0 end ) as '坐席数',campaign_id,SUM(成功+一般) as '成功' ,SUM(未质检) as '未质检' ,sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p.") as '总量',case when sum(成功+一般+失败+预约+非本店".$sql_drop_p.")=0 then concat(0,'%') else concat(ROUND((sum(成功+一般+失败+预约+非本店".$sql_drop_p.")/sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '接通率',case when SUM(成功+一般)=0 then concat(0,'%') else concat(ROUND((SUM(成功+一般)/sum(成功+一般+失败+其他+关机+停机+无人接听+空号传真+预约+非本店".$sql_drop_p."))*100,2),'%') end as '整体成功率',case when SUM(不合格)=0 then concat(0,'%') else concat(ROUND((SUM(不合格)/SUM(未质检+不合格+质量差+普通+优秀))*100,2),'%') end as '不合格率' from( select campaign_id,user,sum(case when status_name='成功' then 1 else 0 end ) as '成功',sum(case when status_name='一般' then 1 else 0 end ) as '一般',sum(case when status_name='失败' then 1 else 0 end ) as '失败',sum(case when status_name='其他' then 1 else 0 end ) as '其他',sum(case when status_name='关机' then 1 else 0 end ) as '关机',sum(case when status_name='停机' then 1 else 0 end ) as '停机',sum(case when status_name='预约' then 1 else 0 end ) as '预约',sum(case when status_name='非本店' then 1 else 0 end ) as '非本店'".$sql_wrjt.",sum(case when status_name='空号传真' then 1 else 0 end ) as '空号传真',sum(case when quality_status='0' and status_name in('成功','一般') then 1 else 0 end ) as '未质检',sum(case when quality_status='1' then 1 else 0 end ) as '不合格',sum(case when quality_status='2' then 1 else 0 end ) as '质量差',sum(case when quality_status='3' then 1 else 0 end ) as '普通',sum(case when quality_status='4' then 1 else 0 end ) as '优秀' from (select a.campaign_id,a.user,a.quality_status,b.status_name from vicidial_log a left join data_sys_status b on a.status=b.status and b.status_type='call_status' where a.call_date BETWEEN '".$yes_day_s."' and '".$yes_day_e."' and a.campaign_id!='') datas group by campaign_id,user  order by null) datas group by campaign_id ".$with_rollup."  order by null)datas left join vicidial_campaigns cam on datas.campaign_id=cam.campaign_id";
