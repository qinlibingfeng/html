...
class AJAXChat {
	...
	function initialize() {
		...
		//echo $this->getChatViewMessagesXML();exit;
		//弹屏数据获取。
		if ($this->_requestVars['tanpingStatus']!=NULL && $this->_requestVars['tanpingStatus']=="1"){
			echo $this->getLastMessageID();
			exit;
		}elseif ($this->_requestVars['tanpingStatus']!=NULL && $this->_requestVars['tanpingStatus']=="2"){
			echo $this->checkIfMessage($this->_requestVars['lastID']);
			exit;
		}elseif ($this->_requestVars['tanpingStatus']!=NULL && $this->_requestVars['tanpingStatus']=="3"){
			echo $this->changeMessageStatus($this->_requestVars['messageID']);
			exit;
		}
		
		//提交群发短信
		if($this->_requestVars['view']=="sendMsg"){
			$names		= isset($_REQUEST['names'])		? $_REQUEST['names']		: "";
			$msg	= isset($_REQUEST['inputField'])? $_REQUEST['inputField']		: "";
			$personArr = explode(",", $names);
			$successArr = array();
			foreach ($personArr as $p){
				if(trim($p)!=""){
					$toUserID = $this->getIDFromNameByVCDial2($p);
					if($toUserID != "") {
						$this->insertCustomMessage($this->getUserID(),$this->getUserName(),0,$this->getPrivateMessageID($toUserID),'/privmsg '.$msg);
						$successArr[] = $this->getUserFullNameById($p);
					}
				}
			}
			$successUserStr = implode(",", $successArr);
			$successUserStr = str_replace(" ","_",$successUserStr);
			$this->insertCustomMessage(
						$this->getUserID(),
						$this->getUserName(),
						$this->getUserRole(),
						$this->getPrivateMessageID(),
						'/privmsgto '.''. $successUserStr .' '.$msg
					);			
			echo "Success Users : " . implode(",", $successArr) . "<script type=\"text/javascript\">window.close();</script>";
			exit;
		}
		
		//echo $this->getUserWordHistory();exit;
	...
	function initRequestVars() {
		...
		$this->_requestVars['messageID']	= isset($_REQUEST['messageID'])		? (int)$_REQUEST['messageID']	: null;
		$this->_requestVars['talk_level']	= isset($_REQUEST['talk_level'])	? (int)$_REQUEST['talk_level']	: null;
		$this->_requestVars['admin_level']	= isset($_REQUEST['admin_level'])	? (int)$_REQUEST['admin_level']	: null;
		//用于处理弹屏
		$this->_requestVars['tanpingStatus']= isset($_REQUEST['tanpingStatus'])	? $_REQUEST['tanpingStatus']	: null;
		//处理搜索
		if($this->_requestVars['view']=="check"){
			$from_date = date("Y-m-d",time()) . " 00:00:00";
			$to_date = date("Y-m-d",time()) . " 23:59:59";
			$this->_requestVars['from']		= isset($_REQUEST['from'])		? $_REQUEST['from']			: $from_date;
			$this->_requestVars['to']		= isset($_REQUEST['to'])		? $_REQUEST['to']			: $to_date;
			$this->_requestVars['send']		= isset($_REQUEST['send'])		? $_REQUEST['send']			: "";
			$this->_requestVars['receive']	= isset($_REQUEST['receive'])	? $_REQUEST['receive']		: "";
	...
	function initDataBaseConnection() {
		...
		$this->vicidialDB = new AJAXChatDataBase($this->_config['dbvicidial']);
		...
		// Use a new database connection if no existing is given:
		if(!$this->_config['dbvicidial']['link']) {
			// Connect to the database server:
			$this->vicidialDB->connect($this->_config['dbvicidial']);
			if($this->vicidialDB->error()) {
				echo $this->vicidialDB->getError();
				die();
			}
			// Select the database:
			$this->vicidialDB->select($this->_config['dbvicidial']['name']);
			if($this->vicidialDB->error()) {
				echo $this->vicidialDB->getError();
				die();
			}
	...
	function sendXHTMLContent() {
		...
		//echo $this->getTemplateFileName();exit;
		// Send HTTP header:
		$httpHeader->send();		
	...
	function getTemplateFileName() {
		switch($this->getView()) {
			...
			case 'history':
				return AJAX_CHAT_PATH.'lib/template/history.html';
			case 'check':
				return AJAX_CHAT_PATH.'lib/template/chat.html';
			default:
	...
	function login() {
		...
		*/
		...
		$this->setSessionVar("groupName", $userData['userGroup']);
		//echo $userData['userGroup'].'---'.$this->getRequestVar('groupName');exit;
	...
	function insertParsedMessage($text) {
		...
		//check if the user has the right to send message.
		$user_level = $this->getUserLevel($this->getUserName());
		
		$talk_level = $this->_config['talk_level'];
		if($this->_requestVars['talk_level']!=null){
			$talk_level = $this->_requestVars['talk_level'];
		}
		
		if ($user_level < $talk_level){
			
			//$this->insertChatBotMessage($this->getPrivateMessageID(),"你没有权发送信息，只能查看信息");
	...
	function insertParsedMessagePrivMsg($textParts) {
		if($this->isAllowedToSendPrivateMessage()) {
			if(count($textParts) < 3) {
				...
				$toUserID = $this->getIDFromNameByVCDial($textParts[1]);
				...
				if($toUserID === null) {
					...
					$this->insertCustomMessage(
						...
						$this->getUserRole(),
						$this->getPrivateMessageID(),
						$command.'to '.$this->getUserFullNameById($textParts[1]).' '.implode(' ', array_slice($textParts, 2))
					);								
	...
	function getUserByGroupName(){
		$users = array();
		$groupName = $this->getSessionVar("groupName");
		mysql_query("SET NAMES UTF8");
		$sql = "select user,full_name from vicidial_users where active='Y' and user_group='" . $groupName . "'";
		...
		while($row = $result->fetch()) {
			$users[] = $row['user'].'+'.$row['full_name'];
			//echo iconv('GB2312', 'utf-8', $row['full_name']);
			//echo $row['full_name'];
		}
		$result->free();
		return $users;
	...
	function getInfoByUser($user){
		$sql = "select user_group,user_id from vicidial_users where user='" . $user . "'";
		$result = $this->vicidialDB->sqlQuery($sql);
		$arr = array();
		if ($result->error()){
			echo $result->getError();exit;
		...
		if($row = $result->fetch()) {
			$arr["userGroup"] = $row['user_group'];
			$arr["userId"] = $row["user_id"];
		}
		return $arr;
	}
	function getUserWordHistory(){
		$userName = $this->getUserName();
		
		$str = "";
		$sql = "select userName,userFullName,dateTime,text from ajax_chat_messages where userName='" . $userName . "' and (channel=0 or channel=" . $this->getPrivateMessageID() . ") and text not like '/privmsg %'";
		$result = $this->db->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();
			die();
		...
		while($row = $result->fetch()) {
			if($i%2==0){
				$str .= '<div class="rowOdd">';
			}else{
				$str .= '<div class="rowEven">';
			}
			
			$content = $this->encodeSpecialChars($row['text']);
			$content = str_ireplace("/privmsgto","发送私人信息",$content);
			$str .= '<span class="dateTime">('.$row['dateTime'].')</span><span class="guest">'.$row['userFullName'].'</span>: ' . $content . '</div>';
			++$i;
		}
		$result->free();
		return $str;
	}
	function getUserWordHistoryCheck(){
		$condition = "";
		if (trim($this->getRequestVar("from"))!=""){
			$condition = $condition . " and msg.dateTime>='" . $this->getRequestVar("from") . "'";
		}
		if (trim($this->getRequestVar("to"))!=""){
			$condition = $condition . " and msg.dateTime<='" . $this->getRequestVar("to") . "'";
		}
		if (trim($this->getRequestVar("send"))!=""){
			$condition = $condition . " and msg.userName = '" . $this->getRequestVar("send") . "'";
		}
		if (trim($this->getRequestVar("receive"))!=""){
			$arr = $this->getInfoByUser($this->getRequestVar("receive"));
			$condition = $condition . " and ((msg.channel=0 and msg.groupname='".$arr["userGroup"]."') or msg.channel=" . $this->getPrivateMessageID($arr["userId"]) . ")";
		}
		$userName = $this->getUserName();
		//echo $condition;exit;
		$str = "";
		$sql = "select msg.id,msg.groupName,msg.userName,msg.userFullName,msg.channel,msg.dateTime,msg.text,us.mid as status from ajax_chat_messages msg left join " .$this->getDataBaseTable('messages_user')." us on (msg.id=us.messageId and msg.channel=1000000000+us.userId) where msg.text NOT LIKE '%/privmsgto%'". $condition;
		//echo $sql;exit;
		$result = $this->db->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();
		...
		while($row = $result->fetch()) {
			$status = "";
			$receiver = "";
			$gropuName = $row['groupName'];
			if($row['channel']!="0"){
				$id_temp = $row['channel'] - 1000000000;
				$receiver = '' .$this->getNameFromIdByVCDial($id_temp) . '';
			}else{
				$receiver = '组' . $gropuName . '';
				$status = $this->getTangpingUsersByMessageId($row['id']);
			...
			$content = $this->encodeSpecialChars($row['text']);
			//$content = str_ireplace("/privmsgto","",$content);
			$content = str_ireplace("/privmsg","",$content);
			$str .= '<span class="dateTime">'.$row['dateTime'].'</span>&nbsp;<span class="guest">'.$row['userFullName']. '</span>对<span class="guest">' . $receiver . '</span>说：' . $content . $status . '</div>';
			++$i;
		}
		$result->free();
		return $str;
	}
	function getTangpingUsersByMessageId($messageId){
		$sql = "select userId from " .$this->getDataBaseTable('messages_user')." where messageId=". $messageId;
		$usersId = array();
		$usersName = array();
		$result = $this->db->sqlQuery($sql);
		if ($result->error()){
		...
		$sql = "select full_name from vicidial_users where user_id in (" . implode(",", $usersId) . ")";
		
		$result2 = $this->vicidialDB->sqlQuery($sql);
		while($row2 = $result2->fetch()){
			$usersName[] = $row2['full_name'];
		}
		
		$result2->free();
		if(count($usersName)>0){
			return "(已弹屏用户:" . implode(",", $usersName) . ")";
		}else{
			return "";
		}
	...
	function getMessageCondition() {
		//修改了$condition，增加groupName组控制。
		$condition = 	'id > '.$this->db->makeSafe($this->getRequestVar('lastID')).'
						AND (
							(channel = '.$this->db->makeSafe($this->getChannel()).' and groupName = '.$this->db->makeSafe($this->getSessionVar("groupName")) . ')
							OR
							channel = '.$this->db->makeSafe($this->getPrivateMessageID()).'
						)
						AND
	...
	function getChatViewMessagesXML() {
		...
		$sql = 'SELECT
					...
					msg.userFullName,
					msg.userRole,
					msg.channel AS channelID,
					UNIX_TIMESTAMP(msg.dateTime) AS timeStamp,
					msg.text,
					us.mid as status
				FROM
					'.$this->getDataBaseTable('messages').' msg
				left join '.$this->getDataBaseTable('messages_user').' us on (msg.id=us.messageId and us.userID='.$this->getUserId().')
				WHERE
	...
	function getIDFromNameByVCDial($userName){
		$sql = "select user_id from vicidial_users where user='" . $userName . "'";
		$result = $this->vicidialDB->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();exit;
		}
		...
		if ($row['user_id']==''){
			die("User isn't exist!");
		}
		
		return  $row['user_id'];
	}
	function getIDFromNameByVCDial2($userName){
		$sql = "select user_id from vicidial_users where user='" . $userName . "' and user_group='" . $this->getSessionVar("groupName") . "'";
		$result = $this->vicidialDB->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();exit;
		}
	...
	function getNameFromIdByVCDial($id){
		$sql = "select full_name from vicidial_users where user_id='" . $id . "'";
		$result = $this->vicidialDB->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();exit;
		}
	...
	function getUserLevel($name){
		$i = 0;
		$sql = "select user_level from vicidial_users where user='" . $name . "'";
		$result = $this->vicidialDB->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();exit;
		}
	...
	function createGuestUserID($userName) {
		// seed with microseconds since last "whole" second:
		
		return $this->getIDFromNameByVCDial($userName);
	}
	...
	function getGuestUser() {
		...
		$userData = array();
		$userData['userID'] = $this->createGuestUserID($userName);
		$userData['userName'] = $userName;
		$userData['userGroup'] = $this->getGroupByName($userName);
	...
	function getGroupByName($name){
		$i = '';
		$sql = "select user_group from vicidial_users where user='" . $name . "'";
		$result = $this->vicidialDB->sqlQuery($sql);
		if ($result->error()){
			echo $result->getError();exit;
	...
	function getLastMessageID(){
		$i = '';
		$sql = "select id from ajax_chat_messages order by id desc limit 0,1";
		$result = $this->db->sqlQuery($sql);
		if ($result->error()){
			echo "0";
		}
	...
	//主要是修改channel通道为默认值
	function getMessageConditionByTP() {
		$condition = 	'id > '.$this->db->makeSafe($this->getRequestVar('lastID')).'
						AND (
							(channel = 0 and groupName = '.$this->db->makeSafe($this->getSessionVar("groupName")) . ')
							OR
							channel = '.$this->db->makeSafe($this->getPrivateMessageID()).'
						)
						AND
	...
	function checkIfMessage(){
		...
		if($row = $result->fetch()) {
			//插入数据
			$sql_temp = 'INSERT INTO '.$this->getDataBaseTable('messages_user').'(
				userId,
				messageId
			)
			VALUES (
				'.$this->db->makeSafe($this->getUserID()).',
				'.$this->db->makeSafe($row['id']).'

			);';
			// Create a new SQL query:
			$result_temp = $this->db->sqlQuery($sql_temp);
			//$result_temp->free();
	...
	function changeMessageStatus($messageID){
		...
		$sql = 'SELECT count(*) as c
				FROM
					'.$this->getDataBaseTable('messages_user').'
				WHERE
	...
	//返回用户列表
	function getUserOptions($type) {
		// Get the last messages in descending order (this optimises the LIMIT usage):
		mysql_query("SET NAMES UTF8");
		$sql = "SELECT user,full_name from vicidial_users where active='Y' and user_group='" . $this->getSessionVar("groupName") . "'";
		...
		// Add the messages in reverse order so it is ascending again:
		while($row = $result->fetch()) {
			if ($row['user']==$this->getRequestVar($type)){
				$options = $options . '<option value="'.$row['user'].'" selected="selected">'.$row['full_name'].'</option>';
			}else{
				$options = $options . '<option value="'.$row['user'].'">'.$row['full_name'].'</option>';
			}		
		}
		$result->free();	
		
		$options = '<select name="'.$type.'" id="'.$type.'">'.$options.'</select>';
		return $options;
	}
	function getUserFullNameById($userid){
		mysql_query("SET NAMES UTF8");

		$sql = "SELECT full_name from vicidial_users where user='" . $userid . "'";
		$full_name = "";
		// Create a new SQL query:
		$result = $this->vicidialDB->sqlQuery($sql);
