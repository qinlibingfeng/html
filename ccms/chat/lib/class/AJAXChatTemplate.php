...
class AJAXChatTemplate {
	...
	function replaceTemplateTags($tag, $tagContent) {
		switch($tag) {
			...
			case 'HTML_DISABLE':
				return $this->getHtmlDisable();
			case 'HTML_DISPLAY_BLOCK_NONE':
				return $this->getHtmlDisplayBlockOrNone();
			case 'CHAT_USER_HISTORY':	
				return $this->ajaxChat->getUserWordHistory();
			case 'CHAT_USER_HISTORY_CHECK':	
				return $this->ajaxChat->getUserWordHistoryCheck();
			case 'COOKIE_DOMAIN':
			...
			case 'SEARCH_FORM_FROM':
				return $this->getSearchChatField("from");
			case 'SEARCH_FORM_TO':
				return $this->getSearchChatField("to");
			case 'SEARCH_FORM_SEND':
				return $this->getSearchChatField("send");
			case 'SEARCH_FORM_RECEIVE':
				return $this->getSearchChatField("receive");
			default:
	...
	function getHtmlDisable(){
		$talk_level = $this->ajaxChat->_config['talk_level'];
		if($this->ajaxChat->_requestVars['talk_level']!=null){
			$talk_level = $this->ajaxChat->_requestVars['talk_level'];
		}
		
		$user_level = $this->ajaxChat->getUserLevel($this->ajaxChat->getUserName());
	...
	function getHtmlDisplayBlockOrNone(){
		$admin_level = $this->ajaxChat->_config['admin_level'];
		if($this->ajaxChat->_requestVars['admin_level']!=null){
			$admin_level = $this->ajaxChat->_requestVars['admin_level'];
		}
		$user_level = $this->ajaxChat->getUserLevel($this->ajaxChat->getUserName());
	...
	function getSearchChatField($type){
		
		switch ($type){
			case 'from':
				return '<input type="text" name="from" id="from" value="'.$this->ajaxChat->getRequestVar($type).'"/>';
			case 'to':
				return '<input type="text" name="to" id="to" value="'.$this->ajaxChat->getRequestVar($type).'"/>';
			case 'send':
				return $this->ajaxChat->getUserOptions($type);
			case 'receive':
				return $this->ajaxChat->getUserOptions($type);
		}
	}
	...
	function getGroupUserList(){
		/*
	  			<div class="rowOdd">
		  			<a href="#" class="guest">(ww)</a>
		  		</div>
		  		<div class="rowEven">
		  			<a href="#" class="guest">(beaddddra)</a>
		  		</div>

		 */
		$users = $this->ajaxChat->getUserByGroupName();
		$num = 0;
		$result = '';
		...
		foreach ($users as $u){
			$u_arr = explode("+", $u);
			if($num % 2 == 0){
				$result .= '<div class="rowOdd">';
			}else{
				$result .= '<div class="rowEven">';
			}
			$result .= '<a href="javascript:ajaxChat.insertMessageWrapper(\'/msg ' . $u_arr[0] . ' \');" class="guest">' . iconv('utf-8','utf-8',$u_arr[1]) . '</a></div>';
	...
	function getGroupName(){
		return $this->ajaxChat->getSessionVar("groupName");
	}
