...
var ajaxChat = {
	...
	handleXML: function(xmlDoc) {
		...
		//this.handleOnlineUsers(xmlDoc.getElementsByTagName('user'));
		this.handleChatMessages(xmlDoc.getElementsByTagName('message'));
		//this.handleChatMyMessages(xmlDoc.getElementsByTagName('mymessage'));
	...
	addMessageToChatList: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		//if(this.tanping_Status){
		//	alert(messageText);
		//	messageText.replace("未弹屏","已弹屏");
		//	this.tanping_Status = false;
		//}
		this.DOMbuffer = this.DOMbuffer + 
	...
	getChatListMessageString: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		var dateTime = this.settings['dateFormat'] ? '<span class="dateTime">'
						+ this.formatDate('(%Y-%m-%d %H:%i:%s)', dateObject) + '</span> ' : '';
		return	'<div id="'
	...
	blinkOnNewMessage: function(dateObject, userID, userName, userRole, messageID, messageText, channelID, ip) {
		...
		if(this.settings['blink'] && this.lastID && !this.channelSwitch && userID != this.userID) {
			//处理弹屏状态
			$.get("index.php?", { messageID: messageID, tanpingStatus:"3"},
			   function(data){
				//if(data == "true"){
				//	this.tanping_Status = true;
				//}else{
				//	this.tanping_Status = false;
				//}
			...
			window.focus();
			alert(userName + "说:" + messageText.replace("/privmsg","(悄悄话)"));
		}
	},
	...
	updateMyChatlistView: function() {		
		if(this.dom['myChatList'].childNodes && this.settings['maxMessages']) {
			while(this.dom['myChatList'].childNodes.length > this.settings['maxMessages']) {
				this.dom['myChatList'].removeChild(this.dom['myChatList'].firstChild);
			}
		}
		
		if(this.settings['autoScroll']) {
			this.dom['myChatList'].scrollTop = this.dom['myChatList'].scrollHeight;
		}
	},
	...
	logout: function() {
		...
		try{
			//通知父窗口，该窗口关闭。
			window.opener.document.getElementById("win_status").value = "no";
			//更新父窗口中lastID与之同步
			window.opener.document.getElementById("win_lastID").value = this.lastID;
		}catch(err){
	
		}
		*/
		alert("关闭窗口将接收不到即时信息，请在主界面点击“即时信息”！");
		var message = 'logout=true';
		this.makeRequest(this.ajaxURL,'POST',message);
		
		this.settingsInitiated = null;
		this.styleInitiated = null;
		this.initializeFunction = null;
		this.finalizeFunction = null;
		this.loginChannelID = null;
		this.loginChannelName = null;
		this.timerRate = null;
		this.timer = null;
		this.ajaxURL = null;
		this.baseURL = null;
		this.regExpMediaUrl = null;
		this.dirs = null;
		this.startChatOnLoad = null;
		this.chatStarted = null;
		this.domIDs = null;
		this.dom = null;
		this.settings = null;
		this.nonPersistentSettings = null;
		this.unusedSettings = null;
		this.bbCodeTags = null;
		this.colorCodes = null;
		this.emoticonCodes = null;
		this.emoticonFiles = null;
		...
		this.cookieSecure = null;
		this.chatBotName = null;
		this.chatBotID = null;
		this.allowUserMessageDelete = null;
		this.inactiveTimeout = null;
		this.privateChannelDiff = null;
		this.privateMessageDiff = null;
		this.showChannelMessages = null;
		this.messageTextMaxLength = null;
		this.socketServerEnabled = null;
		this.socketServerHost = null;
		this.socketServerPort = null;
		this.socketServerChatID = null;
		this.socket = null;
		this.socketIsConnected = null;
		this.socketTimerRate = null;
		this.socketReconnectTimer = null;
		this.socketRegistrationID = null;
		this.userID = null;
		this.userName = null;
		this.userRole = null;
		this.channelID = null;
		...
		this.userMenuCounter = null;
		this.encodedUserName = null;
		this.userNodeString = null;
		this.ignoredUserNames = null;
		this.lastID = null;
		this.tanping_Status= null;
		this.localID = null;
		this.lang = null;
		this.langCode = null;
		...
		this.retryTimerDelay= null;
		this.DOMbuffering = null;
		this.DOMbuffer = null;
		this.DOMbufferRowClass= null;
	},
