...
if ( $usergroup_ct > 0  ){
	   if ( $usergroup_ct == 1 && ( trim($usergroup[0]) == '' || ereg('ALL',$usergroup[0]) ) ){
	   	    $allusergroup  = true;
	   }
	   for( $i=0; $i< $usergroup_ct;$i++){ 
	   	    if ( ereg('ALL',$usergroup[$i])){
	   	    	$allusergroup  = true;
	   	    }else{
	   	    	 $A_usergroup[] = trim($usergroup[$i]);
	   	    }
	   	    
		   	  $select_usergroup = $select_usergroup . trim($usergroup[$i]) . "|";
		   	  $usergroup_QS     = $usergroup_QS . "&usergroup[]= " . trim($usergroup[$i]);    	
	   } 	
}
...
if ( $allusergroup  ){
    ...
    $usergroup_sql 	  = " user_group in ('" . implode("','",$A_usergroup) . "')";
}
	 	 
//echo $usergroup_ct . "|" . trim($usergroup[0]) . "|". $usergroup_sql;
...
while($i < $group_ct)
{
	...
	if (eregi('ALL-ACTIVE',$group_string))
		...
		if ( (ereg('Y',$with_inbound)) and ($campaign_allow_inbound > 0) )
			...
			$stmt="select auto_dial_level,dial_status_a,dial_status_b,dial_status_c,dial_status_d,dial_status_e,lead_order,lead_filter_id,hopper_level,dial_method,adaptive_maximum_level,adaptive_dropped_percentage,adaptive_dl_diff_target,adaptive_intensity,available_only_ratio_tally,adaptive_latest_server_time,local_call_time,dial_timeout,dial_statuses,agent_pause_codes_active,list_order_mix from vicidial_campaigns where campaign_id IN ('$current_campaign_id',$closer_campaignsSQL_temp);";

			$stmtB="select sum(dialable_leads),sum(calls_today),sum(drops_today),avg(drops_answers_today_pct),avg(differential_onemin),avg(agents_average_onemin),sum(balance_trunk_fill),sum(answers_today),max(status_category_1),sum(status_category_count_1),max(status_category_2),sum(status_category_count_2),max(status_category_3),sum(status_category_count_3),max(status_category_4),sum(status_category_count_4) from vicidial_campaign_stats where campaign_id IN ('$current_campaign_id',$closer_campaignsSQL_temp);";
			}
		else
			{
			$stmt="select avg(auto_dial_level),max(dial_status_a),max(dial_status_b),max(dial_status_c),max(dial_status_d),max(dial_status_e),max(lead_order),max(lead_filter_id),max(hopper_level),max(dial_method),max(adaptive_maximum_level),avg(adaptive_dropped_percentage),avg(adaptive_dl_diff_target),avg(adaptive_intensity),max(available_only_ratio_tally),max(adaptive_latest_server_time),max(local_call_time),max(dial_timeout),max(dial_statuses),max(agent_pause_codes_active),max(list_order_mix) from vicidial_campaigns where campaign_id IN('$current_campaign_id');";

			$stmtB="select sum(dialable_leads),sum(calls_today),sum(drops_today),avg(drops_answers_today_pct),avg(differential_onemin),avg(agents_average_onemin),sum(balance_trunk_fill),sum(answers_today),max(status_category_1),sum(status_category_count_1),max(status_category_2),sum(status_category_count_2),max(status_category_3),sum(status_category_count_3),max(status_category_4),sum(status_category_count_4) from vicidial_campaign_stats where campaign_id IN('$current_campaign_id');";
			}
		}
	if ($DB > 0) {echo "\n|$stmt|$stmtB|\n";}
	...
	$stmt="select sum(local_trunk_shortage) from vicidial_campaign_server_stats where campaign_id ='$current_campaign_id'"; 
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$balanceSHORT = $row[0];
	...
	$str_table_top_info .= "<tr><td colspan=\"8\" class=\"font_date2\"><strong>Campaign ID: </strong>$current_campaign_id</td></tr><TR>";
...
if ( $allusergroup === false ){
	 $usergroupSQL = $usergroupSQL . " and " . $usergroup_sql;
}else{
	 $usergroupSQL = '';
}
	...
	while ($j < $talking_to_print)
		...
		if (eregi("INCALL",$Lstatus)) 
			...
			if ($parked_channel > 0)
				...
				{
				$stmtP="SELECT count(*) FROM vicidial_remote_agents where server_ip='$Aserver_ip[$i]' and conf_exten='$Auser[$i]';";
				$rsltP=mysql_query($stmtP,$link);
				$rowP=mysql_fetch_row($rsltP);
				$remote_agent = $rowP[0];
				## Add by fnatic
