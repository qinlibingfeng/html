...
if ($user_login_first == 1)
{
	...
	echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"./images/vicidial_logo.gif\" BORDER=0></TD>";
...
echo "		</td>\n";
echo "        <td rowspan=3 background=./images/dispo_r3_c5.jpg>&nbsp;</td>\n";
echo "        <td><img src=./images/spacer.gif border=0 height=15 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td><img src=./images/spacer.gif border=0 height=36 width=1></td>\n";
echo "      </tr>\n";
...
$VDloginDISPLAY=0;
	...
	if($auth>0)
		...
		if($CAMPactive>0)
            ...
            $Manual_Ring_Launch='NONE';
			$Custom_Dispo='N';
			$Custom_Dispo_Script=""; 
			$WebForm_Button_Display='YES';
			if ($VD_campaign == '006') {
            $Manual_Ring_Launch='SCRIPT';
			$Custom_Dispo='Y';
			$Custom_Dispo_Script="";
			$WebForm_Button_Display='NONE';
			$Skip_Choose_InGroups = 'Y';
			$Skip_Select_PauseCode = 'Y';
			$Skip_Auto_hangup = 'Y';
			}
...
$SLheight =  ($MASTERheight + 169);	# Mod by fnatic +50
...
?>
	...
	var Skip_Choose_InGroups = '<?php echo $Skip_Choose_InGroups ?>';
	var Skip_Select_PauseCode = '<?php echo $Skip_Select_PauseCode ?>';
	var Skip_Auto_hangup = '<?php echo $Skip_Auto_hangup ?>';
    //Modified by Kelvin End
...
?>
    ...
    //End
	var script_height = '<?php echo $SSheight+30 ?>';
		...
		if (xmlhttp) 
			...
			//	filename = filedate + "_" + user_abb;
				...
				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					//Del by fnatic document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"./images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Start Recording\">";
			...
			//		alert(xmlhttp.responseText);
					...
					if (RClookID_array.length > 0)
						...
						//DEL BY FNATIC document.getElementById("RecorDingFilename").innerHTML = RecDispNamE;
						//DEL BY FNATIC document.getElementById("RecorDID").innerHTML = RClookID_array[1];
						}
					}
				}
			delete xmlhttp;
			...
			//		alert(xmlhttp.responseText);
				...
				//		alert("VICIDIAL Call log entered:\n" + document.vicidial_form.uniqueid.value);
						...
						//		recording_id = MDlogRecorDings_array[3];
								...
								//DEL BY FNATIC document.getElementById("RecorDingFilename").innerHTML = RecDispNamE;
								//DEL BY FNATIC document.getElementById("RecorDID").innerHTML = MDlogRecorDings_array[3];
								}
							}
						}
					}
	...
	function ManualDialCheckChanneL(taskCheckOR)
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(MDlookCID); callee ring status
					if (MDlookCID == "NO")
						{
						MD_ring_secondS++;
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								var callnum = dialed_number;
								var dial_display_number = phone_number_format(callnum);
								phone_location(callnum);

								document.getElementById("MainStatuSSpan").innerHTML = " Incoming: " + dial_display_number + "<span id='location'></span> Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
								}
...
// Update vicidial_list lead record with disposition selection
	function DispoSelect_submit()
		...
		<? //yanson campaign edit@20100831
		 if($VD_campaign == '006'){ 
		?>
			//document.vicidial_form.DispoSelection.value = 'SALE';
		...
		if (DispoChoice.length < 1) {alert("You Must Select a Disposition");}
			...
			if ( (DispoChoice == 'CALLBK') && (scheduled_callbacks > 0) ) {showDiv('CallBackSelectBox');}
				...
				document.getElementById("GENDERhideFORie").innerHTML = '<select style="display:none" size=1 name=gender_list class="cust_form" id=gender_list><option value="U">U - Undefined</option><option value="M">M - Male</option><option value="F">F - Female</option></select>';
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
	{
		...
		{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>GROUPS NOT SELECTED</B></td><td><B>SELECTED GROUPS</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd> &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('---DD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>";
			var loop_ct = 0;
		...
		else
		{
			...
			document.getElementById("CloserSelectContent").innerHTML = live_CSC_HTML;
			//yanson@20100926
			if(Skip_Choose_InGroups == 'Y'){
				CloserSelect_submit();
			}
			if(Skip_Select_PauseCode == 'Y'){
				PauseCodeSelect_submit('Docume');
			}
...
// Move a Closer In Group record to the selected column or reverse
	...
	//	alert(taskCSgrp+"|"+taskCSchange+"|"+CloserSelectListValue.length+"|"+CSCchange+"|"+CSCcolumn+"|"+INgroupCOUNT)
		...
		if (CSCchange==1) 
			...
			document.getElementById("CloserSelectAdd").innerHTML = " &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('---DD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>" + live_CSC_HTML_ADD;
...
//	alert("TERR: " + TerritorySelectListValue + "\nCHANGE: " + TERRchange + "\nACTION: " + taskTERRchange + "\nSELECTED: " + taskTERRgrp + "\nTOTAL: " + territoryCOUNT);
		if (TERRchange==1) 
			...
			document.getElementById("TerritorySelectAdd").innerHTML = " &nbsp; <a href=\"#\" onclick=\"TerritorySelect_change('---DD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>" + live_TERR_HTML_ADD;
...
function phone_location(phone){
		var xmlHttp = false;
		  ...
		  if(xmlHttp){
			...
			xmlHttp.open("POST", "phone_location.php", true);
			xmlHttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// Move the Dispo frame to the top and change the link to minimize
	function DispoMaximize()
		{
		document.getElementById("DispoSelectBox").style.top = 1;
		//Del by fnatic document.getElementById("DispoSelectMaxMin").innerHTML = "<a href=\"#\" onclick=\"DispoMinimize()\"> minimize </a>";
		hideDiv('DispoButtonHideA');
		hideDiv('DispoButtonHideB');
		hideDiv('DispoButtonHideC');
		}
	...
	function start_all_refresh()
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if (open_dispo_screen==1)
				...
				document.getElementById("GENDERhideFORieALT").innerHTML = '<select style="display:none" size=1 name=gender_list class="cust_form" id=gender_list><option value="U">U - Undefined</option><option value="M">M - Male</option><option value="F">F - Female</option></select>';
				...
				<? //yanson campaign edit@20100831
					if($VD_campaign == '006' ){ 
				?>
				document.getElementById("iframetree").innerHTML='<iframe  scrolling="auto" width="310px" height="680px" frameborder="no" border="0" marginwidth="0" marginheight="0" scrolling="no" allowtransparency="yes" src="ehsn_finish.php?canpaign=<?= $VD_campaign ?>" ></iframe>';
				...
				<?
                  ...
                  // if($VD_campaign == '006' && VD_live_customer_call==0 && ccms_006_called==1){
					    
						//echo "<script>DispoSelect_submit();</script>";
				  // }
...
<!--
	...
	div.scroll_script {height: <?=$SSheight-30 ?>px; width: <?=$SDwidth-50 ?>px; background:#FFCC99; overflow: scroll; font-size: 12px;  font-family:Arial, Helvetica, sans-serif;}
	div.text_input {overflow: auto; font-size: 10px;  font-family:Arial, Helvetica, sans-serif;}
   .body_text {font-size: 12px;  font-family:Arial, Helvetica, sans-serif;}
   .queue_text_red {font-size: 12px;  font-family:Arial, Helvetica, sans-serif; font-weight: bold; color: red}
   .queue_text {font-size: 12px;  font-family:Arial, Helvetica, sans-serif; color: black}
   .preview_text {font-size: 12px;  font-family:Arial, Helvetica, sans-serif; background: #F46E0B; color: #FFFFFF; font-weight: bold}
   .preview_text_red {font-size: 13px;  font-family:Arial, Helvetica, sans-serif; background: #FFCCCC}
   .banner_text {font-size:12px; font-family:Arial, Helvetica, sans-serif; color:#FFFFFF; font-weight:bold}
   .body_small {font-size: 11px;  font-family:Arial, Helvetica, sans-serif;}
   .body_tiny {font-size: 10px;  font-family:Arial, Helvetica, sans-serif;}
   ...
   .sd_text {font-size: 16px;  font-family:Arial, Helvetica, sans-serif; font-weight: bold;}
   .sh_text {font-size: 14px;  font-family:Arial, Helvetica, sans-serif; font-weight: bold;}
   .sb_text {font-size: 12px;  font-family:Arial, Helvetica, sans-serif;}
   .sk_text {font-size: 11px;  font-family:Arial, Helvetica, sans-serif;}
   .skb_text {font-size: 13px;  font-family:Arial, Helvetica, sans-serif; font-weight: bold;}
   ...
   .cust_form {font-family:Arial, Helvetica, sans-serif; font-size: 10px; overflow: hidden}
   .cust_form_text {font-family:Arial, Helvetica, sans-serif; font-size: 10px; overflow: auto}
    a {font-family: Arial, Helvetica, sans-serif;font-size:11px;color:#FF9900;}
	/*Add css by fnatic*/
  .ScriptPane1/*兼容FF IE6 7 8*/{width:820px!important;*width:100%;_width:100%;border:2px solid #eee;}  
  #hidden_show_control{float:left;}
  #button_pannel{ float:left;width:170px;}
  #pannel_img{ cursor:pointer;}
  div.button_link{text-align:left;margin:5px 0px 0px 0px;}
  div.button_link span{margin:0px 0px;}
  div.button_link a{font-size:11px;display:block;width:auto;height:18px;padding-left:5px;margin:0px 0px;}
  div.button_link a:hover,div.button_link a:active{background-color:#F60;color:#FFFFFF;}
  div.volume_button{ text-align:left; margin-left:3px;}
  #MainTable td {vertical-align:middle; text-align:left;}
  .MainStatuSDiv{width:79%;height:20px;float:left;font-size:11px;color:red;} 
  .queuediv{ float:left; width:20%;}
...
<TABLE width="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0 BGCOLOR=white height="30px" MARGINWIDTH=0 MARGINHEIGHT=0 LEFTMARGIN=0 TOPMARGIN=0 VALIGN=TOP ALIGN=LEFT>
  <TR VALIGN=TOP ALIGN=LEFT>
   <TD width="100px" COLSPAN=2 VALIGN=TOP ALIGN=LEFT  style="background:url(./images/filler.gif); background-repeat:repeat-x">
    <img src="./images/vicidial_logo_top.gif" align="middle" />
    <INPUT TYPE=HIDDEN NAME=extension>
  </TD>
  <TD style="background:url(./images/filler.gif); background-repeat:repeat-x;width:auto;">&nbsp;</TD>

  <TD class="banner_text_2" COLSPAN=4  VALIGN=TOP ALIGN=RIGHT style="background:url(./images/filler.gif); background-repeat:repeat-x" nowrap="nowrap" >
  <span style="color:#FFF; font-size:12px; font-family:Arial, Helvetica, sans-serif" id=status>LIVE</span>&nbsp;&nbsp;&nbsp;&nbsp;
<font class="banner_text">
  <span id="SecondSspan"><font class="body_text"> seconds: 
<span id="SecondSDISP"> &nbsp; &nbsp; </span>
</font></span>&nbsp; &nbsp;&nbsp; &nbsp;
  Customer Time: <input type=text size=20 name=custdatetime class="cust_form" value=""> &nbsp;
  Channel: <input type=text size=20 name=callchannel class="cust_form" value="">&nbsp; &nbsp;
...
<TR VALIGN=TOP ALIGN=LEFT>
<TD ALIGN=LEFT WIDTH=115><A HREF="#" onClick="MainPanelToFront('NO');"><IMG SRC="./images/customer_info.gif" ALT="VICIDIAL" WIDTH=143 HEIGHT=27 BORDER=0></A></TD>
<TD ALIGN=LEFT WIDTH=105><A HREF="#" onClick="ScriptPanelToFront();"><IMG SRC="./images/script_tab.jpg" ALT="SCRIPT" WIDTH=143 HEIGHT=27 BORDER=0></A></TD>
...
<span style="position:absolute;left:0px;top:0px;z-index:51;" id="CallBacKsLisTBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> CALLBACKS FOR AGENT <?php echo $VD_login ?>:<BR>Click on a callback below to call the customer back now. If you click on a record below to call it, it will be removed from the list.
	<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top><!-- NEW MANUAL DIAL LEAD FOR <?php //echo "$VD_login in campaign $VD_campaign" ?>:<BR><BR>Enter information below for the new lead you wish to call.-->
	<BR>
	<?php 
...
<!--<span style="position:absolute;left:15px;top:<?php echo $CBheight-60 ?>px;z-index:19;" id="VolumeControlSpan"><span id="VolumeUpSpan"><IMG SRC="./images/vdc_volume_up_off.gif" BORDER=0></span><BR><span id="VolumeDownSpan"><IMG SRC="./images/vdc_volume_down_off.gif" BORDER=0></span>
</font></span>-->
...
<span style="position:absolute;left:0px;top:<?php echo $CQheight ?>px;width:<?php echo $MNwidth ?>;overflow:scroll;z-index:23;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="callsinqueuedisplay"><TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0 width="100%"><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">Calls In Queue: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="callsinqueuelist">&nbsp;</span></TD></TR></TABLE></span>
...
<span style="position:absolute;left:<?php echo $SBwidth ?>px;top:85px;height:380;overflow:scroll;z-index:35;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="AgentViewSpan"><TABLE CELLPADDING=0 CELLSPACING=0 BORDER=0><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">
Other Agents Status: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="AgentViewStatus">&nbsp;</span></TD></TR></TABLE></span>

<span style="position:absolute;left:60px;top:<?php echo $BPheight ?>px;width:400;height:490;overflow:scroll;z-index:33;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="AgentXferViewSpan"><CENTER><font class="body_text">
...
<span style="position:absolute;left:0px;top:0px;z-index:41;width:100%;" id="WrapupBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> Call Wrapup: <span id="WrapupTimer"></span> seconds remaining in wrapup<BR><BR>
...
<span style="position:absolute;left:200px;top:150px;z-index:42;" id="TimerSpan">
    <table border=1 bgcolor="#F4F4F4" width=400 height=200><TR><TD align=center>
...
<span style="position:absolute;left:0px;top:0px;z-index:48;" id="DispoButtonHideC">
    <table border=0 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=47><TR><TD align=center VALIGN=top>Any changes made to the customer information below at this time will not be comitted, You must change customer information before you Hangup the call. </TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:49;width:100%;" id="DispoSelectBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> DISPOSITION CALL :<span id="DispoSelectPhonE"></span><span id="DispoSelectHAspan"><a href="#" onClick="DispoHanguPAgaiN()">Hangup Again</a></span> &nbsp; &nbsp; &nbsp; <span id="DispoSelectMaxMin"><a href="#" onClick="DispoMinimize()"> minimize </a></span><BR>
	<? //yanson campaign edit@20100831
	 if($VD_campaign == '006'){ 
	?>
	<span id="DispoSelectContent" style="display:none"> End-of-call Disposition Selection </span>
	<div id="iframetree"></div>
	<input type=checkbox name=DispoSelectStop size=1 value="0"> PAUSE AGENT DIALING <BR>
	<div style="display:none">
	<a id="DispoSelectSubmit" href="#" onClick="DispoSelect_submit();return false;">SUBMIT</a>
	<input type=hidden name=DispoSelection ><BR><!-- yanson target -->
...
<span style="position:absolute;left:0px;top:0px;z-index:64;width:100%;" id="PauseCodeSelectBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> SELECT A PAUSE CODE :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:65;width:100%;" id="GroupAliasSelectBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> SELECT A GROUP ALIAS :<BR>
...
<!--End-->
<span style="position:absolute;left:0px;top:0px;z-index:50;width:100%;" id="CallBackSelectBox">
    <!--<table border=1 bgcolor="#F4F4F4" width=<?php //echo $CAwidth ?> height=<?php //echo $WRheight ?>><TR><TD align=center VALIGN=top> Select a CallBack Date :<span id="CallBackDatE">-->
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?>><TR><TD align=center VALIGN=top> Select a CallBack Date :<span id="CallBackDatE">
    </span><BR>
	...
	?>
	CB Comments: <input type=text name="CallBackCommenTsField" id="CallBackCommenTsField" size=50 maxlength=255><BR>

	<a href="#" onClick="CallBackDatE_submit();return false;">SUBMIT</a><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:53;width:100%;" id="CloserSelectBox">
    <table border=1 bgcolor="#F4F4F4"  height=<?php echo $WRheight ?> width=<?php echo $CAwidth ?>><TR><TD align=center VALIGN=top> CLOSER INBOUND GROUP SELECTION <BR>
...
<!--MODIFY BY FNATIC <span style="position:absolute;left:154px;top:65px;z-index:30;" id="ScriptPanel"> -->
<span  style="position:absolute;left:172px;top:75px;z-index:30; text-align:left;" id="ScriptPanel">
    <table width="100%" border=0 bgcolor="#FFFFFF" height="auto"<?php //echo $SSheight+60 ?> class="ScriptPane1" cellpadding="0" cellspacing="0"><TR><TD align=left valign=top ><font class="sb_text">
    
    <div class="noscroll_script" width="100%" id="ScriptContents">AGENT SCRIPT</div>
...
<span style="position:absolute;left:0px;z-index:15;width:100%;" id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>><tr><td height=25><font class="body_small">
<span id="busycallsdisplay"><a href="#"  onclick="conf_channels_detail('SHOW');">Show conference call channel information</a>
<BR><BR>&nbsp;</span></font></td><td align=right height=25>
</td></tr>
...
<TR><TD align="center"><IMG SRC="./images/agc_live_call_OFF.gif" NAME=livecall ALT="Live Call" WIDTH=109 HEIGHT=30 BORDER=0>&nbsp;<IMG id="pannel_img" onClick="javascript:show_pannel_div();" SRC="./images/pannel_control_img.jpg" ALT="hidden" border="0" align="middle"></TD>
<TD>
    <div class="queuediv"><font class="body_text"> 
     <span id=AgentStatusCalls></span>
    </font>&nbsp;&nbsp;&nbsp;&nbsp;
    <font class="body_text">Status:</font></div>
<!--Add by fnatic -->
   <div class="MainStatuSDiv"><span id="MainStatuSSpan"></span></div>
  
</TD></TR>
<tr><td colspan=3><span id="busycallsdebug"></span></td></tr>
<tr><td id="button_pannel_td" align=center valign=top nowrap="nowrap" style="width:175px;">
<div id="button_pannel">
<font class="body_text">
<span STYLE="background-color: #FFFFFF" id="HangupControl"><IMG SRC="./images/vdc_LB_hangupcustomer_OFF.gif" border=0 alt="Hangup Customer"></span>
<span id="DiaLControl"><a href="#" onClick="ManualDialNext('','','','','','0');"><IMG SRC="./images/vdc_LB_dialnextnumber_OFF.gif" border=0 alt="Dial Next Number"></a></span><BR>
<span STYLE="background-color: #FFFFFF" id="ParkControl"><IMG SRC="./images/vdc_LB_parkcall_OFF.gif" border=0 alt="Park Call"></span>
<span STYLE="background-color: #FFFFFF" id="XferControl"><IMG SRC="./images/vdc_LB_transferconf_OFF.gif" border=0 alt="Transfer - Conference"></span><BR>
...
<span id="MDstatusSpan"><a href="#" onClick="NeWManuaLDiaLCalL('NO');return false;">MANUAL DIAL</a></span> <a href="#" onClick="NeWManuaLDiaLCalL('FAST');return false;">FAST DIAL</a>
</font></span>
<span id="PauseCodeButtons"><font class="body_text">
<span id="PauseCodeLinkSpan"></span>
</font></span>
<span id="CallbacksButtons"><font class="body_text">
<span id="CBstatusSpan">X ACTIVE CALLBACKS</span> 
</font></span>
...
<div id="SendDTMFdiv"><span STYLE="background-color:<?php echo $MAIN_COLOR ?>" id="SendDTMF"><a href="#" onClick="SendConfDTMF(session_id);return false;"><IMG SRC="./images/vdc_LB_senddtmf.gif" border=0 alt="Send DTMF" align="absmiddle"></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value="" maxlength=50></div></span><BR>
</font>
<span id="AgentMuteSpan"></span>&nbsp;
<span id="VolumeControlSpan"><span id="VolumeUpSpan"><IMG SRC="./images/vdc_volume_up_off.gif" BORDER=0></span>&nbsp;<span id="VolumeDownSpan"><IMG SRC="./images/vdc_volume_down_off.gif" BORDER=0></span>
</font></span>
</div>
<hr>
<font face="Arial,Helvetica" size=1>
 <!-- version: <?php //echo $version ?> 
  ...
  <?php	echo "<b>Logged in as User:</b> $VD_login&nbsp;<b>Phone Login:</b> $SIP_user <br/> <b>to campaign:</b> $VD_campaign&nbsp; \n"; ?> 
  <br/>
  <b>Session ID:</b>
...
{   
	 ...
	 pannel_img.alt=(pannel_img.alt=="hidden")?'show':'hidden';
}     
</script>
<!--End-->
...
<table border="0" ><tr>
<td align=center><A HREF="#" onClick="MainPanelToFront('NO');"><IMG SRC="./images/customer_info.gif" ALT="VICIDIAL" WIDTH=143 HEIGHT=27 BORDER=0></A>&nbsp;&nbsp;&nbsp;&nbsp;<A HREF="#" onClick="ScriptPanelToFront();"><IMG SRC="./images/script_tab.jpg" ALT="SCRIPT" WIDTH=143 HEIGHT=27 BORDER=0></A></td>
</tr><tr>
<td colspan=4 align=center>Customer Information: <span id="CusTInfOSpaN"></span></td>
</tr><tr>
<td align=left colspan=2>
<table><tr>
<td align=right><font class="body_text"> Title: </font></td>
...
<td align=left><font class="body_text"><span id="GENDERhideFORie"><select style="display:none" size=1 name=gender_list class="cust_form" id=gender_list><option value="U">U - Undefined</option><option value="M">M - Male</option><option value="F">F - Female</option></select></span></td>
</tr><tr>
