...
showPages.prototype.getPage = function(){ //丛url获得当Prev数,如果变量重复只获取最后一个
	var args = location.search;
	var reg = new RegExp('[\?&]?' + this.argName + '=([^&]*)[&$]?', 'gi');
	var chk = args.match(reg);
	this.page = RegExp.$1;
}
showPages.prototype.checkPages = function(){ //进行当Prev数和总页数的验证
...
showPages.prototype.createHtml = function(mode){ //生成html代码
	...
	switch (mode) {
		case 0 : //模式1 (页数,First,Prev,Next,Last)
			...
			strHtml += '</span><br />';
			break;
		case 1 : //模式1 (10页缩略,First,Prev,Next,Last)
			strHtml += '<span class="count">Pages: ' + this.page + ' / ' + this.pageCount + '</span>';
			strHtml += '<span class="number">';
			if (prevPage < 1) {
				strHtml += '<span title="First Page">?</span>';
			...
			strHtml += '</span><br />';
			break;
		case 2 : //模式2 (前后缩略,页数,First,Prev,Next,Last)
			strHtml += '<span class="count">Pages: ' + this.page + ' / ' + this.pageCount + '</span>';
			strHtml += '<span class="number">';
			if (prevPage < 1) {
				strHtml += '<span title="First Page">  First  </span>';
				strHtml += '<span title="Prev Page">  Prev  </span>';
			} else {
				strHtml += '<span title="First Page"><a href="javascript:' + this.name + '.toPage(1);">  First  </a></span>';
				strHtml += '<span title="Prev Page"><a href="javascript:' + this.name + '.toPage(' + prevPage + ');">  Prev  </a></span>';
			}
			...
			if (nextPage > this.pageCount) {
				...
				strHtml += '<span title="Last Page">  Last  </span>';
			} else {
				strHtml += '<span title="Next Page"><a href="javascript:' + this.name + '.toPage(' + nextPage + ');">  Next  </a></span>';
				strHtml += '<span title="Last Page"><a href="javascript:' + this.name + '.toPage(' + this.pageCount + ');">  Last  </a></span>';
			}
			strHtml += '</span><br />';
			break;
		case 3 : //模式3 (箭头样式,First,Prev,Next,Last) (only IE)
			strHtml += '<span class="count">Pages: ' + this.page + ' / ' + this.pageCount + '</span>';
