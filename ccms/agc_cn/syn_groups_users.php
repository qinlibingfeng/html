...
if ($ss_conf_ct > 0)
{
	...
	echo "Can't find the campaign information!";
	exit;
}
...
echo "<html>\n";
echo "<head>\n";
echo "<title>CCMS Vtiger user synchronization utility</title>\n";
echo "<META HTTP-EQUIV=\"Content-Type\" CONTENT=\"text/html; charset=utf-8\">\n";

if ($enable_vtiger_integration < 1)
	{
	echo "<B>ERROR! - Vtiger integration is disabled in the CCMS system_settings";
	exit;
	}
...
echo 'groups:'.$VD_groups_ct.'<hr>';
while ($i < $VD_groups_ct)
	{
	$row=mysql_fetch_row($rslt);
	$group_id =		urlencode($row[0]);
	$group_name =	urlencode($row[1]);
	$i++;
	
	
	
	
	$url = $vtiger_url.'?action=SynGroups&server_ip='.$vtiger_server_ip.'&server_dbname='.$vtiger_dbname.'&server_login='.$vtiger_login.'&server_pass='.$vtiger_pass.'&group_id='.$group_id.'&group_name='.$group_name;
...
while ($i < $VD_users_ct)
	{
		...
		$pass =			urlencode($row[1]);
		$full_name =	urlencode($row[2]);   while (strlen($full_name)>30) {$full_name = eregi_replace(".$",'',$full_name);}
		$user_level =	urlencode($row[3]);
		$active =		urlencode($row[4]);
		$user_group=	urlencode($row[5]);
		$i++;
		
		$url = $vtiger_url.'?action=SynUsers&server_ip='.$vtiger_server_ip.'&server_dbname='.$vtiger_dbname.'&server_login='.$vtiger_login.'&server_pass='.$vtiger_pass.'&user_name='.$user.'&user_pass='.$pass.'&user_fullname='.$full_name.'&user_level='.$user_level.'&user_active='.$active.'&user_group='.$user_group;
		$html=file_get_contents($url);
		echo $html;
		echo '<hr>';
	}
