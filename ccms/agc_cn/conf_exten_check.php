...
# 50706-1525 - Added date-time display for CCMS client display
...
# agent config check in ring mode + fnatic
...
echo "<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
	...
	if ($ACTION == 'refresh')
	{
		...
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten 是无效的\n";
		exit;
		}
		else
		{
		...
		#	$AexternalDEAD=$row[0];
			...
			//if (strstr($user,"fnatic")!==false)
			//		{
			//	writelog($AcalleridCOUNT . "--" . $Astatus . "-----" . $Aagent_log_id . "--------------" . $user);
			//		}
				###writelog($AcalleridCOUNT . "--" . $Astatus . "-----" . $Aagent_log_id . "--------------" . $user);
				if ( ($AcalleridCOUNT < 1) and (eregi("INCALL",$Astatus)) and (strlen($Aagent_log_id) > 0) )
					{
					###writelog("GGGGGGGGGGGGGGG");
					$DEADcustomer++;
					//writelog($DEADcustomer . "++++++++++++++++++++++++++++");
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03006',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					# ring模式的状态需要增加一种XFER
					$call_status = ($inbound_mode == 'ring') ? "'XFER','LIVE'" : "'LIVE'";
					//$stmt="SELECT * from vicidial_auto_calls where status IN($call_status) and ( (campaign_id='$Acampaign') or (campaign_id IN('$AccampSQL')) $ADsql);";
					$stmt="SELECT * from vicidial_auto_calls where status IN($call_status) and ( campaign_id IN('$AccampSQL') $ADsql);";
					echo $stmt;
					if ($DB) {echo "|$stmt|\n";}
					$rslt=mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03007',$user,$server_ip,$session_name,$one_mysql_log);}
					$RingCalls=mysql_num_rows($rslt);


					$row_lgq=mysql_fetch_row($rslt);
					$lgq_phone_number=$row_lgq[9];



					$tt_img="<img src='../agc/images/QueueWarning.gif' border=0 />";
					$inbound_call_count = 0;
					
					if ($inbound_mode=='ring' && $Astatus=="RING")
						{	
							
							$stmt = "SELECT uniqueid FROM ccms_queue_log where user='$user' order by ring_start desc limit 1;";
							$rslt=mysql_query($stmt, $link);
							$row=mysql_fetch_row($rslt);
							$RingCallUniqueid=$row[0];
							$RingCalls = "<img src='../agc/images/ppc.gif' border=0 /><font size=2>分机振铃</font>---$RingCallUniqueid";
						}
					else
						{
						$row=mysql_fetch_row($rslt);
						#$RingCalls_Channel=$row[7];
						#$RingCalls_Channel_Array=explode("*",$RingCalls_Channel);
						#if ($RingCalls > 0) {$RingCalls = "<font class=\"queue_text_red\">等待电话数:<font color=red size=2>$RingCalls</font></font>";}
						#else {$RingCalls = "<font class=\"queue_text\">等待电话数:<font color=red size=2>$RingCalls</font></font>";}

						if ($RingCalls > 0)
							{
							if(substr($row[7],0,4) == 'IAX2')
								{
								#$RingCalls_Channel="<由".$RingCalls_Channel_Array[6]."转过来的电话>";
								$tt_img="<IMG SRC=\"../agc/images/QueueWarning_local.gif\" BORDER=\"0\"/>";
								}
							else
								{
								$RingCalls_Channel="";				 
						...
						if($RingCalls > 0){$RingCalls = $tt_img."<font size=2>".$RingCalls."个转接电话在等待" . $row[7]. "</font>";}
						else{$RingCalls = "";}
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03029',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if ( ($AcalleridCOUNT < 1) and (eregi("INCALL",$Astatus)) and (strlen($Aagent_log_id) > 0) )
					{
					###writelog("aaaaaaaaaaaaaaaGGGGGGGGGGGGGGG");
					$DEADcustomer++;
					### find whether the agent log record has already logged DEAD
					$stmt="SELECT count(*) from vicidial_agent_log where agent_log_id='$Aagent_log_id' and ( (dead_epoch IS NOT NULL) or (dead_epoch > 10000) );";
					if ($DB) {echo "|$stmt|\n";}
			...
			if($inbound_mode=="ring")
				{
				$stmt="SELECT external_hangup,external_status,external_pause,external_dial,external_update_fields,external_update_fields_data,external_timer_action,external_timer_action_message,external_timer_action_seconds,comments from vicidial_live_agents where user='$user' and server_ip='$server_ip';";				
				}
			else
				{
			...
			$stmt="select limit_used_leads_alert,list_lead_total,unused_leads,dial_method from vicidial_campaigns where campaign_id='$Acampaign_id';";
			$rslt=mysql_query($stmt, $link);
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'03016',$VD_login,$server_ip,$session_name,$one_mysql_log);}
			$row=mysql_fetch_row($rslt);
			$limit_used_leads_alert =		$row[0];
			$list_lead_total =		$row[1];
			$unused_leads_total = $row[2]; //可用leads数;
			$dial_method = $row[3];
			
			# agent config check in ring mode + fnatic
			if($inbound_mode=='ring')
				{
				echo 'DateTime: ' . $NOW_TIME . '|UnixTime: ' . $StarTtime . '|Logged-in: ' . $Alogin . '|CampCalls: ' . $RingCalls . '|Status: ' . $Astatus . '|DiaLCalls: ' . $DiaLCalls . '|APIHanguP: ' . $external_hangup . '|APIStatuS: ' . $external_status . '|APIPausE: ' . $external_pause . '|APIDiaL: ' . $external_dial . '|DEADcall: ' . $DEADcustomer . '|InGroupChange: ' . $InGroupChangeDetails . '|APIFields: ' . $external_update_fields . '|APIFieldsData: ' . $external_update_fields_data . '|APITimerAction: ' . $timer_action . '|APITimerMessage: ' . $timer_action_message . '|APITimerSeconds: ' . $timer_action_seconds . '|Comments: '. $comments . '|Limitleads: '. $limit_used_leads_alert . '|List_lead_total: '. $list_lead_total . '|Unused_leads_total: '. $unused_leads_total . '|Dial_method: '. $dial_method .'|phone_number: '.$lgq_phone_number."\n";				
				}
			else
				{
				echo 'DateTime: ' . $NOW_TIME . '|UnixTime: ' . $StarTtime . '|Logged-in: ' . $Alogin . '|CampCalls: ' . $RingCalls . '|Status: ' . $Astatus . '|DiaLCalls: ' . $DiaLCalls . '|APIHanguP: ' . $external_hangup . '|APIStatuS: ' . $external_status . '|APIPausE: ' . $external_pause . '|APIDiaL: ' . $external_dial . '|DEADcall: ' . $DEADcustomer . '|InGroupChange: ' . $InGroupChangeDetails . '|APIFields: ' . $external_update_fields . '|APIFieldsData: ' . $external_update_fields_data . '|APITimerAction: ' . $timer_action . '|APITimerMessage: ' . $timer_action_message . '|APITimerSeconds: ' . $timer_action_seconds . '|Comments: '. '' . '|Limitleads: '. $limit_used_leads_alert . '|List_lead_total: '. $list_lead_total . '|Unused_leads_total: '. $unused_leads_total . '|Dial_method: '. $dial_method .'|phone_number: '.$lgq_phone_number."\n";	
				}
			# end
    ...
    # gab customer park channel start + fnatic
	if(isset($Parked_Channel_Value) && $Parked_Channel_Value!="")
		{$parked_channel_count=0;
	     $stmt="SELECT * FROM live_channels where server_ip = '$server_ip' and channel = '$Parked_Channel_Value';";
		 $rslt=mysql_query($stmt,$link);
		 $parked_channel_count=mysql_num_rows($rslt);
		 if($parked_channel_count>0) { echo "PARKED CHANNEL FOUND"; }
		 else { echo "PARKED CHANNEL NOT FOUND"; }
	    }
	}
    # end
	if ($ACTION == 'register')
	{
		...
		{
		$channel_live=0;
		echo "Conf Exten $conf_exten 是无效的 or Exten $exten 是无效的\n";
		exit;
		}
		else
		{
