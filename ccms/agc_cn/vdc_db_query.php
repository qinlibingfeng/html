...
# 50701-1451 - Added dial log for start and end of CCMS calls
...
# 50818-1305 - Added stop-all-recordings-after-each-CCMS-call option
...
# 51129-1012 - Added ability to accept calls from other CCMS servers
...
# 60821-1600 - Added ability to omit the phone code on CCMS lead dialing
...
# 90611-1423 - Fixed agent log and CCMS log bugs
...
if (isset($_GET["xfre_action"]))				{$xfre_action=$_GET["xfre_action"];}
	elseif (isset($_POST["xfre_action"]))		{$xfre_action=$_POST["xfre_action"];}
if (isset($_GET["transfer_Xfre_Value"]))				{$transfer_Xfre_Value=$_GET["transfer_Xfre_Value"];}
	elseif (isset($_POST["transfer_Xfre_Value"]))	{$transfer_Xfre_Value=$_POST["transfer_Xfre_Value"];}
if (isset($_GET["ISAPI"]))				{$ISAPI=$_GET["ISAPI"];}
	elseif (isset($_POST["ISAPI"]))	{$ISAPI=$_POST["ISAPI"];}
if (isset($_GET["status_type"]))				{$status_type=$_GET["status_type"];}
	elseif (isset($_POST["status_type"]))	{$status_type=$_POST["status_type"];}

if (isset($_GET["agent_dial_start_epoch"]))				{$agent_dial_start_epoch=$_GET["agent_dial_start_epoch"];}
	elseif (isset($_POST["agent_dial_start_epoch"]))	{$agent_dial_start_epoch=$_POST["agent_dial_start_epoch"];}		
if (isset($_GET["redirectcalltoagentid"]))				{$redirectcalltoagentid=$_GET["redirectcalltoagentid"];}
        elseif (isset($_POST["redirectcalltoagentid"]))  {$redirectcalltoagentid=$_POST["redirectcalltoagentid"];}
// 修改vicidial_log.length_in_sec为talktime秒数 + fnatic
if (isset($_GET["out_length_in_sec"]))			{$out_length_in_sec = $_GET["out_length_in_sec"];}
	elseif (isset($_POST["out_length_in_sec"]))	{$out_length_in_sec = $_POST["out_length_in_sec"];}
if (isset($_GET["session_RemotePhone"]))				{$session_RemotePhone=$_GET["session_RemotePhone"];}
	elseif (isset($_POST["session_RemotePhone"]))		{$session_RemotePhone=$_POST["session_RemotePhone"];}
if (isset($_GET["agent_log_id_line2"]))				{$agent_log_id_line2=$_GET["agent_log_id_line2"];}
	elseif (isset($_POST["agent_log_id_line2"]))		{$agent_log_id_line2=$_POST["agent_log_id_line2"];}
require($langu_php);
// 结束
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if( (strlen($user)<2) or (strlen($pass)<2) or ($auth==0))
		...
		if( (strlen($server_ip)<6) or (!isset($server_ip)) or ( (strlen($session_name)<12) or (!isset($session_name)) ) )
			{
			echo "无效 server_ip: |$server_ip|  or  无效 session_name: |$session_name|\n";
			exit;
			}
		else
			{
			...
			$SNauth=$row[0];
			  if($SNauth==0)
				{
				echo "无效 session_name: |$session_name|$server_ip|\n";
				exit;
				}
			  else
				{
...
if ($format=='debug')
	...
	echo "<title>CCMS 数据查询语法";
	echo "</title>\n";
	echo "</head>\n";

	//echo "<script>alert('vdc_db_queue');</script>";
	echo "<BODY BGCOLOR=white marginheight=0 marginwidth=0 leftmargin=0 topmargin=0>\n";
	}

//yhb_log(YHB_LOG,$ACTION,$user,"query");
...
if ($ACTION == 'LogiNCamPaigns')
	...
	if ( (strlen($user)<1) )
		...
		{
		$stmt="SELECT user_group,user_level,agent_shift_enforcement_override,shift_override_flag,disabled_campaigns from vicidial_users where user='$user' and pass='$pass'";
		if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
		$rslt=mysql_query($stmt, $link);
	...
	if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
		...
		if ($VDloginDISPLAY > 0)
			...
			$VDdisplayMESSAGE.= "<INPUT TYPE=Submit NAME=确认 VALUE=确认></FORM><BR><BR><BR><BR>\n";
	...
	if ($show_campaign_list > 0)
		...
		while ($camps_to_print > $o) 
			{
				$rowx=mysql_fetch_row($rslt);
				if ( (!eregi($rowx[0],$disabled_campaigns_user)) ){
		...
		{
		echo "<select size=1 name=VD_campaign id=VD_campaign onFocus=\"login_allowable_campaigns()\">\n";
		echo "<option value=\"\">-- 您必须先登入TIMECLOCK --</option>\n";
		echo "</select>\n";
		}
	exit;
	}
...
if ($ACTION == 'regCLOSER')
	...
	echo "Closer In 群组选项 $closer_choice 被注册至使用耿$user\n";
	}
...
if ($ACTION == 'regTERRITORY')
	...
	if ( (strlen($agent_territories)<1) || (strlen($user)<1) )
		{
		$channel_live=0;
		echo "Territory Choice $agent_territories 是无效的\n";
		exit;
		}
	else
		{
	...
	echo "Territory Choice $agent_territories 被注册至使用耿$user\n";
	}
...
################################################################################
### manDiaLnextCaLL - for manual CCMS dialing this will grab the next lead
...
if ($ACTION == 'manDiaLnextCaLL')
	...
	if ( (strlen($conf_exten)<1) || (strlen($campaign)<1)  || (strlen($ext_context)<1) )
		...
		$channel_live=0;
		echo "HOPPER EMPTY\n";
		echo "Conf Exten $conf_exten or campaign $campaign or ext_context $ext_context 是无效的\n";
		exit;
		}
	else
	{
		...
		if (strlen($phone_number)>=3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST 拨号 TIME",$lead_order)){$order_stmt = 'order by last_local_call_time desc, lead_id asc';}
					if (eregi("DOWN LAST 拨号 TIME",$lead_order)){$order_stmt = 'order by last_local_call_time, lead_id asc';}
	...
	if ($affected_rows > 0)
		...
		if (!$CBleadIDset)
			...
			if ($list_lead_ct > 0)
				...
				$row=mysql_fetch_row($rslt);
			#	$lead_id		= trim("$row[0]");
				$entry_date_vicidial = trim("$row[1]");
				$modify_date_vicidial = trim("$row[2]");
				$dispo			= trim("$row[3]");
				$tsr			= trim("$row[4]");
			...
			if ( (strlen($preview)<1) || ($preview == 'NO') )
				{
				### prepare variables to place manual call from CCMS
				$CCID_on=0;   $CCID='';
				$local_DEF = 'Local/';
				$local_AMP = '@';
				$Local_out_prefix = '9';
			...
			#	$Local_persist = '/n';
				...
				if(strpos($phone_number,"*in")){
					$new_number = (strpos($phone_number,"*in")-8);
					$new_number = substr($phone_number,8,$new_number);
					if (eregi('Y',$omit_phone_code)) 
						{$Ndialstring = "$phone_number";}
					else
						{$Ndialstring = "$phone_code$phone_number";}
					$stmt = "SELECT user from vicidial_live_agents where extension= 'R/$new_number';";
					$rslt=mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00245',$user,$server_ip,$session_name,$one_mysql_log);}
					if ($DB) {echo "$stmt\n";}
					$lio_ct = mysql_num_rows($rslt);
				...
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00033',$new_number,$user,$server_ip,$session_name,$one_mysql_log);}
		...
		#			}
			...
			$stmt = "SELECT pause_sec from vicidial_agent_log where agent_log_id='$agent_log_id';";
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00XXX',$user,$server_ip,$session_name,$one_mysql_log);}
			if ($DB) {echo "$stmt\n";}
			$vald_ct = mysql_num_rows($rslt);
			...
			$row=mysql_fetch_row($rslt);
			$uppause_sec =  $row[0];
			$from_pause_sec = ",pause_sec='$val_pause_sec'";
			if ($uppause_sec > 0)
			{
			$from_pause_sec = "";
			}
				...
				$stmt="UPDATE vicidial_agent_log set wait_sec='$val_pause_sec'$from_pause_sec where agent_log_id='$agent_log_id';";
					if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00XXX',$user,$server_ip,$session_name,$one_mysql_log);}
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$linkB,$mel,$stmt,'00038',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					# 拨号OUTBOUND (formerly ENTERQUEUE)
			...
			$LeaD_InfO .=	$entry_date_vicidial . "\n";
			$LeaD_InfO .=	$modify_date_vicidial . "\n";
			$LeaD_InfO .=	$StarTtime . "\n";
			echo $LeaD_InfO;
			}
		else
...
if ($ACTION == 'alt_phone_change')
{
	...
	if ( (strlen($stage)<1) || (strlen($called_count)<1) || (strlen($lead_id)<1)  || (strlen($phone_number)<1) )
		{
		$channel_live=0;
		echo "其它电话 号码 状怿NOT CHANGED\n";
		echo "$phone_number $stage $lead_id or $called_count 是无效的\n";
		exit;
		}
	else
		{
...
################################################################################
### manDiaLskip - for manual CCMS dialing this skips the lead that was
...
################################################################################
### manDiaLonly - for manual CCMS dialing this sends the call that was
...
################################################################################
### manDiaLlookCaLL - for manual CCMS dialing this will attempt to look up
...
if ($ACTION == 'manDiaLlookCaLL')
	...
	if (strlen($MDnextCID)<18)
		...
		if ($call_good > 0)
			...
			$stmt="UPDATE vicidial_auto_calls set uniqueid='$uniqueid',channel='$channel',status='CLOSER' where callerid='$MDnextCID';";
				if ($format=='debug') {echo "\n<!-- $stmt -->";}
			$rslt=mysql_query($stmt, $link);
...
if ($ACTION == 'manDiaLlookCaLL2')
	...
	if (strlen($MDnextCID)<18)
		...
		{
		##### look for the channel in the UPDATED vicidial_manager record of the call initiation
		$stmt="SELECT uniqueid,channel FROM call_log where caller_code='$MDnextCID' and server_ip='$server_ip'  ORDER BY `uniqueid` DESC LIMIT 1;";
		###writelog("------$MDnextCID------");
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'000521',$user,$server_ip,$session_name,$one_mysql_log);}
		if ($DB) {echo "$stmt\n";}
		$VM_mancall_ct = mysql_num_rows($rslt);
		if ($VM_mancall_ct > 0)
			{
			...
			$uniqueid =$row[0];
			$conf_channel =$row[1];
				$stmt="SELECT channel_outline from live_sip_channels where channel='$conf_channel';";
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'000523',$user,$server_ip,$session_name,$one_mysql_log);}
				$rslt=mysql_query($stmt, $link);
				$row=mysql_fetch_row($rslt);
				$channel=$row[0];
				if ($channel)
				{
				$stmt="UPDATE vicidial_manager SET uniqueid='$uniqueid',channel='$channel',status='UPDATED' where callerid='$MDnextCID' and server_ip='$server_ip' and status NOT IN('UPDATED','DEAD') ORDER BY `man_id` DESC LIMIT 1;";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $link);
				###writelog("$stmt");
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'000522',$user,$server_ip,$session_name,$one_mysql_log);}
				$affected_rows = mysql_affected_rows($link);
				if ($affected_rows > 0)
					{
					$call_output = "$uniqueid\n$channel\n";
...
################################################################################
### manDiaLlogCALL - for manual CCMS logging of calls places record in
...
if ($stage == "start")
	...
	if ( (strlen($uniqueid)<1) || (strlen($lead_id)<1) || (strlen($list_id)<1) || (strlen($phone_number)<1) || (strlen($campaign)<1) )
		...
		echo "LOG未输入\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id or list_id: $list_id or phone_number: $phone_number or campaign: $campaign 是无效的\n";
		exit;
		}
	else
		{
		...
		if ($manualVLexists < 1)
			{
			##### insert log into vicidial_log for manual CCMS call
			$stmt="INSERT INTO vicidial_log (uniqueid,lead_id,list_id,campaign_id,call_date,start_epoch,status,phone_code,phone_number,user,comments,processed,user_group,alt_dial,status_type) values('$uniqueid','$lead_id','$list_id','$campaign','$NOW_TIME','$StarTtime','INCALL','$phone_code','$phone_number','$user','MANUAL','N','$user_group','$alt_dial','$status_type');";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
		...
		if ($affected_rows > 0)
			{
			echo "CCMS_LOG 已插兿 $uniqueid|$channel|$NOW_TIME\n";
			echo "$StarTtime\n";
			}
		else
			{
		...
		$stmt = "UPDATE vicidial_auto_calls SET uniqueid='$uniqueid' where caller_code='$MDnextCID';"; 
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00058',$user,$server_ip,$session_name,$one_mysql_log);}

	#	##### insert log into call_log for manual CCMS call
	...
	#	if ($affected_rows > 0)
	#		{
	#		echo "CALL_LOG 已插兿 $uniqueid|$channel|$NOW_TIME";
	#		}
	#	else
	#		{
	#		echo "LOG未输入\n";
...
if ($stage == "end")
	...
	$VLA_inOUT='NONE';
	$stmt="SELECT comments,extension FROM vicidial_live_agents where user='$user' order by last_update_time desc limit 1;";
	$rslt=mysql_query($stmt, $link);
	...
	if($VLA_inOUT == 'INBOUND' && $transfer_Xfre_Value == 1){$VLA_inOUT = 'AUTO';}


  #### add by heibo 2011-4-15 14:29:59 bug716
  if ( $inOUT == "OUT"  && (strlen($uniqueid)<1) ) {
			$stmt = "UPDATE vicidial_live_agents set status='PAUSED',last_state_change='$NOW_TIME' where user='$user' and server_ip='$server_ip';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'10415',$user,$server_ip,$session_name,$one_mysql_log);}
  }
 ###  end heibo bug716
	...
	if ( (strlen($uniqueid)<1) or (strlen($lead_id)<1) )
		{
		echo "LOG未输入\n";
		echo "uniqueid $uniqueid or lead_id: $lead_id 是无效的\n";
		exit;
		}
	else
		{
		$term_reason='NONE';
		/*add by heibo bug 705 2011-6-22 17:14:08 */

		if (  !empty($sipphone) && substr($sipphone,0,3) == "SIP" ){
    ...
    //end
		...
		if ($VLA_inOUT == 'INBOUND')
			{
			$stmt = "UPDATE vicidial_closer_log set end_epoch='$StarTtime', length_in_sec='$length_in_sec', status='$status_dispo' where comments!='line2' and lead_id='$lead_id' and user='$user' and call_date > \"$four_hours_ago\" order by call_date desc limit 1;";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
	...
	#		fclose($fp);
		...
		if ( ($VLA_inOUT == 'AUTO') or ($VLA_inOUT == 'MANUAL') )
			...
			// 修改vicidial_log.length_in_sec为talktime秒数 + fnatic
                $stmt="SELECT start_epoch from vicidial_log where uniqueid='$uniqueid' and lead_id='$lead_id' and user='$user' order by call_date desc limit 1;";
				$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00090',$user,$server_ip,$session_name,$one_mysql_log);}
				...
				if ($VAC_qm_ct > 0)
					...
					if ($startepoch > 0)
					{
					$out_length_in_sec = ($StarTtime - $startepoch);
					}
					}
			if($SQLterm != ""){
				$SQLterm = $SQLterm . "length_in_sec='$out_length_in_sec', end_epoch='$StarTtime',";
			}
			...
			// $stmt="UPDATE vicidial_log set $SQLterm end_epoch='$StarTtime', length_in_sec='$out_length_in_sec', status='$status_dispo' where uniqueid='$uniqueid' and lead_id='$lead_id' and user='$user' order by call_date desc limit 1;";
			$stmt="UPDATE vicidial_log set $SQLterm  status='$status_dispo',status_type='$status_type' where uniqueid='$uniqueid' and lead_id='$lead_id' and user='$user' order by call_date desc limit 1;";
			// 结束
			...
			if (strlen($SQLterm) > 0)
				{
				##### update the duration and end time in the vicidial_log table
				$stmt="UPDATE vicidial_closer_log set $SQLterm, status='$status_dispo' where  comments!='line2' and lead_id='$lead_id' and user='$user' and call_date > \"$four_hours_ago\" order by call_date desc limit 1;";
	...
	##### if CCMS call and hangup_after_each_call activated, find all recording 
		...
		if ( ( ($conf_dialed > 0) or ($hangup_all_non_reserved > 0) ) and ($leaving_threeway < 1) and ($blind_transfer < 1) )
			...
			while($loop_count < $total_hangup)
				...
				if (strlen($hangup_channels[$loop_count])>5)
					{
						if(preg_match("/Local\/4/i",$hangup_channels[$loop_count]) || preg_match("/Local\/111/i",$hangup_channels[$loop_count])){
							//writelog("========$hangup_channels[$loop_count]============");
						}else{
							...
							//$stmt="INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Redirect','CH12346$StarTtime$loop_count','Channel: $hangup_channels[$loop_count]','Context:default','Exten: 8304','Priority: 1','','','','','','');";
								if ($format=='debug') {echo "\n<!-- $stmt -->";}
							$rslt=mysql_query($stmt, $link);
		...
		while($loop_count < $total_rec)
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00099',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					if ($fn_count)
						...
						if(($vidSQL == '' && ($transfer_Xfre_Value == 2 || $transfer_Xfre_Value == 4))) $vidSQL = ",vicidial_id='$uniqueid' ";
						elseif($transfer_Xfre_Value == 1 || $transfer_Xfre_Value == 3) $vidSQL = ",vicidial_id='$uniqueid' ";
...
################################################################################
### VDADREcheckINCOMING - for auto-dial CCMS dialing this will recheck for
...
if ($ACTION == 'VDADREcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) || (strlen($lead_id)<1) )
		...
		$channel_live=0;
		echo "0\n";
		echo "Campaign $campaign 是无效的\n";
		echo "lead_id $lead_id 是无效的\n";
		exit;
		}
	else
		{
...
################################################################################
### VDADcheckINCOMING - for auto-dial CCMS dialing this will check for calls
...
if ($ACTION == 'VDADcheckINCOMING')
	...
	if ( (strlen($campaign)<1) || (strlen($server_ip)<1) )
		...
		{
		### grab the call and lead info from the vicidial_live_agents table
		$stmt = "SELECT lead_id,uniqueid,callerid,channel,call_server_ip,comments,extension FROM vicidial_live_agents where server_ip = '$server_ip' and user='$user' and campaign_id='$campaign' and status='QUEUE';";
		...
		if ($queue_leadID_ct > 0)
			...
			### update the agent status to INCALL in vicidial_live_agents
			$stmt = "UPDATE vicidial_live_agents set status='INCALL',last_call_time='$NOW_TIME',calls_today='$calls_today',external_hangup=0,external_status='',external_pause='',external_dial='',last_state_change='$NOW_TIME',last_inbound_time='$NOW_TIME' where user='$user' and server_ip='$server_ip';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
			...
			if ( ($call_type=='OUT') or ($call_type=='OUTBALANCE') )
				...
				if (strlen($list_id)>0)
					...
					$stmt = "select type,number_dialed,extension from call_log where uniqueid='$uniqueid';";
					if ($DB) {echo "$stmt\n";}
					$rslt=mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'002821',$user,$server_ip,$session_name,$one_mysql_log);}
					$VDIG_did_ct = mysql_num_rows($rslt);
					if ($VDIG_did_ct > 0)
						{
						$row=mysql_fetch_row($rslt);
						if (strlen($row[0]) > 0)
							{$VDCL_did_type =	$row[0];}
						if (strlen($row[1]) > 0)
							{$VDCL_did_number_dialed =	$row[1];}
						if (strlen($row[2]) > 0)
							{$VDCL_did_extension =	$row[2];}
						}
						if ($VDCL_did_type == "IAX2")
						{
						$stmt = "select type,number_dialed,extension from call_log where caller_code='$VDCL_did_extension' order by uniqueid desc limit 1;";
						if ($DB) {echo "$stmt\n";}
						$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'002822',$user,$server_ip,$session_name,$one_mysql_log);}
						$VDIG_did_ct = mysql_num_rows($rslt);
						if ($VDIG_did_ct > 0)
							{
							$row=mysql_fetch_row($rslt);
							...
							$stmt = "select called_400 from vicidial_called_map where called='$VDCL_did_number_dialed' and active = 'Y' and isdel = 'N' limit 1;";
							if ($DB) {echo "$stmt\n";}
							$rslt=mysql_query($stmt, $link);
							if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'002823',$user,$server_ip,$session_name,$one_mysql_log);}
							$VDIG_did_ct = mysql_num_rows($rslt);
							if ($VDIG_did_ct > 0)
								{
								$row=mysql_fetch_row($rslt);
								if (strlen($row[0]) > 0)
									{$VDCL_did_number_dialed =	$row[0];}
								}


				### if web form is set then send on to vicidial.php for override of WEB_FORM address
				if ( (strlen($VDCL_group_web)>5) or (strlen($VDCL_group_name)>0) ) {echo "$VDCL_group_web|$VDCL_group_name|$VDCL_group_color|$VDCL_fronter_display|$VDADchannel_group|$VDCL_ingroup_script|$VDCL_get_call_launch|$VDCL_xferconf_a_dtmf|$VDCL_xferconf_a_number|$VDCL_xferconf_b_dtmf|$VDCL_xferconf_b_number|$VDCL_default_xfer_group|$VDCL_ingroup_recording_override|$VDCL_ingroup_rec_filename|$VDCL_default_group_alias|$VDCL_caller_id_number|$VDCL_group_web_vars|$VDCL_group_web_two|$VDCL_timer_action|$VDCL_timer_action_message|$VDCL_timer_action_seconds|$VDCL_xferconf_c_number|$VDCL_xferconf_d_number|$VDCL_xferconf_e_number|$VDCL_did_number_dialed|\n";}
				else {echo "X|$VDCL_group_name|$VDCL_group_color|$VDCL_fronter_display|$VDADchannel_group|$VDCL_ingroup_scriptDCL_get_call_launch|$VDCL_xferconf_a_dtmf|$VDCL_xferconf_a_number|$VDCL_xferconf_b_dtmf|$VDCL_xferconf_b_number|$VDCL_default_xfer_group|$VDCL_ingroup_recording_override|$VDCL_ingroup_rec_filename|$VDCL_default_group_alias|$VDCL_caller_id_number|$VDCL_group_web_vars|$VDCL_group_web_two|$VDCL_timer_action|$VDCL_timer_action_message|$VDCL_timer_action_seconds|$VDCL_xferconf_c_number|$VDCL_xferconf_d_number|$VDCL_xferconf_e_number|$VDCL_did_number_dialed|\n";}
				
				//add by bear begin
				$estra_info = getExtensionInfo($campaign,$uniqueid);
				...
				if ($VDU_cid_ct > 0)
					...
					echo $fronter_full_name . '|' . $tsr . $estra_info . "\n";
					}
				else {echo '|' . $tsr . $estra_info . "\n";}
				}
			...
			### If 拨号BK, change vicidial_callback record to INACTIVE
			...
			if (strlen($VDCL_start_call_url) > 7)
				...
				$VDCL_start_call_url = eregi_replace('--A--extension_temp--B--',urlencode(trim($extension_temp)),$VDCL_start_call_url);
				$VDCL_start_call_url = eregi_replace('--A--extension_temp2--B--',urlencode(preg_replace('/^SIP\//','',$extension_temp)),$VDCL_start_call_url);
			...
			{
			echo "0\n";
			$stmt = "SELECT acw_hold_time from vicidial_campaigns where campaign_id='$campaign'";
			$rslt=mysql_query($stmt, $link);
			$row = mysql_fetch_row($rslt);
			$acw_hold_time = $row[0];
			echo "acw_hold_time:$acw_hold_time\n";
...
################################################################################
### userLOGout - Logs the user out of CCMS client, deleting db records and 
...
if ($ACTION == 'userLOGout')
	...
	if ( (strlen($campaign)<1) || (strlen($conf_exten)<1) )
		{
		echo "NO\n";
		echo "campaign $campaign or conf_exten $conf_exten 是无效的\n";
		exit;
		}
	else
		{
		...
		//start  add by pie 20130311 for Agent use 'remote agent's exten' logout CCMS;
		if($session_RemotePhone !="")
		{
			$stmt="UPDATE vicidial_remote_agents set status = 'ACTIVE' WHERE conf_exten = '$session_RemotePhone';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01044',$session_RemotePhone,$server_ip,$session_name,$one_mysql_log);}
			$affected_rows = mysql_affected_rows($link);
		}
		//end  add;
...
if ($ACTION == 'updateDISPO')
	...
	if ( (strlen($dispo_choice)<1) || (strlen($lead_id)<1) )
		{
		echo "Dispo Choice $dispo or lead_id $lead_id 是无效的\n";
		exit;
		}
	else
		{
		$dispo_choice_agent =	$dispo_choice;
		$auto_submit_dispo = "N";
		$stmt = "SELECT auto_submit_dispo,auto_dispo_time from ccms_campaigns where campaign_id = '$campaign'";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'002841',$user,$server_ip,$session_name,$one_mysql_log);}
		$VC_dcu_ct = mysql_num_rows($rslt);
		if ($VC_dcu_ct > 0)
			{
			$row=mysql_fetch_row($rslt);
			$auto_submit_dispo = $row[0];
			}
	...
	if ($ISAPI > 0)
			{
			$stmt = "select start_epoch,end_epoch,length_in_sec from call_log where uniqueid = '$uniqueid';";
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) 
			{
			mysql_error_logging($NOW_TIME,$link,$mel,$stmtsf,'09998',$user,$server_ip,$session_name,$one_mysql_log);
			$row=mysql_fetch_row($rslt);
			$SF_start_epoch = $row[0];
			$SF_end_epoch = $row[1];
			$SFlength_in_sec = $row[2];
			}
			$isConnected = 1;
	if (($dispo_choice == 'A') ||  ($dispo_choice == 'AFAX') ||  ($dispo_choice == 'B') ||  ($dispo_choice == 'N') ||  ($dispo_choice == 'U') ||  ($dispo_choice == '999_NA'))
			{
			$isConnected = 0;
			$SF_dispo_choice = $dispo_choice;
			}
			$stmtsf = "UPDATE sf_callback SET isUpdate='1',isConnected='$isConnected',unConnectedReason='$SF_dispo_choice',uniqueid='$uniqueid',callStartTime='$SF_start_epoch',callEndTime='$SF_end_epoch',duration='$SFlength_in_sec' where user='$user' and dst='$phone_number' order by sfepoch desc limit 1;";
			if ($DB) {echo "$stmt\n";}
			$rsltsf=mysql_query($stmtsf, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmtsf,'09999',$user,$server_ip,$session_name,$one_mysql_log);}
			}
		...
		if ($row[0] > 0)
			...
			if(trim($transfer_Xfre_Value) == 1 || trim($transfer_Xfre_Value) == 3){
				...
				$transfer_Xfre_Value == 0;
			}
			else $stmt = "UPDATE vicidial_closer_log set status='$dispo_choice' where comments!='line2' and lead_id='$lead_id' and user='$user' order by closecallid desc limit 1;";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
			...
			if ( ($auto_dial_level < 1) or (preg_match('/^M/',$MDnextCID)) )
				{
				## 修复外拨未应答挂断后vicidial_log无数捿+fnatic  
				## 增加对外拨电话类型的判断,条件为user!=VDAD并且campaign不能存在转接的前缀AGENTDIRECT +fnatic
				//if (strlen($uniqueid) == 0 && $user != 'VDAD' && strpos($campaign,'AGENTDIRECT')) 该语句有问题(modify bear)
				if (strlen($uniqueid) == 0 && $user != 'VDAD' && strpos($campaign,'AGENTDIRECT')== false) 
					{
					$stmt = "SELECT vicidial_campaigns.dial_prefix from vicidial.vicidial_campaigns where campaign_id='$campaign';";
                        		$rslt = mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
                			$row=mysql_fetch_row($rslt);
					$campaign_dial_prefix = $row[0];
					if ($campaign_dial_prefix > 0){
                			$stmt="SELECT user_group FROM vicidial_users where user='$user' LIMIT 1;";
                			$rslt=mysql_query($stmt, $link);
                        		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
                			if ($DB) {echo "$stmt\n";}
                			$ug_record_ct = mysql_num_rows($rslt);
                        		$row=mysql_fetch_row($rslt);
                        		$user_group = trim("$row[0]");
					$stmt = "SELECT call_log.uniqueid,start_time,start_epoch from call_log where call_log.extension='$campaign_dial_prefix$phone_number' and not exists (SELECT * from vicidial_log where vicidial_log.uniqueid=call_log.uniqueid) order by call_log.uniqueid desc limit 1";
					$rslt = mysql_query($stmt, $link);
					$row = mysql_fetch_row($rslt);
					$calllog_uniqueid = $row[0];
					$calllog_start_time = $row[1];
					$calllog_start_epoch = $row[2];
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
					if (strlen($calllog_uniqueid) > 0)
						{
						$end_time_n = time();
						$out_length_in_sec_n = $end_time_n - $calllog_start_epoch;
						$dispo_choice_now = N;
						$ring_in_sec = ring_in_sec;
						sleep(2);
						$stmt="SELECT dialstatus FROM vicidial_carrier_log where uniqueid='$calllog_uniqueid' and server_ip='$server_ip' and dialstatus IN('ANSWER','NOANSWER','CANCEL','BUSY','CHANUNAVAIL','CONGESTION') LIMIT 1;";
                            $rslt=mysql_query($stmt, $link);
                            if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$auto_submit_dispo,$user,$server_ip,$session_name,$one_mysql_log);}
                            if ($DB) {echo "$stmt\n";}
                            $CL_mancall_ct = mysql_num_rows($rslt);
                            if ($CL_mancall_ct > 0){
                            $row=mysql_fetch_row($rslt);
						$dialstatus =$row[0];
						if ($dialstatus=='CONGESTION' || $dialstatus=='BUSY'){$dispo_choice = "B";}
						if ($dialstatus=='CANCEL' || $dialstatus=='CHANUNAVAIL'){$dispo_choice = "N";}
						if ($dialstatus=='ANSWER')
						{
						$stmt="UPDATE  call_log set length_in_sec='$out_length_in_sec_n' where uniqueid='$calllog_uniqueid';";
						###writelog("----$stmt----");
						$rslt = mysql_query($stmt, $link);
						$affected_rows = mysql_affected_rows($link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
						$dispo_choice_now = $dispo_choice;
						$ring_in_sec = length_in_sec;
						}
						}
						if($auto_submit_dispo != 'Y'){$dispo_choice = $dispo_choice_agent;}
						$stmt="INSERT INTO vicidial_log (uniqueid,lead_id,list_id,campaign_id,call_date,start_epoch,status,phone_code,phone_number,user,comments,processed,user_group,alt_dial,end_epoch,$ring_in_sec) values('$calllog_uniqueid','$lead_id','$list_id','$campaign','$calllog_start_time','$calllog_start_epoch','$dispo_choice','$phone_code','$phone_number','$user','MANUAL','$dispo_choice_now','$user_group','$alt_dial','$end_time_n','$out_length_in_sec_n');";
						###writelog("----$stmt----");
						$rslt = mysql_query($stmt, $link);
						$affected_rows = mysql_affected_rows($link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
						$dispo_choice = $dispo_choice_now;
						}
					}
				}
				## 结束
				### Check carrier log for error
																				sleep(2);
                                        $stmt="SELECT dialstatus,hangup_cause FROM vicidial_carrier_log where uniqueid='$uniqueid' and server_ip='$server_ip' and dialstatus IN('BUSY','CANCEL','CHANUNAVAIL','CONGESTION') LIMIT 1;";
                                        $rslt=mysql_query($stmt, $link);
                                                if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
                                        if ($DB) {echo "$stmt\n";}
                                        $CL_mancall_ct = mysql_num_rows($rslt);
						...
						if ($dialstatus=='CONGESTION')
							...
							$dispo_choice = "B";
							if($auto_submit_dispo != 'Y'){$dispo_choice = $dispo_choice_agent;}
							$stmt="UPDATE vicidial_log set length_in_sec='0' where lead_id='$lead_id' and user='$user' and call_date > \"$four_hours_ago\" order by uniqueid desc limit 1;";
                                                	if ($format=='debug') {echo "\n<!-- $stmt -->";}
                                        		$rslt=mysql_query($stmt, $link);
                                                	if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'009999999999',$user,$server_ip,$session_name,$one_mysql_log);}
							}
				...
				$stmt="SELECT uniqueid FROM call_log where caller_code='$MDnextCID' and server_ip='$server_ip' order by start_time desc LIMIT 1;";
				...
				if ($VM_mancallX_ct > 0)
					{
					$row=mysql_fetch_row($rslt);
					$call_log_uniqueid =		$row[0];

					}
					
				$beginUNIQUEID = preg_replace("/\..*/","",$call_log_uniqueid);
				$stmt="SELECT count(*) from vicidial_log where lead_id='$lead_id' and user='$user' and phone_number='$phone_number' and uniqueid LIKE \"$beginUNIQUEID%\";";
				...
				if ($row[0] > 0)
					...
					if($auto_submit_dispo != "Y"){$dispo_choice = $dispo_choice_agent;}
					//$stmt="UPDATE vicidial_log set status='$dispo_choice' where lead_id='$lead_id' and user='$user' and call_date > \"$four_hours_ago\" order by uniqueid desc limit 1;";
					  $stmt="UPDATE vicidial_log set status='$dispo_choice' where lead_id='$lead_id' and user='$user' and uniqueid LIKE \"$beginUNIQUEID%\" order by uniqueid desc limit 1;";
						if ($format=='debug') {echo "\n<!-- $stmt -->";}
					$rslt=mysql_query($stmt, $link);
				...
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00216',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					$length_in_sec_temp = $StarTtime - $agent_dial_start_epoch;
					$agent_dial_start_date = date('Y-m-d H:i:s',$agent_dial_start_epoch);
                		$stmt = "SELECT uniqueid from call_log where number_dialed like '%$VLphone_number' order by uniqueid desc limit 1;";
                        if ($DB) {echo "$stmt\n";}
					$rslt=mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00251',$user,$server_ip,$session_name,$one_mysql_log);}
					$VDAP_out_ct = mysql_num_rows($rslt);
					if ($VDAP_out_ct > 0)
                                        {
                                        $row=mysql_fetch_row($rslt);
                                        $outuniqueid =           $row[0];
					...
					//$stmt = "INSERT INTO vicidial_log set uniqueid='$FAKEcall_id',lead_id='$lead_id',list_id='$VLlist_id',campaign_id='$campaign',call_date='$agent_dial_start_date',start_epoch='$agent_dial_start_epoch',end_epoch='$StarTtime',length_in_sec='$length_in_sec_temp',status='$dispo_choice',phone_code='$VLphone_code',phone_number='$VLphone_number',user='$user',comments='MANUAL',processed='N',user_group='$user_group',term_reason='AGENT',alt_dial='$VLalt';";
					if($auto_submit_dispo != 'Y'){$dispo_choice = $dispo_choice_agent;}
					$stmt = "INSERT INTO vicidial_log set uniqueid='$FAKEcall_id',lead_id='$lead_id',list_id='$VLlist_id',campaign_id='$campaign',call_date='$agent_dial_start_date',start_epoch='$StarTtime',end_epoch='$StarTtime',length_in_sec='0',status='$dispo_choice',phone_code='$VLphone_code',phone_number='$VLphone_number',user='$user',comments='MANUAL',processed='N',user_group='$user_group',term_reason='AGENT',alt_dial='$VLalt',status_type='$status_type';";
					
					if ($DB) {echo "$stmt\n";}
					$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00215',$user,$server_ip,$session_name,$one_mysql_log);}
					
					$stmt="UPDATE call_log set end_epoch='$StarTtime',length_in_sec='$length_in_sec_temp' where uniqueid='$FAKEcall_id' order by uniqueid desc limit 1;";
					$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00215',$user,$server_ip,$session_name,$one_mysql_log);}
					...
					$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00215',$user,$server_ip,$session_name,$one_mysql_log);}
					$stmt="UPDATE recording_log set vicidial_id='$FAKEcall_id' where filename like '%$VLphone_number' and user='$user' and vicidial_id is null order by start_time desc limit 1;";
					//writelog($stmt);
					$rslt=mysql_query($stmt, $link);
					$MAN_vl_insert++;
					}
				...
				## 修复外拨未应答挂断后vicidial_log无数捿+ fnatic  
					if($auto_submit_dispo != "Y"){$dispo_choice = $dispo_choice_agent;}
					$stmt="UPDATE vicidial_log set status='$dispo_choice',status_type='$status_type' where lead_id='$lead_id' and user='$user' and call_date > \"$four_hours_ago\" order by uniqueid desc limit 1;";
						if ($format=='debug') {echo "\n<!-- $stmt -->";}
					$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00145',$user,$server_ip,$session_name,$one_mysql_log);}
				}
	...
	####### START Vtiger Call Logging #######
		...
		$campaign_sql="SELECT * FROM ".$db_config["database"].".vicidial_campaigns where campaign_id = '$campaign'";
		$campaign_row=$db->row_query_one($campaign_sql);
		//echo '222222222'.$campaign_row['interation_vtiger'];
		if($campaign_row['integration_vtiger'] == 'N'){
			require('/var/www/html/config.php');
			//echo '/12345677';
				$stmt = "SELECT count(*) from ".$db_config["database"].".vicidial_inbound_groups where group_id='$stage';";
					if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_query($stmt, $link);
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00143',$user,$server_ip,$session_name,$one_mysql_log);}
				$row=mysql_fetch_row($rslt);
				...
				{	//inbound
					...
					//callout
					$sql="SELECT uniqueid,lead_id,list_id,campaign_id,call_date,start_epoch,end_epoch, length_in_sec,status,phone_code,phone_number,user,comments,processed,user_group, term_reason,alt_dial,talk_time,status_type,ring_in_sec,quality_status,call_des, talk_length_sec FROM ".$db_config["database"].".vicidial_log where uniqueid='$uniqueid';";
					$call_type_ss=0;
					echo "66666666666/";
				}
				
				$row_q=$db->row_query_one($sql);
				if($db->isNeedSyn($row_q)){
					$db->SynRecords($row_q,$call_type_ss);
					if($call_type_ss==1){
							require('/var/www/html/config.php');
							$str = "UPDATE ".$db_config["database"].".vicidial_closer_log SET drop_history='1'  where uniqueid='$uniqueid'";
							$db->row_query($str);
						}
					echo '11111111111'.$row_q['uniqueid'];
					...
					if (ereg('DISPO',$vtiger_create_call_record))
						...
						if ($vtiger_callback_id > 0)
							...
							if ($activity_check > 0)
								...
								$stmt = "UPDATE vtiger_crmentity SET modifiedby='$user_id', description='$act_description - CCMS Call user $user',modifiedtime='$NOW_TIME',viewedtime='$NOW_TIME' where crmid='$vtiger_callback_id';";
								if ($DB) {echo "|$stmt|\n";}
								$rslt=mysql_query($stmt, $linkV);
	...
	if (strlen($dispo_call_url) > 7)
		...
		//$stmt = "SELECT enable_vtiger_integration FROM system_settings;";
		$stmt = "SELECT enable_vtiger_integration FROM vicidial_campaigns where campaign_id='$campaign';";
		$rslt=mysql_query($stmt, $link);
	...
	echo 'Lead ' . $lead_id . ' 已被修改丿' . $dispo_choice . " 状态\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'updateLEAD')
	...
	if ( (strlen($phone_number)<1) || (strlen($lead_id)<1) )
		{
		echo "phone_number $phone_number or lead_id $lead_id 是无效的\n";
		exit;
		}
	else
		{
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
	...
	if ( (strlen($stage)<2) || (strlen($server_ip)<1) )
		...
		$random = (rand(1000000, 9999999) + 10000000);
		$stmt="UPDATE vicidial_live_agents set lead_id=0,uniqueid=0,callerid='',channel='', random_id='$random',comments='',last_state_change='$NOW_TIME' $vla_autodialSQL where user='$user' and server_ip='$server_ip';";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
		$rslt=mysql_query($stmt, $link);
		...
		if ($ACTION == 'VDADpause')
			...
			$row=mysql_fetch_row($rslt);
			$uppause_sec =  $row[0];
			$from_pause_sec = ",pause_sec='$wait_sec'";
			if ($uppause_sec > 0)
			{
			$from_pause_sec = "";
			}
			...
			if ( (eregi("NULL",$dispo_epoch)) or ($dispo_epoch < 1000) )
				{
				$stmt="UPDATE vicidial_agent_log set wait_sec='$wait_sec'$from_pause_sec where agent_log_id='$agent_log_id';";
					if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_query($stmt, $link);
...
if ($ACTION == 'UpdatEFavoritEs')
	...
	if ( (strlen($favorites_list)<1) || (strlen($user)<1) || (strlen($exten)<1) )
		{
		echo "favorites list $favorites_list 是无效的\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'PauseCodeSubmit')
	...
	if ( strlen($agent_log_id)<1 )
		{
		echo "agent_log_id $agent_log_id or pause_code $status 是无效的\n";
		exit;
		}
	else
		{
		...
		if ($stage < 1)
			{
		### used to every times insert pausedcode success ,delete codition NULL or ''
		...
		if ($affected_rows > 0) 
			...
			##############################################
			##### ADD BY FNATIC LOOKUP PAUSECODE DESC START#####
            $pause_code_desc="";
            $stmt="select vicidial_pause_codes.pause_code_name from vicidial_pause_codes  where vicidial_pause_codes.pause_code='".$status."' and vicidial_pause_codes.campaign_id='". $campaign . "'";
			
			$rslt=mysql_query($stmt, $link);
            $row=mysql_fetch_row($rslt);
            $pause_code_desc=$row[0];
			if($pause_code_desc == ""){
				$pause_code_desc = $status;
			}
			##############################################
			##### ADD BY FNATIC LOOKUP PAUSECODE DESC END#####
...
if ($ACTION == 'ringcallredirect')
	{
		
		$stmt="SELECT callerid from vicidial_auto_calls where auto_call_id='$stage'";
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$callerid =	$row[0];

		$stmt="update vicidial_live_agents set comments = '$callerid' where user='$account';";
		$rslt=mysql_query($stmt, $link);
...
if ($ACTION == 'AGENTSview')
	...
	*/
	...
	if (isset($_GET["agentViewTechgroup"]))					{$agentViewTechgroup=$_GET["agentViewTechgroup"];}
		elseif (isset($_POST["agentViewTechgroup"]))			{$agentViewTechgroup=$_POST["agentViewTechgroup"];}
	if (isset($_GET["agentViewUsergroup"]))					{$agentViewUsergroup=$_GET["agentViewUsergroup"];}
		elseif (isset($_POST["agentViewUsergroup"]))			{$agentViewUsergroup=$_POST["agentViewUsergroup"];}
	
	if(strpos($agentViewUsergroup,",")){
		$AGENTviewSQL = " and vu.user_group in($agentViewUsergroup)";
	}else{
		$AGENTviewSQL = " and vu.user_group='$agentViewUsergroup'";
	}
	if(strpos($agentViewTechgroup,",")){
		$temp_arr = explode(",",$agentViewTechgroup);
		$sql_arr = array();
		$remote_sql_arr = array();
		foreach($temp_arr as $temp){
			$sql_arr[] = "vla.closer_campaigns LIKE '% $temp %'";
			$remote_sql_arr[] = " LOCATE('$temp',closer_campaigns)<>'0'";
		}
		$AGENTviewSQL2 = " and (" . implode(" or ",$sql_arr) . ")";
		$remot_techgp_SQL = " and (" . implode(" or ",$remote_sql_arr) . ")";
	}else{
		$AGENTviewSQL2 = " and vla.closer_campaigns LIKE '% $agentViewTechgroup %'";
		$remot_techgp_SQL = " and LOCATE('$agentViewTechgroup',closer_campaigns)<>'0'";
	}
	echo "<font style=\"font-size:10px;font-family:sans-serif;\"><font style=\"background-color:#ADD8E6;\"> &nbsp; &nbsp;&nbsp; &nbsp;</font> $lang_agent_available &nbsp; <font style=\"background-color:#D8BFD8;\">&nbsp; &nbsp;&nbsp; &nbsp;</font> $lang_agent_talking &nbsp; <font style=\"background-color:#F0E68C;\"> &nbsp; &nbsp;&nbsp; &nbsp;</font> $lang_agent_pause &nbsp;\n";
	...
	$stmt="SELECT vla.user,vla.status,vu.full_name,UNIX_TIMESTAMP(last_call_time),UNIX_TIMESTAMP(last_call_finish),extension,campaign_id,sub_status,UNIX_TIMESTAMP(last_state_change) from vicidial_live_agents vla,vicidial_users vu where vla.user=vu.user $AGENTviewSQL $AGENTviewSQL2 order by vu.full_name;";
	...
	while ($agents_count > $loop_count)
		...
		$call_finish =	$row[4];
		$extension = $row[5];
		$this_campaign_id =	$row[6];
		$this_status =	$row[7];
		$last_state_change = $row[8];
		$call_time_S = ($StarTtime - $last_state_change);
		$call_time_MS = ($call_time_S / 60);
		$call_time_MS_int = floor($call_time_MS);
		$call_time_MS_int = intval("$call_time_MS_int");
		$call_time_S = ($call_time_MS - $call_time_MS_int);
		$call_time_S = ($call_time_S * 60);
		$call_time_S = round($call_time_S, 0);
		if ($call_time_S < 10) {$call_time_S = "0$call_time_S";}
		$call_time_last = "$call_time_MS_int:$call_time_S";
		$call_time_MS =		($call_time_S / 60);
		$call_time_MS =		sprintf("%7s", $call_time_MS);
		$local_phone = explode("/",$extension);
		$dial_link = "";
		if ( ($status=='READY') or ($status=='CLOSER') ) 
			{
			$statuscolor='#ADD8E6';
			$call_time = ($StarTtime - $call_finish);
			if($redirectcalltoagentid == ""){
				$dial_link = "<a href=\"#\" onclick=\"InternalDialByAgentClick('$local_phone[1]');\">[$lang_inline_dial]</a>";
			}else{
				$dial_link = "<a href=\"#\" onclick=\"ringcallredirect2('$redirectcalltoagentid','$user');\">[$lang_transfer]</a>";
			}
			}
		...
		if($campaign_system_status_content[$this_campaign_id][$status] == 'Y' || $campaign_system_status_content[$this_campaign_id][$status][$this_status]=='Y'){
			$dial_link = "<a href=\"#\" onclick=\"InternalDialByAgentClick('$local_phone[1]');\">[呼叫]</a>";
		}
		...
		if ($comments=='AgentXferViewSelect') 
			{
			echo "<TR BGCOLOR=\"$statuscolor\"><TD><font style=\"font-size: 12px; font-family: sans-serif;\"> &nbsp; <a href=\"#\" onclick=\"AgentsXferSelect('$row[0]','$comments');return false;\">$row[0] - $row[2]</a>&nbsp;</font></TD><TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; $call_time_last &nbsp;</font></TD>";
			...
			echo "$row[0] - $row[2] ";
			# Local internal call + fnatic
		    echo "$dial_link";
			# end 
			echo "&nbsp;</font></TD>";
			echo "<TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; $call_time_last &nbsp;</font></TD>";
			if ($agent_status_view_time > 0)
				{echo "<TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; $call_time &nbsp;</font></TD>";}
			echo "</TR>";
			}
...
//add by  pie 20130306 for "show remote agent";
//begin;
	$stmt="SELECT remote_agent_id,user_start,number_of_lines,server_ip,conf_exten,status,campaign_id FROM vicidial_remote_agents WHERE campaign_id='$campaign' $remot_techgp_SQL ORDER BY user_start ASC;";
	...
	if ($rslt)
	{
		$remote_agents_count = mysql_num_rows($rslt);
	}
	$loop_remote_count=0;
	while ($remote_agents_count > $loop_remote_count)
	{
		$row=mysql_fetch_row($rslt);
		$user_start =			$row[1];
		$conf_exten =	$row[4];
		...
		{
			$statuscolor='#ADD8E6';
			$dial_link = "<a href=\"#\" onclick=\"InternalDialByAgentClick('$conf_exten');\">[$lang_inline_dial]</a>";
//			echo "<input type=\"hidden\" name=\"isDial_link\"  id=\"isDial_link\"  value=\"1\">";
		...
		if ($comments=='AgentXferViewSelect') 
		{
			echo "<TR BGCOLOR=\"$statuscolor\"><TD><font style=\"font-size: 12px; font-family: sans-serif;\"> &nbsp; <a href=\"#\" onclick=\"AgentsXferSelect('$row[1]','$comments');return false;\">$row[1] - $row[1]</a>&nbsp;</font></TD>";
//			if ($agent_status_view_time > 0)
//				{echo "<TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; $call_time &nbsp;</font></TD>";}
			echo "</TR>";
		}
		else
		{
			...
			echo "$dial_link";
			# end 
			echo "<TD><font style=\"font-size: 12px; font-family: sans-serif;\">&nbsp  $call_time_last  </font></TD>";
	//		if ($agent_status_view_time > 0)
	//		{echo "<TD><font style=\"font-size: 12px;  font-family: sans-serif;\">&nbsp; $call_time &nbsp;</font></TD>";}
			echo "</TR>";
		}
...
if ($ACTION == 'CALLSINQUEUEview')
	{
	$stmt="SELECT view_calls_in_queue from vicidial_campaigns where campaign_id='$campaign'";
	if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
	$rslt=mysql_query($stmt, $link);
	...
	$stmt="SELECT grab_calls_in_queue from vicidial_users where user='$user'";
	if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
	$rslt=mysql_query($stmt, $link);
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00228',$user,$server_ip,$session_name,$one_mysql_log);}
	$row=mysql_fetch_row($rslt);
	$grab_calls_in_queue =	$row[0];

	if (eregi('NONE',$view_calls_in_queue))
		{
		echo "该Campaign已关闭查看队列中Calls的功能\n";
		exit;
		}
	else
		{
		...
		$stmt="SELECT lead_id,campaign_id,phone_number,uniqueid,UNIX_TIMESTAMP(call_time),call_type,auto_call_id from vicidial_auto_calls where status IN('LIVE','XFER') and ( (campaign_id='$Acampaign') or (campaign_id IN('$AccampSQL')) $ADsql) order by queue_priority,call_time;";
		if ($DB) {echo "|$stmt|\n";}
		$rslt=mysql_query($stmt, $link);
		...
		echo "<TR>";
		echo "<TD> &nbsp; </TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; $lang_customer_number &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; $lang_customer_name &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; $lang_customer_wait_time &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; $lang_customer_from &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; $lang_customer_in_group &nbsp; </font></TD>";
		echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; $lang_customer_type &nbsp; </font></TD>";
		echo "</TR>";
		...
		while ( ($calls_count > $loop_count) and ($view_calls_in_queue > $loop_count) )
			...
			if ( (eregi('Y',$grab_calls_in_queue)) and ($OQcall_type[$loop_count]=='IN') )
				{
				echo "<TR $Qcolor>";
				echo "<TD> <a href=\"#\" onclick=\"checkqueuegrab('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">提取</a> &nbsp;<a href=\"#\" onclick=\"ringcallredirect('$OQauto_call_id[$loop_count]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">转接</a></TD>";
...
if ($ACTION == 'CALLSINQUEUEgrab')
	...
	if ( (eregi('NONE',$view_calls_in_queue)) or (eregi('N',$grab_calls_in_queue)) )
		{
		echo "ERROR: 该Campaign已关闭查看队列中Calls的功能\n";
		exit;
		}
	else
		{
...
if ($ACTION == 'outbound_redirect_line2')
	{
		if (isset($_GET["session_id"]))				{$session_id=$_GET["session_id"];}
		elseif (isset($_POST["session_id"]))		{$session_id=$_POST["session_id"];}
		//writelog("---------outbound_redirect_line2---$exten---+++++$session_id----$server_ip----");
		$live_channel_line2 = "";
		$recording_channe_line2 = "";
		#####line2 channel
		$extension_line2 = $session_id;
		if($exten=='8301'){
			$extension_line2 = $session_id;
		}else if($exten==$session_id){
			$extension_line2 = "park";
		}
		$stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and extension = '$extension_line2' and channel like 'Local/111$session_id%';";
		$rslt=mysql_query($stmt, $link);
		//writelog("++++++$stmt+++++++++++++++++++++");
		$row=mysql_fetch_row($rslt);
		$live_channel_line2 = "$row[0]";
		$stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and extension = '$extension_line2' and channel like 'Local/4$session_id%';";
		//writelog("++++++$stmt+++++++++++++++++++++");
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$recording_channe_line2 = "$row[0]";
		//writelog("++++++$live_channel_line2++++++++++$recording_channe_line2+++++++++++++++++++++");
		if($live_channel_line2!=""){
		
			if($exten=='8301'){
				$stmt="INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Redirect','$queryCID','Channel: $live_channel_line2','Context: default','Exten: 8301','Priority: 1','CallerID: $queryCID','ExtraChannel: $recording_channe_line2','','','','');";
			}else if($exten==$session_id){
				$stmt="INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Redirect','$queryCID','Channel: $live_channel_line2','Context: default','Exten: 111$session_id','Priority: 1','CallerID: $queryCID','ExtraChannel: $recording_channe_line2','','','','');";
			}
			//writelog("-------$stmt--------");
			$rslt=mysql_query($stmt, $link);
		}
...
if ($ACTION == 'CALLSOUTBOUNDECHECK')
	{
	$stmt="SELECT count(*) from live_sip_channels where channel like 'Local/111$conf_exten%';";
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$count=$row[0];
	echo "$count";
...
if ($ACTION == 'outbound_dial_line2')
    {
        
        $filename=date("Ymd")."-".date("His")."_"."$phone_number";
        $uniqueid=$StarTtime.".".$conf_exten;
        $start_epoch = time();
        
        $agent_log_phone_number = $phone_number;
        ...
        $phone_number = "$dial_prefix" . "$phone_number";
        $stmt = "INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Originate','M0315141603035363273','Exten: $phone_number','Context: default','Channel: Local/111$conf_exten@default','Priority: 1','Callerid: M0315141603035363273','Timeout: 60000','$account','','','');";
        $rslt=mysql_query($stmt, $link);

        $stmt="INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Originate','$filename','Channel: Local/4$conf_exten@default','Context: default','Exten: 8311','Priority: 1','Callerid: $filename','','','','','');";
        $rslt=mysql_query($stmt, $link);

        //recording_log
        $stmt = "INSERT INTO recording_log (channel,server_ip,extension,start_time,start_epoch,filename,lead_id,user,vicidial_id,campaign) values('','$server_ip','','$NOW_TIME','$StarTtime','$filename','','$user','$uniqueid','$campaign')";
        $rslt=mysql_query($stmt, $link);
        
       // $stmt = "INSERT INTO `call_log` (`uniqueid` ,`channel` ,`channel_group` ,`type` ,`server_ip` ,`extension` ,`number_dialed` ,`caller_code` ,`start_time` ,`start_epoch` ,`end_time` ,`end_epoch` ,`length_in_sec` ,`length_in_min` )VALUES ('$uniqueid', 'Channel: Local/111$conf_exten@default', '', 'Local', '$server_ip', '$extension', '', '$extension', '$StarTtime' , '$start_epoch', NULL , NULL , NULL , NULL );";
      //  $rslt=mysql_query($stmt, $link);
        
        //检查lead_id是否为0 add by heibo 
        $stmt="SELECT manual_dial_list_id from vicidial_campaigns where campaign_id='$campaign'";
        $rslt=mysql_query($stmt, $link);
        $list_id_count = mysql_num_rows($rslt);
        if ( $list_id_count > 0 ){
        	   $rowx=mysql_fetch_row($rslt);
        	   $list_id = $rowx[0];
        	   
        	   $stmt = "SELECT lead_id FROM vicidial_list where phone_number='$agent_log_phone_number' order by modify_date desc LIMIT 1;";
        	   //file_put_contents("/tmp/debug2.txt",$stmt . "\r\n",FILE_APPEND);
        	   $rslt=mysql_query($stmt, $link);
        	   $manual_leadid_ct = mysql_num_rows($rslt);
        	   if ( $manual_leadid_ct > 0 ){
        	   	    $row_lead = mysql_fetch_row($rslt);
        	   	    $lead_id  = $row_lead[0];
        	   }else{
				          $stmt = "INSERT INTO vicidial_list SET phone_number='$agent_log_phone_number',list_id='$list_id',status='GRAB',user='$user',called_since_last_reset='Y',entry_date='$ENTRYdate',last_local_call_time='$NOW_TIME';";
				          $rslt = mysql_query($stmt, $link);
				          $lead_id = mysql_insert_id($link);   
        	 	 }
        }
        //end lead_id
			  ...
			  //vicidial_agent_log  bug 2187 提取电话少插入数据
        $stmt="INSERT INTO vicidial_agent_log (user,server_ip,lead_id,status,event_time,campaign_id,talk_epoch,pause_epoch,pause_sec,wait_epoch,user_group) values('$user','$server_ip','$lead_id','GRAB','$NOW_TIME','$campaign','$StarTtime','$StarTtime','0','$StarTtime','$user_group');";
        $rslt=mysql_query($stmt, $link);
        //file_put_contents("/tmp/debug2.txt",$stmt . "\r\n",FILE_APPEND);
		    if ($DB) {echo "$stmt\n";}
			  if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'5002187',$user,$server_ip,$session_name,$one_mysql_log);}
		    $agent_log_id = mysql_insert_id($link);
		    //end bug 2187 由于提取电话小结状态没有,默认给一个'GRAB'        
        
        echo $uniqueid . "|" . $agent_log_id;
        exit;
        
    }
if ($ACTION == 'outbound_hangup_line2')
    {
		//writelog("--------------outbound_hangup_line2------------");
        $term_reason = "NONE";
        $end_epoch = time();
        //挂断外线
        $stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and (extension = '$conf_exten' or extension = 'park') and channel like 'Local/111$conf_exten%';";
        $rslt=mysql_query($stmt, $link);
        $row=mysql_fetch_row($rslt);
        $GCchannel = "$row[0]";
        if($GCchannel!=""){
            $stmt= "INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Hangup','HLvdcW1320207256even','Channel: $GCchannel','','','','','','','','','');";
            $rslt=mysql_query($stmt, $link);
            $term_reason = "AGENT";
        }else{
            $term_reason = "CALLER";
        ...
        //提取录音通道信息
        $stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and (extension = '$conf_exten' or extension = 'park') and channel like 'Local/4$conf_exten%';";
        $rslt=mysql_query($stmt, $link);
        $row=mysql_fetch_row($rslt);
        $recording_channel = "$row[0]";
        //结束录音
        $stmt= "INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Hangup','RH1234513202198020','Channel: $recording_channel','','','','','','','','','');";
        $rslt=mysql_query($stmt, $link);

		
        $stmt="SELECT start_epoch FROM vicidial_log where uniqueid = '$uniqueid';";
        $rslt=mysql_query($stmt, $link);
        $row=mysql_fetch_row($rslt);
        $start_epoch = "$row[0]";
        
        $length_in_sec = $end_epoch-$start_epoch;
        $length_in_min = round($length_in_sec/60,2);
		//writelog("==========1>$stmt|$start_epoch|$end_epoch|");
		
        //vicidial_log
        $stmt="UPDATE vicidial_log set term_reason='$term_reason', end_epoch='$StarTtime', length_in_sec='$length_in_sec', status='GRAB' where uniqueid='$uniqueid' and user='$user' order by call_date desc limit 1;";
        $rslt=mysql_query($stmt, $link);
        //recording_log
        $stmt = "update recording_log set end_time='$NOW_TIME',end_epoch='$StarTtime' where vicidial_id='$uniqueid' and user='$user'";
        $rslt=mysql_query($stmt, $link);
        
        echo "recording_log ..........ssssssss"."<br />";
        
		    //bug 2187 挂线时更新vicidial_agent_log 
	      $talk_sec_line2 = 0;
	      $talk_epochSQL_line2 = '';

	      $StarTtime_line2 = date("U");
	      $stmt = "select talk_epoch from vicidial_agent_log where agent_log_id='$agent_log_id_line2';";
	      if ($DB) {echo "$stmt\n";}
	      $rslt=mysql_query($stmt, $link);
		    if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'9002187',$user,$server_ip,$session_name,$one_mysql_log);}
	      $VDpr_ct_line2 = mysql_num_rows($rslt);
	      if ($VDpr_ct_line2 > 0)
		    {
		        $row=mysql_fetch_row($rslt);

			      $talk_epochSQL_line2 = ",talk_epoch='$StarTtime_line2',dead_epoch='$StarTtime_line2' ";
		        $talk_sec_line2 = $StarTtime_line2 - $row[0];
		    }
	      $stmt="UPDATE vicidial_agent_log set talk_sec='$talk_sec_line2',dispo_epoch='$StarTtime_line2' $talk_epochSQL_line2 where agent_log_id='$agent_log_id_line2';";
	      //file_put_contents("/tmp/debug2.txt",$stmt . "|$agent_log_id_line2|$talk_epochSQL_line2|\r\n",FILE_APPEND);
		    if ($format=='debug') {echo "\n<!-- $stmt -->";}
	      $rslt=mysql_query($stmt, $link);
		    if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'2002187',$user,$server_ip,$session_name,$one_mysql_log);}
		    // bug 2187 end
			
		/*	
        $stmt = "SELECT start_time FROM call_log where uniqueid = '$uniqueid';";
        $rslt=mysql_query($stmt, $link);
        $rows_ = mysql_num_rows($rslt);
        if ( $rows_ > 0 ){
        	$row_array = mysql_fetch_row($rslt);
			$row_start_time = $row_array[0];
		}
		writelog("========1>".$stmt);
        ...
        */
        ...
        if ( $rows_ > 0 ){
        	$row_array = mysql_fetch_row($rslt);
			$line2_start_epoch = $row_array[0];
		}
		
		
        $stmt = "UPDATE call_log set end_time = '$NOW_TIME',end_epoch = '$end_epoch', length_in_sec = '$length_in_sec' ,length_in_min = '$length_in_min',uniqueid = $uniqueid where end_epoch <= '$end_epoch' and channel LIKE 'Local/111$conf_exten@%' order by start_time desc limit 1";
        $rslt=mysql_query($stmt, $link);
    } 
if ($ACTION == 'agent_grab_line2')
	{
		//writelog("---------agent_grab_line2--------");
		### grab the status and campaign/in-group information for this agent to display
		$ADsql='';
		$stmt="SELECT status,campaign_id,closer_campaigns from vicidial_live_agents where user='$user' and server_ip='$server_ip';";
		if ($DB) {echo "|$stmt|\n";}
		...
		$stmt="SELECT lead_id,campaign_id,phone_number,uniqueid,UNIX_TIMESTAMP(call_time),call_type,auto_call_id from vicidial_auto_calls where status IN('LIVE') and agent_only='$user' and ( (campaign_id='$Acampaign') or (campaign_id IN('$AccampSQL')) $ADsql) order by queue_priority,call_time limit 1;";
		
		$rslt=mysql_query($stmt, $link);
		if ($rslt) {$calls_count = mysql_num_rows($rslt);}
		//writelog($calls_count . '---' . $stmt);
		if ($calls_count==1){
			$row=mysql_fetch_row($rslt);
			echo "<input type='button' onclick=\"checkqueuegrab('$row[6]');return false;\" value=\"提取\" />";
			//echo "<a href=\"#\" onclick=\"checkqueuegrab('$row[6]');return false;\"><font style=\"font-size: 11px; font-family: sans-serif;\">提取</a>";
		}else{
			echo "";
		}
...
if ($ACTION == 'CALLSINQUEUEgrab2')
	{
	//writelog("--inbound--grab-------");
	$stmt="SELECT view_calls_in_queue from vicidial_campaigns where campaign_id='$campaign'";
	if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
	$rslt=mysql_query($stmt, $link);
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00233',$user,$server_ip,$session_name,$one_mysql_log);}
	...
	$grab_calls_in_queue =	$row[0];
		...
		//提取录音通道信息
		$stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and extension = '$conf_exten' and channel like '%$conf_exten%';";
		
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$recording_channel = "$row[0]";
		$recording_channel = str_replace(",","@",$recording_channel);
		$recording_channel_arr = explode("@",$recording_channel);
		$recording_channel = $recording_channel_arr[1];

		$stmt="UPDATE vicidial_auto_calls set status='CLOSER',stage='CLOSER-0' where auto_call_id='$stage' and status='LIVE';";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
		$rslt=mysql_query($stmt, $link);
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00234',$user,$server_ip,$session_name,$one_mysql_log);}
		$affected_rows = mysql_affected_rows($link);
		if ($affected_rows > 0) 
			{
			$stmt="SELECT call_time,campaign_id,uniqueid,phone_number,lead_id,queue_priority,call_type,callerid,channel from vicidial_auto_calls where auto_call_id='$stage';";
			$rslt=mysql_query($stmt, $link);
				if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00270',$user,$server_ip,$session_name,$one_mysql_log);}
			if ($rslt) {$vac_count = mysql_num_rows($rslt);}
			if ($vac_count > 0)
			...
			//vicidial_closer_log
			$stmt = "UPDATE vicidial_closer_log set comments='line2',user_group='$user_group',status='INCALL',user='$user',term_reason='ABANDON' where uniqueid='$GCuniqueid' and status='QUEUE';";
			$rslt=mysql_query($stmt, $link);
			//拉外线进会议室
			$stmt= "INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Redirect','$GCcaller_id','Channel: $GCchannel','Context: default','Exten: $conf_exten','Priority: 1','CallerID: $GCchannel','','','','','');";
			$rslt=mysql_query($stmt, $link);
			//开始录音
			$file_name = $ENTRYdate . '_' . $GCphone_number;
			$stmt = "INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Originate','$file_name','Channel: Local/4$conf_exten@default','Context: default','Exten: $last_name','Priority: 1','Callerid: $file_name','','','','','');";
			$rslt=mysql_query($stmt, $link);

			//recording_log
			$stmt = "INSERT INTO recording_log (channel,server_ip,extension,start_time,start_epoch,filename,lead_id,user,vicidial_id,campaign) values('','$server_ip','','$NOW_TIME','$StarTtime','$file_name','','$user','$GCuniqueid','$campaign')";
			$rslt=mysql_query($stmt, $link);
			
			//vicidial_agent_log  bug 2187 提取电话少插入数据
      $stmt="INSERT INTO vicidial_agent_log (user,server_ip,lead_id,status,event_time,campaign_id,talk_epoch,pause_epoch,pause_sec,wait_epoch,user_group) values('$user','$server_ip','$GClead_id','GRAB','$NOW_TIME','$campaign','$StarTtime','$StarTtime','0','$StarTtime','$user_group');";
      $rslt=mysql_query($stmt, $link);
      //file_put_contents("/tmp/debug.txt",$stmt . "\r\n",FILE_APPEND);
		  if ($DB) {echo "$stmt\n";}
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'0002187',$user,$server_ip,$session_name,$one_mysql_log);}
		  $agent_log_id = mysql_insert_id($link);
		  //end bug 2187 由于提取电话小结状态没有,默认给一个'GRAB'
		      
			echo "$stage|$recording_channel|$GCchannel|$GCuniqueid|$agent_log_id";
			exit;
			}
		else
			{
...
if ($ACTION == 'CALLSINQUEUECHECK')
	{
	//writelog("AAAAAAAAAAAAAAAA");
	$stmt="SELECT count(*) from vicidial_auto_calls where auto_call_id='$stage';";
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$count=$row[0];
	echo "$count";
...
if ($ACTION == 'CALLSINQUEUEHANGUP')
	{
	$recording_channel_arr = explode("|",$stage);
	$stage = $recording_channel_arr[0];
	$original_record_str = $recording_channel_arr[1];
	$GCchannel = $recording_channel_arr[2];
	$GCuniqueid = $recording_channel_arr[3];
	$endtype = $recording_channel_arr[4];
		//挂断外线
		$stmt= "INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Hangup','HLvdcW1320207256even','Channel: $GCchannel','','','','','','','','','');";
		$rslt=mysql_query($stmt, $link);
		//vicidial_closer_log
		$stmt="SELECT start_epoch FROM vicidial_closer_log where uniqueid='$GCuniqueid' and status='INCALL';;";
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$length_in_sec = $StarTtime - $row[0];
		if($endtype == 0){
			$stmt = "UPDATE vicidial_closer_log set status='GRAB',user='$user',term_reason='AGENT',end_epoch='$StarTtime' where uniqueid='$GCuniqueid' and status='INCALL';";
		}else{
			$stmt = "UPDATE vicidial_closer_log set status='GRAB',user='$user',term_reason='CALLER',end_epoch='$StarTtime' where uniqueid='$GCuniqueid' and status='INCALL';";
		}
		$rslt=mysql_query($stmt, $link);
		//提取录音通道信息
		$stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and extension = '$conf_exten' and channel like '%$conf_exten%' and channel not like '$conf_exten@$original_record_str';";
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$recording_channel = "$row[0]";
		//结束录音
		...
		$rslt=mysql_query($stmt, $link);
		//recording_log
		$stmt = "update recording_log set end_time='$NOW_TIME',end_epoch='$StarTtime' where vicidial_id='$GCuniqueid' and user='$user'";
		$rslt=mysql_query($stmt, $link);
		//bug 2187 挂线时更新vicidial_agent_log 
	  $talk_sec_line2 = 0;
	  $talk_epochSQL_line2 = '';
	  ...
	  if ($DB) {echo "$stmt\n";}
	  $rslt=mysql_query($stmt, $link);
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'1002187',$user,$server_ip,$session_name,$one_mysql_log);}
	  $VDpr_ct_line2 = mysql_num_rows($rslt);
	  if ($VDpr_ct_line2 > 0)
		{
		  $row=mysql_fetch_row($rslt);
	  ...
	  $stmt="UPDATE vicidial_agent_log set talk_sec='$talk_sec_line2',dispo_epoch='$StarTtime_line2' $talk_epochSQL_line2 where agent_log_id='$agent_log_id_line2';";
	  //file_put_contents("/tmp/debug.txt",$stmt . "|$agent_log_id_line2|$talk_epochSQL_line2|\r\n",FILE_APPEND);
		if ($format=='debug') {echo "\n<!-- $stmt -->";}
	  $rslt=mysql_query($stmt, $link);
		if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'2002187',$user,$server_ip,$session_name,$one_mysql_log);}
		// bug 2187 end
...
################################################################################
### CalL_Recording - List the USERONLY Recording for an agent
################################################################################
...
if ($ACTION == 'CalLBacKLisT')
	...
	if ($agentonly_callback_campaign_lock > 0)
		...
		{$campaignCBsql = '';}
	$stmt = "select callback_id,lead_id,campaign_id,status,entry_time,callback_time,comments from vicidial_callbacks where (recipient='ANYONE' or (recipient='USERONLY' and user='$user'))  $campaignCBsql and status NOT IN('INACTIVE','DEAD') order by callback_time;";
...
if ($ACTION == 'CalLBacKCounT')
	...
	if ($agentonly_callback_campaign_lock > 0)
		...
		{$campaignCBsql = '';}
	$stmt = "select count(*) from vicidial_callbacks where (recipient='ANYONE' or (recipient='USERONLY' and user='$user')) $campaignCBsql and status NOT IN('INACTIVE','DEAD');";
	//file_put_contents("yhb.txt",$stmt);
	if ($DB) {echo "$stmt\n";}
	$rslt=mysql_query($stmt, $link);
...
################################################################################
### CallingStatus - get calling status for transfer
################################################################################
if ($ACTION == 'CallingStatus')
{
		$transferTarget = $_REQUEST['transferTarget'];
		$transferNum	= $_REQUEST['transferNum'];
		if($transferTarget == 'out'){
			$stmt = "select count(*) from live_sip_channels where channel like '%$transferNum%' and extension = 'ring';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00252',$user,$server_ip,$session_name,$one_mysql_log);}
			if ($rslt) {$dialable_count = mysql_num_rows($rslt);}
			...
			$stmt = "select count(*) from vicidial_auto_calls where status='CLOSER' and agent_only = '$transferNum'";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00252',$user,$server_ip,$session_name,$one_mysql_log);}
			if ($rslt) {$dialable_count = mysql_num_rows($rslt);}
...
################################################################################
### add by pie 2013-04-11
### When the Agent change the Status to 'PAUSE', Get the Count of MAX Pauses for Check from 
### the 'ccms_campaigns' table
################################################################################
if ($ACTION == 'CheckMaxPauses')
{
	$stmt = "SELECT max_pauses from ccms_campaigns where campaign_id = '$campaign'";
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$max_pauses =$row[0];

	$stmt = "SELECT count(*) as pause_agents from vicidial_live_agents where campaign_id = '$campaign' and status='PAUSED'";
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$pause_agents =$row[0];
	...
	if($max_pauses!=0)
	{
		if($pause_agents>=$max_pauses)
		{
			echo "To many Agent is PAUSED !";
		}
		else
		{
			echo "null";
...
if ($ACTION == 'XferUpdate'){
	###writelog("-------$user---xfer--$xfre_action------$uniqueid----");
	if($uniqueid != '' && $user != '' && $xfre_action != ''){
		$now_date_epoch = time();
		if($xfre_action == 'start'){
			$stmt="SELECT `caller_code` FROM `call_log` where `uniqueid`='$uniqueid'";
			$rslt=mysql_query($stmt, $link);
			$row=mysql_fetch_row($rslt);
			$incall_type = $row[0];
			//writelog($stmt."==>".$incall_type);
			
			if(strpos($incall_type,"->") != false){
				$transfer_Xfre_Value = 0;
				$stmt = "UPDATE call_log set caller_code = REPLACE(caller_code,'-->','->'), start_time = '$NOW_TIME',start_epoch = '$now_date_epoch' where uniqueid = '$uniqueid';";
				$rslt=mysql_query($stmt, $link);
				//writelog($stmt);
		
				if(strpos($incall_type,"-->") != false){		#outbound
		
					$stmt = "UPDATE vicidial_log set call_date = '$NOW_TIME',start_epoch = '$now_date_epoch',end_epoch = '$now_date_epoch' where uniqueid = '$uniqueid';";
					$rslt=mysql_query($stmt, $link);
					//writelog("OUTBOUND==>".$stmt);
		
					$transfer_Xfre_Value = 1;
		
				}
				else{								#inbound
					$stmt = "UPDATE vicidial_closer_log set call_date = '$NOW_TIME',start_epoch = '$now_date_epoch' where uniqueid = '$uniqueid';";
					$rslt=mysql_query($stmt, $link);
					//writelog("INBOUND==>".$stmt);
					$transfer_Xfre_Value = 2;
				}
			}
		}
		elseif($xfre_action == 'hangup'){
			$stmt = "UPDATE vicidial_log set end_epoch = '$now_date_epoch',length_in_sec = ($now_date_epoch - start_epoch) where uniqueid = '$uniqueid';";
			$rslt=mysql_query($stmt, $link);
			//writelog("=========hangup==>".$stmt);
			
			$transfer_Xfre_Value = $transfer_Xfre_Value+2;
		}
	}
	echo "||transfer_Xfre_Value=$transfer_Xfre_Value";
}
...
//读取"来电扩展信息"
function getExtensionInfo($campaign,$uniqueid){ 
	global $link;
	$estra_info = '|';
	$stmt = "SELECT extension_info_integration_enable,extension_info_db_server_ip,extension_info_db_name,extension_info_db_login,extension_info_db_password,extension_info_sql from ccms_campaigns where campaign_id = '$campaign'";
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$extension_info_integration_enable =$row[0];
	$extension_info_db_server_ip =$row[1];
	$extension_info_db_name =$row[2];
	$extension_info_db_login =$row[3];
	$extension_info_db_password =$row[4];
	$extension_info_sql= $row[5];
	if($extension_info_integration_enable == 1 && !empty($extension_info_sql)){

		$link_extension = mysql_connect("$extension_info_db_server_ip", "$extension_info_db_login", "$extension_info_db_password");
		if (!$link_extension) 
		{
			return $estra_info;
		}
		mysql_select_db("$extension_info_db_name");
		$extension_info_sql = str_ireplace("--A--uniqueid--B--",$uniqueid,$extension_info_sql);
		//writelog("--------" . $extension_info_sql);
		$rslt=mysql_query($extension_info_sql, $link_extension);
		$row=mysql_fetch_row($rslt);
		if($row){
			$estra_info = $estra_info . $row[0];
		}
