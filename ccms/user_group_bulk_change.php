...
$browser = getenv("HTTP_USER_AGENT");
  ...
  if( (strlen($PHP_AUTH_USER)<2) or (strlen($PHP_AUTH_PW)<2) or (!$auth))
	...
	if($auth>0)
		{
			$stmt="SELECT full_name,change_agent_campaign,modify_timeclock_log,user_level,user from vicidial_users where user='$PHP_AUTH_USER' and pass='$PHP_AUTH_PW'";
			$rslt=mysql_query($stmt, $link);
			$row=mysql_fetch_row($rslt);
			$LOGfullname =				$row[0];
			...
			$LOGuser_level		  =		$user_level;
			$LOGuser_name_id	  =		$row[4];
		if ($webroot_writable > 0)
			{
			fwrite ($fp, "CCMS|GOOD|$date|$PHP_AUTH_USER|$PHP_AUTH_PW|$ip|$browser|$LOGfullname|\n");
			fclose($fp);
			}
		}
	else
...
if($user_level != 9)
{
	$stmt="select user_group,group_name from vicidial_user_groups where manager='$PHP_AUTH_USER' or supervisor='$PHP_AUTH_USER' order by user_group desc;";
}
else
{
