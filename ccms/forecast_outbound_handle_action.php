...
while($i < $group_ct)
{
	...
	$stmt = "update vicidial_campaigns set limit_used_leads_alert='$limit_used_leads_alert',list_lead_total=$list_lead_total,unused_leads='$unused_leads' where campaign_id='$current_campaign_id'";
	$result = mysql_query($stmt);
	...
	$stmt="select dial_level_change_date from vicidial_campaigns where campaign_id='$current_campaign_id' limit 1"; 
		$result = mysql_query($stmt, $link);
			...
			$dial_level4 = $dial_level_change_date;
			...
			if($result && mysql_num_rows($result)){
				...
				if($dial_level_change > 0){$updown = "<font color=blue> ↑</font>";}
				if($dial_level_change < 0){$updown = "<font color=blue> ↓</font>";}
...
foreach ($arr_all as $v){
	...
	if (count($listarr) > 2){
		$list_id = $listarr[0].",".$listarr[1]."   <a href=\"admin.php?list_search_campaign_id=$v[0]&list_search_active=Y\" target=\"_blank\"> More</a>";
		$list_name = $list_name_arr[0].",".$list_name_arr[1]."   <a href=\"admin.php?list_search_campaign_id=$v[0]&list_search_active=Y\" target=\"_blank\"> More</a>";
	}else{
