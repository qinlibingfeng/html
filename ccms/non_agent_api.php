...
# other programs to interact with all non-agent-screen CCMS functions
# 
# required variables:
#  - $user
#  - $pass
...
if (isset($_GET["user_phone_extension"]))       {$user_phone_extension=$_GET["user_phone_extension"];}
    elseif (isset($_POST["user_phone_extension"])) {$user_phone_extension=$_POST["user_phone_extension"];}
		...
		if ( ($allowed_user < 1) and ($source != 'queuemetrics') )
			...
			if ($session_exists < 1)
				...
				if ($stage == 'text')
					...
					$output .= "话务员: $agent_user - $full_name </TD><TD>\n";
					$output .= " &nbsp; Campaign: $campaign_id</TD><TD>\n";
					$output .= "<a href=\"#\" onclick=\"hide_ingroup_info();\">关闭</a></TD></TR><TR><TD COLSPAN=3 BGCOLOR=\"#CCCCFF\">\n";
					...
					if ( ($allowed_user_change_ingroups > 0) and ($stage == 'change') )
						...
						$output .= "<TR><TD ALIGN=RIGHT VALIGN=TOP>已选技能组: </TD><TD ALIGN=LEFT>\n";
						...
						$output .= "<TR><TD ALIGN=RIGHT>操作:\n";
						$output .= "</TD><TD ALIGN=LEFT>\n";
						$output .= "<SELECT SIZE=1 NAME=ingroup_add_remove_change ID=ingroup_add_remove_change>\n";
						$output .= "<option value=\"CHANGE\" >转组</option>\n";
						$output .= "<option value=\"ADD\"　>追加</option>\n";
						$output .= "<option value=\"REMOVE\" >删除</option>\n";
						$output .= "</SELECT>\n";
						$output .= "</TD></TR>\n";

						$output .= "<TR><TD ALIGN=RIGHT>混合模式(自动外拨):\n";
						$output .= "</TD><TD ALIGN=LEFT>\n";
						...
						$output .= "<INPUT TYPE=BUTTON NAME=SUBMIT ID=SUBMIT VALUE=\"提交\" onclick=\"submit_ingroup_changes('$agent_user')\">\n";
						$output .= "</TD></TR>\n";
		...
		if ( ($allowed_user < 1) and ($source != 'queuemetrics') )
			...
			if ($session_exists < 1)
				...
				if ( ($phone_exists < 1) and ($source != 'queuemetrics') )
					...
					### add by fnatic whisper function  start ###

					if ( (ereg('WHISPER',$stage))) 
						{						  
						  $stmt_0 = "select conf_exten from vicidial_conferences where  left(conf_exten,5) ='86003' and vicidial_conferences.conf_exten not in (select live_channels.extension from live_channels)";
						  $rslt_0 = mysql_query($stmt_0,$link);
                          $row_0 = mysql_fetch_row($rslt_0);
						  
						  $whisper_new_session_id = $row_0[0];//创建新的会议室
						  $whisper_old_session_id = '556'.$session_id;

						  $stage = '01'; //exten => _55886003XX,1,Meetme,${EXTEN:3}|AFzx1
						  $admin_phone_number = $dialplan_number; //admin分机

						  $user_stage = '02'; //exten => _55786003XX,1,Meetme,${EXTEN:3}|Fzx1
						  $user_phone_extension = '556'.$user_phone_extension; //耳语话务员分机

					      $stmt_1 = "INSERT INTO vicidial_manager values ('','','$NOW_TIME','NEW','N','$monitor_server_ip','','Originate','$BMquery','Channel: Local/$monitor_dialstring$stage$whisper_new_session_id@default','Context; default','Exten: $dialplan_number','Priority: 1','Callerid: \"VC Blind Monitor\" <$outbound_cid>','','','','','');";
                          $rslt=mysql_query($stmt_1, $link);
                         

						  $stmt_2 = "INSERT INTO vicidial_manager values ('','','$NOW_TIME','NEW','N','$monitor_server_ip','','Originate','$BMquery','Channel: Local/$monitor_dialstring$user_stage$whisper_new_session_id@default','Context; default','Exten: $user_phone_extension','Priority: 1','Callerid: \"VC Blind Monitor\" <$outbound_cid>','','','','','');";
					      $rslt=mysql_query($stmt_2, $link);
                        

//		                  $stmt = "INSERT INTO vicidial_manager values ('','','$NOW_TIME','NEW','N','$monitor_server_ip','','Originate','$BMquery','Channel: Local/$monitor_dialstring$user_stage$whisper_new_session_id@default','Context; default','Exten: $whisper_old_session_id','Priority: 1','Callerid: \"VC Blind Monitor\" <$outbound_cid>','','','','','');";

						}
					### add by fnatic whisper function  end ###
			        else 
						{
