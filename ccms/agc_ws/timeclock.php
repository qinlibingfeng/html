<?php
# timeclock.php - CCMS system user timeclock
# 
...
if ( ($stage == 'login') or ($stage == 'logout') )
	...
	$stmt="SELECT count(*) from vicidial_users where binary user='$user' and binary pass='$pass' and user_level > 0;";
	if ($DB) {echo "|$stmt|\n";}
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$valid_user=$row[0];
	print "<!-- CCMS_users active count for $user:   |$valid_user| -->\n";

	if ($valid_user < 1)
		{
		### NOT A VALID USER/PASS
		$VDdisplayMESSAGE = "您所输入的账号和密码并未在系统中被启动<BR>请重试:";
		...
		$stmt="SELECT full_name,user_group from vicidial_users where binary user='$user' and binary pass='$pass';";
		if ($DB) {echo "|$stmt|\n";}
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$full_name =	$row[0];
		$user_group =	$row[1];
		print "<!-- CCMS_users name and group for $user:   |$full_name|$user_group| -->\n";
		...
		if ($vts_count > 0)
			...
			print "<!-- CCMS_timeclock_status previous status for $user:   |$status|$event_epoch|$last_action_sec| -->\n";
			}
		else
			{
			...
			print "<!-- 新 CCMS_timeclock_status record inserted for $user:   |$affected_rows| -->\n";
			}
		if ( ($last_action_sec < 30) and ($status != 'START') )
			{
			### You cannot log in or out within 30 秒 of your last login/logout
			$VDdisplayMESSAGE = "您不能在前次注销或登入后30秒内再次注销或登入";
		...
		if ($commit == 'YES')
			...
			if ( ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') ) and ($stage=='login') )
				...
				print "<!-- 新 CCMS_timeclock_log record inserted for $user:   |$affected_rows|$timeclock_id| -->\n";
				...
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				print "<!-- CCMS_timeclock_status record updated for $user:   |$affected_rows| -->\n";
				...
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				print "<!-- 新 CCMS_timeclock_audit_log record inserted for $user:   |$affected_rows| -->\n";
				}

			if ( ($status=='LOGIN') and ($stage=='logout') )
				{
				$VDdisplayMESSAGE = "您目前已注销";
				$LOGtimeMESSAGE = "您注销在$NOW_TIME<BR>您登入的时间(多久时间): $totTIME_HMS";
				...
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				print "<!-- CCMS_timeclock_log record updated for $user:   |$affected_rows| -->\n";
				...
				$rslt=mysql_query($stmt, $link);
				$affected_rows = mysql_affected_rows($link);
				print "<!-- CCMS_timeclock_audit_log record updated for $user:   |$affected_rows| -->\n";
				}
			...
			if ($referrer=='agent') 
				{$BACKlink = "<A HREF=\"./vicidial.php?pl=$phone_login&pp=$phone_pass&VD_login=$user\"><font color=\"#003333\">回到话务员登入界面</font></A>";}
			if ($referrer=='admin') 
				{$BACKlink = "<A HREF=\"../vicidial/admin.php\"><font color=\"#003333\">回到系统管理</font></A>";}
			if ($referrer=='welcome') 
				{$BACKlink = "<A HREF=\"$welcomeURL\"><font color=\"#003333\">回到欢迎界面</font></A>";}
		...
		if ( ($status=='AUTOLOGOUT') or ($status=='START') or ($status=='LOGOUT') )
			{
			$VDdisplayMESSAGE = "自从您上次登入已经有(多久时间): $totTIME_HMS";
			$log_action = 'login';
			$button_name = 'LOGIN';
			$LOGtimeMESSAGE = "您上次注销在: $last_action_date<BR><BR>点选下方登入进行登入";
			}
		if ($status=='LOGIN')
			{
			$VDdisplayMESSAGE = "您已经登入的时间(多久时间): $totTIME_HMS";
			$log_action = 'logout';
			$button_name = 'LOGOUT';
			$LOGtimeMESSAGE = "您登入在: $last_action_date<BR>您已经登入的时间(多久时间): $totTIME_HMS<BR><BR>点选下方注销进行注销";
			}
