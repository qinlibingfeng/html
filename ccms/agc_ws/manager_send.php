...
# 50804-1148 - Added RedirectVD for CCMS blind redirection with logging
...
# 51118-1204 - Fixed Blind transfer bug from CCMS when in manual dial mode
# 51129-1014 - Added ability to accept calls from other CCMS servers
# 51129-1253 - Fixed Hangups of other agents channels in CCMS AD
...
# 80331-1433 - Added second transfer try for CCMS transfers on manual dial calls
...
#Add by fantic - fix recording log filename bug
if (isset($_GET["custphone"]))				{$custphone=$_GET["custphone"];}
	elseif (isset($_POST["custphone"]))		{$custphone=$_POST["custphone"];}
//$filename=date("Ymd")."-".date("His")."_".$custphone;
$filename= $user . "-" . date("Ymd")."-".date("His")."_".$custphone; 
#End
...
//yhb_log(YHB_LOG,$ACTION,$user,"manager");
...
if ($ACTION=="OriginateVDRelogin")
	...
	if ( ($enable_sipsak_messages > 0) and ($allow_sipsak_messages > 0) and (eregi("SIP",$protocol)) )
		...
		//file_put_contents("yhb.txt","/usr/local/bin/sipsak -M -O desktop -B \"$SIPSAK_prefix$campaign\" -r 5060 -s sip:$extension@$phone_ip > /dev/null");
...
if ($ACTION=="Redirect")
	{
	### for manual dial CCMS calls send the second attempt to transfer the call
	if ($stage=="2NDXfeR")
		{
		$local_DEF = 'Local/';
		$local_AMP = '@';
...
if ( ($ACTION=="Monitor") || ($ACTION=="StopMonitor") )
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($filename)<8) )
		...
		if ($channel_live==1)
			...
			if ($ACTION=="Monitor")
				{
				$stmt = "INSERT INTO recording_log (channel,server_ip,extension,start_time,start_epoch,filename,lead_id,user,campaign) values('$channel','$server_ip','$exten','$NOW_TIME','$StarTtime','$filename','$lead_id','$user','$campaign')"; 
					if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_query($stmt, $link);
		...
		if ($ACTION=="MonitorConf")
			...
			if ($FROMvdc=='YES')
				...
				if ($VLA_inOUT == 'INBOUND')
					...
					##### look for the CCMS ID in the vicidial_closer_log table
					...
					{
					##### look for the CCMS ID in the vicidial_log table
