...
#
# CHANGELOG
# 50607-1426 - First Build of CCMS web client basic login process finished
...
# 50804-1628 - Added Blind transfer, activated 650LIVE CALL image and fixed bugs
...
# 50818-1327 - Added stop-all-recordings-after-each-CCMS-call option
...
# 51129-1010 - Added ability to accept calls from other CCMS servers
...
# 80331-1433 - Added second transfer try for CCMS transfers/hangups on manual dial calls
...
# 80707-2325 - Added vicidial_id to recording_log for tracking of CCMS or closer log to recording
...
# 100913-0914 - Addded Manual_Ring_Launch/WebForm_Button_Display control


$version = '3.0.3 (Build 03)';
$build = '001';
...
if (isset($_GET["opener"]))				{$opener=$_GET["opener"];}
        elseif (isset($_POST["opener"]))  {$opener=$_POST["opener"];}
if (isset($_GET["user_inbound_mode"]))				{$user_inbound_mode=$_GET["user_inbound_mode"];}
        elseif (isset($_POST["user_inbound_mode"]))  {$user_inbound_mode=$_POST["user_inbound_mode"];}
...
//当opener和VD_campaign不为空时在新窗口中打开
if(($opener == "y")&&(!empty($VD_campaign))){
	$query_string = $_SERVER['QUERY_STRING'];
	//去掉opener=y
	if(strrpos($query_string,"opener=y")){
		$query_string = str_ireplace("&opener=y","",$query_string);
	}else{
		$query_string = str_ireplace("opener=y&","",$query_string);
	}
	...
	$screen_align = "Right";
	$screen_width = "100%";
	$stmt = "select ccms_agent_window_align,ccms_agent_window_width from ccms_campaigns where campaign_id = '$VD_campaign';";
	...
	function openWin(){
		if(navigator.userAgent.toLowerCase().indexOf("theworld")==-1){
			var i = screen.width;
			var width = i;
			var left = 0;
			var top = 0;
			...
			var screen_align = "screen_align_template";
			var screen_width = "screen_width_template";
			if(screen_width.indexOf("%")>0){
				width = i * parseInt(screen_width.substring(0,screen_width.length - 1)) /100;
			}else{
				width = parseInt(screen_width);
			}
			if(screen_align == "Left"){
				left = 0;
			}else if (screen_align == "Center"){
				left = (i - width)/2;
			}else{
				left = i - width;
			}
			
			window.open("vicidial.php?para_template","","height=" + height + ", width=" + width + ", top=" + top + ", left=" + left + ", toolbar=no, menubar=no, scrollbars=yes, resizable=no, location=no, status=no");
			opener = null;
			close();
		}else{
			window.location.href("vicidial.php?para_template");
		}
	}
	openWin();
	</script>
EOD;
	$opener_script = str_ireplace("para_template",$query_string,$opener_script);
	$opener_script = str_ireplace("screen_align_template",$screen_align,$opener_script);
	$opener_script = str_ireplace("screen_width_template",$screen_width,$opener_script);
	echo $opener_script;exit;
}
...
if ($qm_conf_ct > 0)
	...
	$stmt = "SELECT enable_vtiger_integration,vtiger_server_ip,vtiger_dbname,vtiger_login,vtiger_pass,vtiger_url FROM vicidial_campaigns where campaign_id='$VD_campaign';";
	$rslt=mysql_query($stmt, $link);
	if ($DB) {echo "$stmt\n";}
...
$HKuser_level			= '5';	# minimum CCMS user_level for HotKeys
...
$PreseT_DiaL_LinKs		= '0';	# set to 1 to show a W?HLEN link for Dial Presets
...
$hangup_all_non_reserved= '1';	# set to 1 to force hangup all non-reserved channels upon 
$LogouTKicKAlL			= '1';	# set to 1 to hangup all calls in session upon agent logout
$PhonESComPIP			= '2';	# set to 1 to log computer IP to phone if blank, set to 2 to force log each login
$DefaulTAlTDiaL			= '0';	# set to 1 to enable ALT W?HLEN by default if enabled for the campaign
...
$stretch_dimensions		= '1';	# sets the CCMS screen to the size of the browser window
...
if (ereg("443",$server_port)) {$HTTPprotocol = 'https://';}
...
if ($campaign_login_list > 0)
	...
	if ($relogin == 'YES')
		{
		$stmt="SELECT user_group from vicidial_users where binary user='$VD_login' and binary pass='$VD_pass';";
	...
	while ($camps_to_print > $o) 
		...
		if ($VD_campaign)
			{
			if ( (ereg("$VD_campaign",$rowx[0])) and (strlen($VD_campaign) == strlen($rowx[0])) )
				{$camp_form_code .= "<option value=\"$rowx[0]\" selected>$rowx[0]$campname</option>\n";}
			else
				{
				if (!eregi('login_allowable_campaigns',$camp_form_code))
...
if ($LogiNAJAX > 0)
	...
	function startCall(callnum){			
			document.getElementById('MDPhonENumbeR').value = callnum;
			NeWManuaLDiaLCalLSubmiT("NEW");
	}
	
	
	
	function fillNumber(callnum){
	...
	function pause(){
	    check_max_pause_count();
	}  
	
	function logout(){
	    NormalLogout();
	} 	  
	function login(loginusr, loginpass){
	    $url_ws = "vicidial.php?VD_campaign=edu&VD_login="+loginusr+"&VD_pass="+loginpass+"&phone_login="+loginusr+"&phone_pass="+loginpass;
	    alert($url_ws);
	    location.href=$url_ws; 	
	}
	
	function getNumber(){
	
		var $num= '<?php   echo  getInCallNumber($link, $VD_login); ?>';
	...
	/*	
		var datas="ACTION=GetInCallNumber&user="+user+"&pass="+pass+"&server_ip=" + server_ip + "&session_name=" + session_name ;
		...
		$.ajax({
     	...
     	success: function (data) {
     		$("#vtiger_inbound_number").val(data);
      }
    });
    
    return $("#vtiger_inbound_number").val();
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						document.getElementById("LogiNReseT").innerHTML = "<INPUT TYPE=BUTTON VALUE=\"<?php echo $lang_campaign_refresh; ?>Campaign\" OnClick=\"login_allowable_campaigns()\">";
				...
				if(name!=''){
					...
					xmlhttp.onreadystatechange = function() 
						...
						if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
							...
							Nactiveext = xmlhttp.responseText;
							if(Nactiveext =="false"){
								alert("<?php echo $lang_login_check_ring_mode; ?>");
								return false;
							}
							var Nactiveext_temp = "";
							if(status ==2){
...
//echo "<!-- ILPV -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><!--<a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_cn/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">简体中文 <img src=\"../agc/images/cn.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a>--></TD>\n";//echo "</TR></TABLE>\n";

echo "<div align=center><img src=../agc/images/vicidial_logo.gif /></div>";
echo "<table align=center border=0 cellpadding=0 cellspacing=0>\n";
echo "      <tbody><tr>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=1 width=15></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=1 width=77></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=1 width=19></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=1 width=68></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=1 width=19></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=1 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td><img name=dispo_r1_c1 src=../agc/images/dispo_r1_c1.jpg id=dispo_r1_c1 border=0 height=21 width=15></td>\n";
echo "        <td background=../agc/images/dispo_r1_c2.jpg>&nbsp;</td>\n";
echo "        <td background=../agc/images/dispo_r1_c3.jpg>&nbsp;</td>\n";
echo "        <td background=../agc/images/dispo_r1_c4.jpg>&nbsp;</td>\n";
echo "        <td><img name=dispo_r1_c5 src=../agc/images/dispo_r1_c5.jpg id=dispo_r1_c5 border=0 height=21 width=19></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=21 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td rowspan=3 background=../agc/images/dispo_r3_c1.jpg>&nbsp;</td>\n";
echo "        <td colspan=3 rowspan=3 background=../agc/images/dispo_r2_c2.jpg valign=top>\n";


echo "<FORM NAME=vicidial_form ID=vicidial_form ACTION=\"$agcPAGE\" METHOD=POST onsubmit='login_check();return false;'>\n";
echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
echo "<INPUT TYPE=HIDDEN NAME=JS_browser_height VALUE=\"\">\n";
echo "<INPUT TYPE=HIDDEN NAME=JS_browser_width VALUE=\"\">\n";
echo "<CENTER><TABLE WIDTH=400 CELLPADDING=0 CELLSPACING=0 >";
echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
echo "<INPUT TYPE=HIDDEN NAME=user_inbound_mode ID=user_inbound_mode VALUE=\"\">\n";
echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";

if($phone_login!=""){
echo "<INPUT TYPE=hidden ID=phone_login NAME=phone_login VALUE=\"$phone_login\">\n";
}else{
echo "<TR><TD ALIGN=RIGHT>$lang_phone_number: </TD>";
echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT ID=phone_login NAME=phone_login SIZE=10 maxlength=20 VALUE=\"$phone_login\"></TD></TR>\n";
}

if($phone_pass!=""){
echo "<TD ALIGN=LEFT><INPUT TYPE=hidden NAME=phone_pass VALUE=\"$phone_pass\"></TD></TR>\n";
}else{
echo "<TR><TD ALIGN=RIGHT>$lang_phone_password:  </TD>";
echo "<TD ALIGN=LEFT><INPUT TYPE=PASSWORD NAME=phone_pass SIZE=10 maxlength=20 VALUE=\"$phone_pass\"></TD></TR>\n";
}

echo "<TR><TD ALIGN=RIGHT>$lang_account:  </TD>";
echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT ID=VD_login NAME=VD_login SIZE=10 maxlength=20 VALUE=\"$VD_login\"></TD></TR>\n";
echo "<TR><TD ALIGN=RIGHT>$lang_password:  </TD>";
...
echo "<TR><TD ALIGN=CENTER COLSPAN=2><INPUT TYPE=Submit NAME=$lang_submit VALUE=$lang_submit> &nbsp; \n";
echo "<span id=\"LogiNReseT\"><INPUT TYPE=BUTTON VALUE=\"$lang_refresh_campaign\" OnClick=\"login_allowable_campaigns()\"></span></TD></TR>\n";
echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1><BR></TD></TR>\n";
echo "</TABLE>\n";
echo "</FORM>\n\n";

echo "		</td>\n";
echo "        <td rowspan=3 background=../agc/images/dispo_r3_c5.jpg>&nbsp;</td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=75 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=36 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=75 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td><img name=dispo_r5_c1 src=../agc/images/dispo_r5_c1.jpg id=dispo_r5_c1 border=0 height=25 width=15></td>\n";
echo "        <td background=../agc/images/dispo_r5_c2.jpg>&nbsp;</td>\n";
echo "        <td background=../agc/images/dispo_r5_c3.jpg>&nbsp;</td>\n";
echo "        <td background=../agc/images/dispo_r5_c3.jpg>&nbsp;</td>\n";
echo "        <td><img name=dispo_r5_c5 src=../agc/images/dispo_r5_c5.jpg id=dispo_r5_c5 border=0 height=25 width=19></td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=25 width=1></td>\n";
echo "      </tr>\n";
echo "    </tbody></table>\n";
echo "</body>\n\n";
echo "</html>\n\n";
...
if ($user_login_first == 1)
{
	...
	echo "<!-- ILPV -->\n";
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><!--<a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_cn/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">简体中文 <img src=\"../agc/images/cn.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a>--></TD>\n";	echo "</TR></TABLE>\n";
	echo "<FORM  NAME=vicidial_form ID=vicidial_form ACTION=\"$agcPAGE\" METHOD=POST onsubmit='login_user_check(\"VD_login\",2);return false;'>\n";
	echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
	echo "<INPUT TYPE=HIDDEN NAME=JS_browser_height VALUE=\"\">\n";
	echo "<INPUT TYPE=HIDDEN NAME=JS_browser_width VALUE=\"\">\n";
	#echo "<INPUT TYPE=HIDDEN NAME=phone_login VALUE=\"$phone_login\">\n";
	#echo "<INPUT TYPE=HIDDEN NAME=phone_pass VALUE=\"$phone_pass\">\n";
	echo "<CENTER><BR><B>$lang_account</B><BR><BR>";
	echo "<TABLE WIDTH=460 CELLPADDING=0 CELLSPACING=0 BGCOLOR=\"$MAIN_COLOR\"><TR BGCOLOR=WHITE>";
	echo "<TD ALIGN=LEFT VALIGN=BOTTOM><IMG SRC=\"../agc/images/vicidial_logo.gif\" BORDER=0></TD>";
	echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Campaign登陆 </TD>";
	echo "</TR>\n";
	echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>$lang_account:  </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=VD_login ID=VD_login SIZE=10 maxlength=20 VALUE=\"$VD_login\"></TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>$lang_password:  </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=PASSWORD NAME=VD_pass SIZE=10 maxlength=20 VALUE=\"$VD_pass\"></TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>Campaign:  </TD>";
	echo "<TD ALIGN=LEFT><span id=\"LogiNCamPaigns\">$camp_form_code</span></TD></TR>\n";
	...
	else
	{
		...
		{
		$stmt="SELECT phone_login,phone_pass from vicidial_users where binary user='$VD_login' and binary pass='$VD_pass' and user_level > 0 and active='Y';";
		if ($DB) {echo "|$stmt|\n";}
		$rslt=mysql_query($stmt, $link);
...
echo "<FORM  NAME=vicidial_form ID=vicidial_form ACTION=\"$agcPAGE\" METHOD=POST onsubmit='login_user_check(\"phone_login\",3);return false;'>\n";
echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
echo "<TABLE WIDTH=400 CELLPADDING=0 CELLSPACING=0 ALIGN=CENTER>";
echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
echo "<TR><TD ALIGN=RIGHT>$lang_phone_number: </TD>";
echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=phone_login ID=phone_login SIZE=10 maxlength=20 VALUE=\"\"></TD></TR>\n";
echo "<TR><TD ALIGN=RIGHT>$lang_phone_password:  </TD>";
...
echo "		</td>\n";
echo "        <td rowspan=3 background=../agc/images/dispo_r3_c5.jpg>&nbsp;</td>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=15 width=1></td>\n";
echo "      </tr>\n";
echo "      <tr>\n";
echo "        <td><img src=../agc/images/spacer.gif border=0 height=36 width=1></td>\n";
echo "      </tr>\n";
...
$VDloginDISPLAY=0;
	...
	{
	$stmt="SELECT count(*) from vicidial_users where binary user='$VD_login' and binary pass='$VD_pass' and user_level > 0 and active='Y';";
	if ($DB) {echo "|$stmt|\n";}
	$rslt=mysql_query($stmt, $link);
...
//		$password=strtoupper($VD_pass);
		...
		$stmt="SELECT full_name,user_level,hotkeys_active,agent_choose_ingroups,scheduled_callbacks,agentonly_callbacks,agentcall_manual,vicidial_recording,vicidial_transfers,closer_default_blended,user_group,vicidial_recording_override,alter_custphone_override,alert_enabled,agent_shift_enforcement_override,shift_override_flag,allow_alerts,closer_campaigns,agent_choose_territories,custom_one,custom_two,custom_three,custom_four,custom_five,grab_calls_in_queue,view_agent_status from vicidial_users where binary user='$VD_login' and binary pass='$VD_pass'";
		$rslt=mysql_query($stmt, $link);
		...
		$grab_calls_in_queue=					$row[24];
		$VU_view_agent_status = 					$row[25];
		...
		if ( (eregi('Y',$forced_timeclock_login)) or ( (eregi('ADMIN_EXEMPT',$forced_timeclock_login)) and ($VU_user_level < 8) ) )
			{
			$last_agent_event='';
			$HHMM = date("Hi");
			...
			$stmt="SELECT event from vicidial_timeclock_log where binary user='$VD_login' and event_epoch >= '$EoD' order by timeclock_id desc limit 1;";
			$rslt=mysql_query($stmt, $link);
			...
			if ( (strlen($last_agent_event)<2) or (ereg('LOGOUT',$last_agent_event)) )
				{
				$VDloginDISPLAY=1;
				$VDdisplayMESSAGE = "您必须先登入TIMECLOCK<BR>";
				}
			}
		...
		if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "经理登入: <INPUT TYPE=TEXT NAME=\"MGR_login$loginDATE\" SIZE=10 maxlength=20><br>\n";
				$VDdisplayMESSAGE.= "经理密码: <INPUT TYPE=PASSWORD NAME=\"MGR_pass$loginDATE\" SIZE=10 maxlength=20><br>\n";
				$VDdisplayMESSAGE.= "<INPUT TYPE=Submit NAME=$lang_submit VALUE=$lang_submit></FORM>\n";
				}
			}
		...
		if ( (!ereg(" $VD_campaign ",$LOGallowed_campaigns)) and (!ereg("ALL-CAMPAIGNS",$LOGallowed_campaigns)) )
			{
			echo "<title>CCMS</title>\n";
			echo "</head>\n";
			echo "<BODY BGCOLOR=WHITE MARGINHEIGHT=0 MARGINWIDTH=0 onResize=\"browser_dimensions();\"  onLoad=\"browser_dimensions();\">\n";
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><!--<a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_cn/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">简体中文 <img src=\"../agc/images/cn.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a>--></TD>\n";			echo "</TR></TABLE>\n";
			...
			echo "密码: <INPUT TYPE=PASSWORD NAME=VD_pass SIZE=10 maxlength=20 VALUE=\"$VD_pass\"><br>\n";
			echo "Campaign: <span id=\"LogiNCamPaigns\">$camp_form_code</span><br>\n";
			echo "<INPUT TYPE=Submit NAME=$lang_submit VALUE=$lang_submit> &nbsp; \n";
		...
		$stmt="SELECT count(*) FROM vicidial_campaigns where binary campaign_id='$VD_campaign' and active='Y';";
		if ($DB) {echo "|$stmt|\n";}
		$rslt=mysql_query($stmt, $link);
		...
		if($CAMPactive>0)
			...
			$stmt="SELECT status,status_name FROM vicidial_statuses WHERE $selectableSQL status != 'NEW' and status_type = '1' order by status limit 50;";
			$rslt=mysql_query($stmt, $link);
			...
			$stmt="SELECT status,status_name FROM vicidial_campaign_statuses WHERE $selectableSQL status != 'NEW' and status_type = '1' and binary campaign_id='$VD_campaign' order by status limit 80;";
			$rslt=mysql_query($stmt, $link);
			...
			$VARNAstatuses='';
			$VARNAstatusnames='';
			##### grab the NAstatuses that can be used for dispositioning by an agent
			$stmt="SELECT status,status_name FROM vicidial_statuses WHERE $selectableSQL status != 'NEW' and status_type = '0' order by status limit 50;";
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01010',$VD_login,$server_ip,$session_name,$one_mysql_log);}
			if ($DB) {echo "$stmt\n";}
			$VD_NAstatuses_ct = mysql_num_rows($rslt);
			$i=0;
			while ($i < $VD_NAstatuses_ct)
				{
				$row=mysql_fetch_row($rslt);
				$NAstatuses[$i] =$row[0];
				$NAstatus_names[$i] =$row[1];
				$VARNAstatuses = "$VARNAstatuses'$NAstatuses[$i]',";
				$VARNAstatusnames = "$VARNAstatusnames'$NAstatus_names[$i]',";
				$i++;
				}

			##### grab the campaign-specific NAstatuses that can be used for dispositioning by an agent
			$stmt="SELECT status,status_name FROM vicidial_campaign_statuses WHERE $selectableSQL status != 'NEW' and status_type = '0' and binary campaign_id='$VD_campaign' order by status limit 80;";
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01011',$VD_login,$server_ip,$session_name,$one_mysql_log);}
			if ($DB) {echo "$stmt\n";}
			$VD_NAstatuses_camp = mysql_num_rows($rslt);
			$j=0;
			while ($j < $VD_NAstatuses_camp)
				{
				$row=mysql_fetch_row($rslt);
				$NAstatuses[$i] =$row[0];
				$NAstatus_names[$i] =$row[1];
			...
			$VD_NAstatuses_ct = ($VD_NAstatuses_ct+$VD_NAstatuses_camp);
			$VARNAstatuses = substr("$VARNAstatuses", 0, -1); 
			$VARNAstatusnames = substr("$VARNAstatusnames", 0, -1); 

			##### grab the campaign-specific HotKey statuses that can be used for dispositioning by an agent
			$stmt="SELECT hotkey,status,status_name FROM vicidial_campaign_hotkeys WHERE selectable='Y' and status != 'NEW' and binary campaign_id='$VD_campaign' order by hotkey limit 9;";
			$rslt=mysql_query($stmt, $link);
			...
			##### select more parameter inbound mode + fnatic 
			#$stmt="SELECT park_ext,park_file_name,web_form_address,allow_closers,auto_dial_level,dial_timeout,dial_prefix,campaign_cid,campaign_vdad_exten,campaign_rec_exten,campaign_recording,campaign_rec_filename,campaign_script,get_call_launch,am_message_exten,xferconf_a_dtmf,xferconf_a_number,xferconf_b_dtmf,xferconf_b_number,alt_number_dialing,scheduled_callbacks,wrapup_seconds,wrapup_message,closer_campaigns,use_internal_dnc,allcalls_delay,omit_phone_code,agent_pause_codes_active,no_hopper_leads_logins,campaign_allow_inbound,manual_dial_list_id,default_xfer_group,xfer_groups,disable_alter_custphone,display_queue_count,manual_dial_filter,agent_clipboard_copy,use_campaign_dnc,three_way_call_cid,dial_method,three_way_dial_prefix,web_form_target,vtiger_screen_login,agent_allow_group_alias,default_group_alias,quick_transfer_button,prepopulate_transfer_preset,view_calls_in_queue,view_calls_in_queue_launch,call_requeue_button,pause_after_each_call,no_hopper_dialing,agent_dial_owner_only,agent_display_dialable_leads,web_form_address_two,agent_select_territories,crm_popup_login,crm_login_address,timer_action,timer_action_message,timer_action_seconds,start_call_url,dispo_call_url,xferconf_c_number,xferconf_d_number,xferconf_e_number FROM vicidial_campaigns where binary campaign_id = '$VD_campaign';";
			$stmt="SELECT park_ext,park_file_name,web_form_address,allow_closers,auto_dial_level,dial_timeout,dial_prefix,campaign_cid,campaign_vdad_exten,campaign_rec_exten,campaign_recording,campaign_rec_filename,campaign_script,get_call_launch,am_message_exten,xferconf_a_dtmf,xferconf_a_number,xferconf_b_dtmf,xferconf_b_number,alt_number_dialing,scheduled_callbacks,wrapup_seconds,wrapup_message,closer_campaigns,use_internal_dnc,allcalls_delay,omit_phone_code,agent_pause_codes_active,no_hopper_leads_logins,campaign_allow_inbound,manual_dial_list_id,default_xfer_group,xfer_groups,disable_alter_custphone,display_queue_count,manual_dial_filter,agent_clipboard_copy,use_campaign_dnc,three_way_call_cid,dial_method,three_way_dial_prefix,web_form_target,vtiger_screen_login,agent_allow_group_alias,default_group_alias,quick_transfer_button,prepopulate_transfer_preset,view_calls_in_queue,view_calls_in_queue_launch,call_requeue_button,pause_after_each_call,no_hopper_dialing,agent_dial_owner_only,agent_display_dialable_leads,web_form_address_two,agent_select_territories,crm_popup_login,crm_login_address,timer_action,timer_action_message,timer_action_seconds,start_call_url,dispo_call_url,xferconf_c_number,xferconf_d_number,xferconf_e_number,inbound_mode,xfer_a_name,xfer_b_name,xfer_c_name,xfer_d_name,xfer_e_name,crm_target,acw_hold_time FROM vicidial_campaigns where binary campaign_id = '$VD_campaign';";
			$rslt=mysql_query($stmt, $link);
			...
			$inbound_mode =				$row[66]; # inbound mode value + fnatic

			$inbound_mode = $user_inbound_mode;
			...
			$acw_hold_time	 			 =	$row[73];

			//modify by bear begin
			$stmt = "SELECT manual_Ring_Launch,custom_Dispo,custom_Dispo_Script,webForm_Button_Display,Default_Pause_Code_Enable,agent_First_Login_Time,Conference_Channel_Display,Xfer_Blind_Display,Xfer_Local_Closer_Display,Xfer_Dial_With_Customer_Display,parked_Channel_Value,im_Enable,phone_place_enable,im_Talk_Level,im_Admin_Level,Lead_Preview_Display,Dial_Next_Display,Xfer_Answer_Machine_Message_Display,fast_Hangup_Xferline_And_Grab_Custline,Xfer_Target_Unavailable_Remind_Enable,ingroup_Change_Enable,Skip_Choose_Ingroup_Enable,Incoming_Web_Play_Music_Enable,Incoming_Web_Play_Music_Filename,Xfer_Waiting_Web_Play_Music_Enable,Xfer_Waiting_Web_Play_Music_Filename,Customer_Hangup_Goto_Dispo_Enable,Pause_Code_Selected_Link_Display,Default_Pause_Code,ccms_project,default_call_result,auto_submit_dispo,auto_dispo_time,agent_hangup_confirm,agent_available_reset,agent_available_reset_codde,dtmf_enable from ccms_campaigns where campaign_id = '$VD_campaign'";
			$rslt=mysql_query($stmt, $link);
			$row=mysql_fetch_row($rslt);
			...
			if(!empty($row[28]) && $agent_pause_codes_active=='FORCE' ){
				$Default_Pause_Code = $row[28];
			}
			$ccms_project = $row[29];
			$default_call_result = $row[30];
			$auto_submit_dispo = $row[31];
			$auto_dispo_time = $row[32];
			$agent_hangup_confirm = $row[33];
			$agent_available_reset = $row[34];
			$agent_available_reset_codde = $row[35];
			$dtmf_enable = $row[36];
			require($Custom_Dispo_Script);
			//modify by bear begin
			...
			$stmt = "select group_web_vars from vicidial_campaign_agents where binary campaign_id='$VD_campaign' and binary user='$VD_login';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01056',$VD_login,$server_ip,$session_name,$one_mysql_log);}
			...
			if ( (strlen(trim($VU_vicidial_recording_override))!=0 && !ereg('DISABLED',$VU_vicidial_recording_override)) and ($VU_vicidial_recording > 0) )
				{
			...
			if ( (ereg('Y',$agent_pause_codes_active)) or (ereg('FORCE',$agent_pause_codes_active)) )
				...
				$stmt="SELECT pause_code,pause_code_name FROM vicidial_pause_codes WHERE binary campaign_id='$VD_campaign' order by pause_code limit 50;";
				$rslt=mysql_query($stmt, $link);
			...
			if ($agent_select_territories > 0)
				{
				$stmt="SELECT territory from vicidial_user_territories where binary user='$VD_login';";
				$rslt=mysql_query($stmt, $link);
			...
			$stmt="SELECT count(*) FROM vicidial_hopper where binary campaign_id = '$VD_campaign' and status='READY';";
			$rslt=mysql_query($stmt, $link);
			...
			{
			$VDloginDISPLAY=1;
			$VDdisplayMESSAGE = "Campaign未启用，请再试一次<BR>";
			}
		}
	else
		{
		...
		$VDloginDISPLAY=1;
		$VDdisplayMESSAGE = "话务员帐号或密码错误！<BR>";
		}
	}
	if ($VDloginDISPLAY)
	{
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><!--<a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_cn/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">简体中文 <img src=\"../agc/images/cn.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a>--></TD>\n";	//echo "</TR></TABLE>\n";
	...
	echo "<INPUT TYPE=HIDDEN NAME=phone_pass VALUE=\"$phone_pass\">\n";
	echo "<INPUT TYPE=HIDDEN NAME=user_inbound_mode ID=user_inbound_mode VALUE=\"\">\n";
	echo "<CENTER><B>$VDdisplayMESSAGE</B>";
	echo "<TABLE WIDTH=400 CELLPADDING=0 CELLSPACING=0 ><TR>";
	echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
	//echo "<TD ALIGN=CENTER VALIGN=MIDDLE> Campaign登陆 </TD>";
	echo "</TR>\n";
	echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1> &nbsp; </TD></TR>\n";
...
#   login: ca101,cb101,cc101
	$alias_found=0;
$stmt="select count(*) from phones_alias where binary alias_id = '$phone_login';";
$rslt=mysql_query($stmt, $link);
...
if ($alias_found > 0)
	{
	$stmt="select alias_name,logins_list from phones_alias where binary alias_id = '$phone_login' limit 1;";
	$rslt=mysql_query($stmt, $link);
...
$pa=0;
if ( (ereg(',',$phone_login)) and (strlen($phone_login) > 2) )
	{
	$phoneSQL = "(";
	...
	while($pa < $phones_auto_ct)
		...
		$phoneSQL .= "(binary login='$phones_auto[$desc]' and binary pass='$phone_pass')";
		$pa++;
		}
	$phoneSQL .= ")";
	}
else {$phoneSQL = "binary login='$phone_login' and binary pass='$phone_pass'";}
...
echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  NOWRAP><!--<a href=\"../agc_en/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">English <img src=\"../agc/images/en.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a></TD>\n";echo "<TD WIDTH=100 ALIGN=RIGHT VALIGN=TOP  BGCOLOR=\"#CCFFCC\" NOWRAP><a href=\"../agc_cn/vicidial.php?relogin=YES&VD_login=$VD_login&VD_campaign=$VD_campaign&phone_login=$phone_login&phone_pass=$phone_pass&VD_pass=$VD_pass\">简体中文 <img src=\"../agc/images/cn.gif\" BORDER=0 HEIGHT=14 WIDTH=20></a>--></TD>\n";	echo "</TR></TABLE>\n";
	...
	echo "</TR>\n";
	echo "<TR><TD ALIGN=CENTER COLSPAN=2><font size=1> &nbsp; <BR><FONT SIZE=3>抱歉，分机所输入的账号密码于系统中尚未启用，请再试一次: <BR> &nbsp;</TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>$lang_phone_number: </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=TEXT NAME=phone_login SIZE=10 maxlength=20 VALUE=\"$phone_login\"></TD></TR>\n";
	echo "<TR><TD ALIGN=RIGHT>$lang_phone_password:  </TD>";
	echo "<TD ALIGN=LEFT><INPUT TYPE=PASSWORD NAME=phone_pass SIZE=10 maxlength=20 VALUE=\"$phone_pass\"></TD></TR>\n";
	echo "<TR><TD ALIGN=CENTER COLSPAN=2><INPUT TYPE=Submit NAME=$lang_submit VALUE=$lang_submit></TD></TR>\n";
	echo "<TR><TD ALIGN=LEFT COLSPAN=2><font size=1><BR></TD></TR>\n";
	echo "</TABLE>\n";
	echo "</FORM>\n\n";
	echo "</body>\n\n";
...
### logged in and use that phone login account
	if ($pa > 0)
		...
		while($pb < $phones_auto_ct)
			...
			$stmtx="SELECT server_ip from phones where binary login = '$phones_auto[$pb]';";
			if ($DB) {echo "|$stmtx|\n";}
			if ($non_latin > 0) {$rslt=mysql_query("SET NAMES 'UTF8'");}
		...
		echo "<!-- 电话s balance selection: $phone_login|$pb_server_ip|$past_minutes_date|     |$pb_log -->\n";
		}
	echo "<title>CCMS</title>\n";
	$stmt="SELECT extension,dialplan_number,voicemail_id,phone_ip,computer_ip,server_ip,login,pass,status,active,phone_type,fullname,company,picture,messages,old_messages,protocol,local_gmt,ASTmgrUSERNAME,ASTmgrSECRET,login_user,login_pass,login_campaign,park_on_extension,conf_on_extension,VICIDIAL_park_on_extension,VICIDIAL_park_on_filename,monitor_prefix,recording_exten,voicemail_exten,voicemail_dump_exten,ext_context,dtmf_send_extension,call_out_number_group,client_browser,install_directory,local_web_callerID_URL,VICIDIAL_web_URL,AGI_call_logging_enabled,user_switching_enabled,conferencing_enabled,admin_hangup_enabled,admin_hijack_enabled,admin_monitor_enabled,call_parking_enabled,updater_check_enabled,AFLogging_enabled,QUEUE_ACTION_enabled,CallerID_popup_enabled,voicemail_button_enabled,enable_fast_refresh,fast_refresh_rate,enable_persistant_mysql,auto_dial_next_number,VDstop_rec_after_each_call,DBX_server,DBX_database,DBX_user,DBX_pass,DBX_port,DBY_server,DBY_database,DBY_user,DBY_pass,DBY_port,outbound_cid,enable_sipsak_messages,email,template_id,conf_override,phone_context,phone_ring_timeout,conf_secret from phones where binary login='$phone_login' and binary pass='$phone_pass' and active = 'Y';";
	if ($DB) {echo "|$stmt|\n";}
	$rslt=mysql_query($stmt, $link);
	...
	// + fantic
	elseif ($dial_method=='INBOUND_MAN' && $inbound_mode=='ring')
		{
		$no_empty_session_warnings=1;
		}
	// End 
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		echo "<!-- Campaign CUSTOM 停泊ING:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
	echo "<!-- Campaign DEFAULT 停泊ING: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if (strlen($web_form_address)>0)
		...
		echo "<!-- Campaign 客制表格:   |$VICIDiaL_web_form_address| -->\n";
		}
	else
		{
		$VICIDiaL_web_form_address = "$VICIDiaL_web_URL";
		print "<!-- Campaign 预设表格:  |$VICIDiaL_web_form_address| -->\n";
	...
	if (strlen($web_form_address_two)>0)
		...
		echo "<!-- Campaign 客制表格 2:   |$VICIDiaL_web_form_address_two| -->\n";
		}
	else
		{
		$VICIDiaL_web_form_address_two = "$VICIDiaL_web_form_address";
		echo "<!-- Campaign 预设表格 2:  |$VICIDiaL_web_form_address_two| -->\n";
	...
	if ($allow_closers=="Y")
		...
		echo "<!-- Campaign 允许关闭:    |$VICIDiaL_allow_closers| -->\n";
		}
	else
		{
		$VICIDiaL_allow_closers = 0;
		echo "<!-- Campaign 不允许关闭: |$VICIDiaL_allow_closers| -->\n";
		}
	...
	if ( ( ($campaign_allow_inbound == 'Y') and ($dial_method != 'MANUAL') ) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		if ($prev_login_ct > 0)
			{echo "<!-- 使用先前的会议室 - $session_id - $NOW_TIME - $SIP_user -->\n";}
		else
			{
			...
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01034',$VD_login,$server_ip,$session_name,$one_mysql_log);}
				...
				$stmt="SELECT conf_exten from vicidial_conferences where server_ip='$server_ip' and ( (extension='$SIP_user') or (binary extension='$VD_login') );";
					if ($format=='debug') {echo "\n<!-- $stmt -->";}
				$rslt=mysql_query($stmt, $link);
			...
			echo "<!-- 使用新的会议室 - $session_id - $NOW_TIME - $SIP_user -->\n";
			}
		...
		echo "<!-- 旧的队列和INCALL回复列表:   |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- 旧的队列和INCALL回复 Hopper: |$affected_rows| -->\n";
	...
	#	echo "<B>You have logged in as user: $VD_login on phone: $SIP_user 登陆 campaign: $VD_campaign</B><BR>\n";
		...
		if ( ($enable_sipsak_messages > 0) and ($allow_sipsak_messages > 0) and (ereg("SIP",$protocol)) )
			{
			$SIPSAK_prefix = 'LIN-';
		...
		if ($inbound_mode!="ring") # inbound mode + fnatic
			{
			...
			echo "<!-- call placed to session_id: $session_id from phone: $SIP_user $SIP_user_DiaL -->\n";		
				...
				$stmt="delete from ccms_live_phones where user='$VD_login' or phone_ext='$SIP_user'";
				if ($DB) {echo "$stmt\n";}
				$rslt=mysql_query($stmt, $link);
			}
		...
		$stmt="SELECT campaign_weight,calls_today FROM vicidial_campaign_agents where binary user='$VD_login' and binary campaign_id = '$VD_campaign';";
		$rslt=mysql_query($stmt, $link);
		...
		if ($auto_dial_level > 0)
			{
			echo "<!-- Campaign已设定为自动拨号: $auto_dial_level -->\n";
      
      

			$closer_chooser_string='';
			$stmt="INSERT INTO vicidial_live_agents (user,server_ip,conf_exten,extension,status,lead_id,campaign_id,uniqueid,callerid,channel,random_id,last_call_time,last_update_time,last_call_finish,closer_campaigns,user_level,campaign_weight,calls_today,last_state_change,outbound_autodial,manager_ingroup_set,inbound_mode) values('$VD_login','$server_ip','$session_id','$SIP_user','PAUSED','','$VD_campaign','','','','$random','$NOW_TIME','$tsNOW_TIME','$NOW_TIME','$closer_chooser_string','$user_level','$campaign_weight','$calls_today','$NOW_TIME','Y','N','$user_inbound_mode');";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01044',$VD_login,$server_ip,$session_name,$one_mysql_log);}
			$affected_rows = mysql_affected_rows($link);
			echo "<!-- 录音档已被新增: |$affected_rows| -->\n";
			
			//start  add by pie 20130311 for Agent use 'remote agent's exten' login CCMS;
			$stmt="SELECT user_start, server_ip, conf_exten, status, campaign_id FROM vicidial_remote_agents WHERE conf_exten = '$phone_login';";
			if ($DB) {echo "$stmt\n";}
			$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01044',$phone_login,$server_ip,$session_name,$one_mysql_log);}
			$remote_record_ct = mysql_num_rows($rslt);
			$session_RemotePhone = "";
			if ($remote_record_ct > 0)
			{
				$row=mysql_fetch_row($rslt);
				$remote_exten		=	$row[2];
				$remote_status		=	$row[3];
				...
				{
					$stmt="UPDATE vicidial_remote_agents set status = 'INACTIVE' WHERE conf_exten = '$phone_login';";
					if ($DB) {echo "$stmt\n";}
					$rslt=mysql_query($stmt, $link);
					if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'01044',$phone_login,$server_ip,$session_name,$one_mysql_log);}
					$affected_rows = mysql_affected_rows($link);
					if($affected_rows>0)
					{
						//session_register("remote_extension");//注册remote_extension 变量；
						//$_SESSION['remote_extension'] = $phone_login;//被session变量赋值；
						//$session_RemotePhone = $_SESSION['remote_extension'];
						$session_RemotePhone = $phone_login;
						echo "<!-- remote_extension: |".$session_RemotePhone."| -->\n";
						$stmt="DELETE from vicidial_live_agents WHERE user = '$phone_login';";
						if ($DB) {echo "$stmt\n";}
						$rslt=mysql_query($stmt, $link);
						if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'010440',$phone_login,$server_ip,$session_name,$one_mysql_log);}
					}
				}
			}
			//end add; 
			...
			{
			print "<!-- Campaign已设定为人工拨号: $auto_dial_level -->\n";
...
$CAwidth =  "100%";	# Mod by Fnatic
//$SBwidth =	($MASTERwidth + 331);	# 761 - SideBar starting point
$SBwidth =	($MASTERwidth + 351);	# Mod by Fnatic -300
//$MNwidth =  ($MASTERwidth + 330);	# 760 - main frame
$MNwidth =  "100%";	# Mod by Fnatic
...
$WRheight =  ($MASTERheight + 240);	# Mod by fnatic +70
//$CQheight =  ($MASTERheight + 140);	# 440 - Calls in queue section
$CQheight =  ($MASTERheight + 185);	# Mod by fnatic +50
//$SLheight =  ($MASTERheight + 122);	# 422 - SideBar link, Calls in queue link
$SLheight =  ($MASTERheight + 215);	# Mod by fnatic +93
//$HKheight =  ($MASTERheight + 105);	# 405 - HotKey active Button
$HKheight =  ($MASTERheight + 155);	# Mod by fnatic +50
...
##### Add by fnatic start #####
##### grab the agent frist login time ######
$stmt="select event_time from vicidial_agent_log where event_time>curdate() and binary user='$VD_login'  and sub_status='LOGIN' order by event_time asc limit 1";
$rslt=mysql_query($stmt, $link);
if ($DB) {echo "$stmt\n";}
$row=mysql_fetch_row($rslt);
$Agent_First_Login_Time=$row[0];
##### Add by fnatic end #####
...
$_SESSION['VD_campaign'] = $VD_campaign;
$_SESSION['extension'] = $extension;
$_SESSION['dialplan_number'] = $dialplan_number;
$_SESSION['ext_context'] = $ext_context;
$_SESSION['server_ip'] = $server_ip;
$_SESSION['session_id'] = $session_id;
$_SESSION['auto_dial_level'] = $auto_dial_level;
$_SESSION['three_way_dial_prefix'] = $three_way_dial_prefix;
$_SESSION['campaign_cid'] = $campaign_cid;
$_SESSION['campaign_vdad_exten'] = $campaign_vdad_exten;
$_SESSION['campaign_cid'] = $campaign_cid;
$_SESSION['campaign_rec_filename'] = $campaign_rec_filename;

session_register("username__");
?>
...
<!--Add by fnatic jquery1.4.2 end-->
    ...
    var Manual_Ring_Launch='<?php echo $Manual_Ring_Launch ?>';
	var Custom_Dispo='<?php echo $Custom_Dispo ?>';
    var Custom_Dispo_Script='<?php echo $Custom_Dispo_Script ?>';
	var WebForm_Button_Display='<?php echo $WebForm_Button_Display ?>';
	var Skip_Choose_Ingroup_Enable = '<?php echo $Skip_Choose_Ingroup_Enable ?>';
	var Customer_Hangup_Goto_Dispo_Enable = '<?php echo $Customer_Hangup_Goto_Dispo_Enable ?>';
	//var Customer_Hangup_Goto_Dispo_Enable = 'N';
	var Default_Pause_Code_Enable = '<?php echo $Default_Pause_Code_Enable ?>';
	var Default_Pause_Code = '<?php echo $Default_Pause_Code ?>';
	var Original_Pause_Code = '<?php echo $Default_Pause_Code ?>';
	var Conference_Channel_Display = '<?php echo $Conference_Channel_Display?>';
	var Pause_Code_Selected_Link_Display = '<?php echo $Pause_Code_Selected_Link_Display?>';
	var Xfer_Blind_Display='<?php echo $Xfer_Blind_Display?>';
	var Xfer_Local_Closer_Display='<?php echo $Xfer_Local_Closer_Display?>';
	var Xfer_Dial_With_Customer_Display='<?php echo $Xfer_Dial_With_Customer_Display;?>';
	var Cortorl_Pausecode_Insert_Db='Y';
	var Parked_Channel_Value='<?php echo $Parked_Channel_Value;?>';
	var Lead_Preview_Display='<?php echo $Lead_Preview_Display;?>';
	var Dial_Next_Display='<?php echo $Dial_Next_Display;?>';
	var Xfer_Answer_Machine_Message_Display='<?php echo $Xfer_Answer_Machine_Message_Display;?>';
	var Fast_Hangup_Xferline_And_Grab_Custline='<?php echo $Fast_Hangup_Xferline_And_Grab_Custline;?>';
	var Xfer_Target_Unavailable_Remind_Enable='<?php echo $Xfer_Target_Unavailable_Remind_Enable;?>';
	var Xfer_Target_Unavailable_Remind_Enable_Count = 0;
	var Incoming_Web_Play_Music_Enable = '<?php echo $Incoming_Web_Play_Music_Enable;?>';
	var Incoming_Web_Play_Music_Filename = '<?php echo $Incoming_Web_Play_Music_Filename;?>';
	var Xfer_Waiting_Web_Play_Music_Enable = '<?php echo $Xfer_Waiting_Web_Play_Music_Enable;?>';
	var Queue_Music_Alert_Count = 0;
	var crm_target='<?php echo $crm_target;?>';
	// campaign inbound mode is ring add new parameter + fnatic
	var inbound_mode='<?php echo $inbound_mode;?>'; 
	...
	VARNAstatuses = new Array(<?php echo $VARNAstatuses ?>);
	VARNAstatusnames = new Array(<?php echo $VARNAstatusnames ?>);
	var VD_NAstatuses_ct = '<?php echo $VD_NAstatuses_ct ?>';
...
?>
	...
	var auto_dispo_time = '<?php echo $auto_dispo_time ?>';
	var auto_dispo_time_count = 0;
	
	var acw_hold_time = '<?php echo $acw_hold_time;?>';
    ...
    //End
	var script_height = '<?php echo $SSheight+100 ?>';
	...
	var agent_hangup_confirm = '<?php echo $agent_hangup_confirm ?>';
	var timer_action='';
	...
	var DiaLControl_auto_HTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a>";
	var DiaLControl_auto_HTML_ready = "<a href=\"#\" onclick=\"check_max_pause_count();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"></a><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\">";
	var DiaLControl_auto_HTML_OFF = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\">";
	var DispoSelectBoxStatus = 0;
	var Ring_Uniqueid_old = "";
	//Mod by fnatic start
	if(Dial_Next_Display=='Y')
	   {
	var DiaLControl_manual_HTML = "<a style=\"display:none;\" href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif\" border=0 alt=\"<?php echo $lang_dial_next_number; ?>\"></a>";
	   }
	else
	  {
	...
	var image_livecall_OFF = new Image();
		image_livecall_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>agc_live_call_OFF_cn.gif";
	var image_livecall_ON = new Image();
		image_livecall_ON.src="../agc/images/<?php echo $lang_images_dir; ?>agc_live_call_ON_cn.gif";
	var image_livecall_DEAD = new Image();
		image_livecall_DEAD.src="../agc/images/<?php echo $lang_images_dir; ?>agc_live_call_DEAD_cn.gif";
	var image_LB_dialnextnumber = new Image();
		image_LB_dialnextnumber.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif";
	var image_LB_hangupcustomer = new Image();
		image_LB_hangupcustomer.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_hangupcustomer_cn.gif";
	var image_LB_transferconf = new Image();
		image_LB_transferconf.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_cn.gif";
	var image_LB_grabparkedcall = new Image();
		image_LB_grabparkedcall.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_grabparkedcall_cn.gif";
	var image_LB_parkcall = new Image();
		image_LB_parkcall.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_cn.gif";
	var image_LB_webform = new Image();
		image_LB_webform.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_cn.gif";
	...
	var image_LB_pause = new Image();
		image_LB_pause.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_cn.gif";
	var image_LB_resume = new Image();
		image_LB_resume.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_cn.gif";
	...
	var image_LB_dialnextnumber_OFF = new Image();
		image_LB_dialnextnumber_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_OFF_cn.gif";
	var image_LB_hangupcustomer_OFF = new Image();
		image_LB_hangupcustomer_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_hangupcustomer_OFF_cn.gif";
	var image_LB_transferconf_OFF = new Image();
		image_LB_transferconf_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_OFF_cn.gif";
	...
	var image_LB_parkcall_OFF = new Image();
		image_LB_parkcall_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_OFF_cn.gif";
	var image_LB_webform_OFF = new Image();
		image_LB_webform_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_OFF_cn.gif";
	...
	var image_LB_pause_OFF = new Image();
		image_LB_pause_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif";
	var image_LB_resume_OFF = new Image();
		image_LB_resume_OFF.src="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif";
	...
	var status_type = 0;
	var agent_available_reset = '<?php echo $agent_available_reset ?>';
	var agent_available_reset_codde = '<?php echo $agent_available_reset_codde ?>';
	var agent_available_reset_count = 0;
	var agent_available_reset_check = 0;
	var redirectcalltoagentid = "";
	var session_RemotePhone = '<?php echo $session_RemotePhone ?>';
...
title:'<?php echo $lang_pause_reason; ?>',
height:'auto',
width:550,
modal:true
});
...
draggable:false,
resizable:false,
title: '<?php echo $lang_link_agent_status; ?>',
height:'auto',
width: 500,
modal: false
});
...
draggable:true,
resizable:false,
title: '<?php echo $lang_call_dispo; ?>',
height:'auto',
width: '750px',
modal: false,
closeOnEscape: false,
...
draggable:true,
resizable:false,
title: '<?php echo $lang_link_agent_callback_notice; ?>',
height:'auto',
width: 580,
modal: true,
closeOnEscape: false,
...
draggable:false,
resizable:false,
title:'<?php echo $lang_link_grap_queue ?>',
width:950,
height:'auto',
modal: true
...
draggable:false,
resizable:false,
title:'<?php echo $lang_transfer_call; ?>',
height:360,
width:390,
modal:false,
position:['center',200]
...
draggable:false,
resizable:false,
title:'<?php echo $lang_system_alert ;?>',
width:250,
height:140,
modal: true
});
	...
	function volume_control(taskdirection,taskvolchannel,taskagentmute) 
		...
		if (taskagentmute=='AgenT')
			...
			if (taskdirection=='MUTING')
				{
				document.getElementById("AgentMuteSpan").innerHTML = "<a href=\"#CHAN-" + agentchannel + "\" onclick=\"volume_control('UNMUTE','" + agentchannel + "','AgenT');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_volume_UNMUTE_cn.gif\" BORDER=0></a>";
				}
			else
				{
				document.getElementById("AgentMuteSpan").innerHTML = "<a href=\"#CHAN-" + agentchannel + "\" onclick=\"volume_control('MUTING','" + agentchannel + "','AgenT');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_volume_MUTE_cn.gif\" BORDER=0></a>";
				}
			}
...
// Send alert control command for agent
	function alert_control(taskalert) 
		...
		if (taskalert=='ON')
			{
			alert_enabled = 'ON';
			document.getElementById("AgentAlertSpan").innerHTML = "<a href=\"#\" onclick=\"alert_control('OFF');return false;\"><IMG SRC=\"../agc/images/cn/lgq_ss_off.gif\" border=0> </a>";
			}
		else
			{
			alert_enabled = 'OFF';
			document.getElementById("AgentAlertSpan").innerHTML = "<a href=\"#\" onclick=\"alert_control('ON');return false;\"><IMG SRC=\"../agc/images/cn/lgq_ss_on.gif\" border=0> </a>";
			}
	...
	function SendManualDial(taskFromConf)
		...
		try{
			...
			if(Xfer_Dial_With_Customer_Display=='Y')
				{
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_dialwithcustomer_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_dial_with_customer; ?>\" style=\"vertical-align:middle\"></a>";
                }
			//Mod by fnatic end
			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_parkcustomerdial_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_dial_del_cliente_del_parque; ?>\" style=\"vertical-align:middle\"></a>";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_blindtransfer_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_blind_transfer; ?>\" style=\"vertical-align:middle\">";
			document.getElementById("agentdirectlink").innerHTML = "<?php echo $lang_agent_link; ?>";
			//Mod by fnatic
	...
	function basic_originate_call(tasknum,taskprefix,taskreverse,taskdialvalue,tasknowait,taskconfxfer,taskcid,taskusegroupalias) 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupxferline_cn.gif\" border=0 alt=\"<?php echo $lang_hangup_xfer_line; ?>\"></a>";
						}
						xmlhttp = null;
						CollectGarbage();
					}
						...
						if ( (VLAStatuS == 'PAUSED') && (AutoDialWaiting == 1) )
							...
							if (PausENotifYCounTer > 10)
								...
								if(dial_method=='INBOUND_MAN' && inbound_mode=='ring')
									...
									//		{
											alert("派call失败，请检查分机<?php echo $SIP_user;?>是否处于摘机状态，你的会话被暂停！");
											AutoDial_ReSume_PauSe('VDADpause');
									...
									//		}
										...
										{
										alert('<?php echo $lang_session_stop; ?>');
										AutoDial_ReSume_PauSe('VDADpause');
										PausENotifYCounTer=0;
										}
									}
					...
					//	document.getElementById("debugbottomspan").innerHTML=check_ALL_array[1];
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<TABLE border=0 cellspacing=1 cellpadding=1 width=100%><TR BGCOLOR='#CCCCCC'><TD>#</TD><TD><?php echo $lang_remote_channel; ?></TD><TD><?php echo $lang_hangup; ?></TD><TD><?php echo $lang_volume; ?></TD></TR>";
	...
	//	conf_dialed=1;
		...
		if (xmlhttpXF) 
			...
			if (taskvar == 'ParK')
				...
				document.getElementById("ParkControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_keep_phone; ?>\">";
				
				//当按了"保持电话"时，转接按钮不可用
				if (VU_vicidial_transfers == '1'){
					document.getElementById("XferControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_transfer; ?>\">";
				}
			...
			if (taskvar == 'FROMParK')
				...
				if(AutoDialWaiting == 2){
					alert("<?php echo $lang_alert_grabed_number_calling; ?>");
					return;
				}
				blind_transfer=0;
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_cn.gif\" border=0 alt=\"<?php echo $lang_keep_phone; ?>\"></a>";
				//转接按钮可以用
				if (VU_vicidial_transfers == '1'){
					document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('ON');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_cn.gif\" border=0 alt=\"<?php echo $lang_transfer; ?> - <?php echo $lang_conference_call; ?>\"></a>";
				}
				customerparked=0;
				}
...
// Finish the alternate dialing and move on to disposition the call
	function ManualDialAltDonE()
		...
		open_dispo_screen=1;
		document.getElementById("MainStatuSSpan").innerHTML = "<font style=\"font-family:'宋体';font-size:12px;\"><?php echo $lang_dial_next_number; ?></font>";
		}
// ################################################################################
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (CBcounT == 0) {var CBprint = "";}
						...
						document.getElementById("CBstatusSpan").innerHTML ="<a href=\"#\" onclick=\"CalLBacKsLisTCheck();return false;\"><IMG SRC=\"../agc/images/cn/lgq_ss_callback.gif\" BORDER=0></a>";
...
// Request list of USERONLY callbacks for this agent
	function CalLBacKsLisTCheck()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			{
			alert("<?php echo $lang_alert_pause_to_read_callback; ?>");
			}
		else
			{
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=100% border=0 cellspacing=1 cellpadding=1><tr bgcolor='#CCCCCC'><td>#</td><td><?php echo $lang_callback_time; ?></td><td><?php echo $lang_tel_number; ?></td><td><?php echo $lang_agent_status; ?></td><td>CAMPAIGN</td><td><?php echo $lang_last_talk_time; ?></td><td><?php echo $lang_remark; ?></td></tr>"
						while (loop_ct < CB_calls)
							...
							if (CB_status == 'LIVE')
								{
									CB_status = "<?php echo $lang_has_passed_callback_time; ?>";
								}
							else if(CB_status == 'ACTIVE')
								{
									CB_status = "<?php echo $lang_not_over_callback_time; ?>";
								}
...
// Add by Fnatic 用于向vtiger_search.php传递号码去查询vtigerCRM该号码的详细资料 
	...
	//	alert("111");
		if(DispoSelectBoxStatus == 1){
            alert("<?php echo $lang_alert_submit_tel_dispo; ?>");
            return false;
        } 
    // 	alert("222");   
	//	alt_phone_dialing=1;
	...
	//	document.vicidial_form.DiaLAltPhonE.checked=true;
		...
		//	alert("444");   
	//		var sss = "<?php echo $web_form_address; ?>?iocheck=in&user="+user+"&campaign="+campaign+"&phone="+taskPhoneNumber+"\"";
		//	alert(sss);

			//document.getElementById("ScriptContents").innerHTML = "<iframe onload=\"SetCwinHeight(this)\"  src=\"<?php echo $web_form_address; ?>&iocheck=in&user="+user+"&campaign="+campaign+"&phone="+taskPhoneNumber+"\" style=\"background-color:transparent;\" scrolling=\"auto\" frameborder=\"0\" allowtransparency=\"true\" id=\"popupFrame\" name=\"popupFrame\" width=\"99%\" height=\"" + script_height + "\" STYLE=\"z-index:17\"></iframe>";
...
// Open page to enter details for a new manual dial lead
	function NeWManuaLDiaLCalL(TVfast)
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			{
			alert("<?php echo $lang_alert_pause_before_dial; ?>");
			}
		else
			{
		*/
			if (TVfast=='FAST')
				...
				({ draggable: false,
			       ...
			       height:'auto',
			       modal: true,
			       title:"<?php echo $lang_link_manual_dial; ?>"
		        });
...
// Insert the new manual dial as a lead and go to manual dial screen
	function NeWManuaLDiaLCalLSubmiT(tempDiaLnow)
	{
		...
		if(status_temp.indexOf("vdc_LB_hangupcustomer_cn.gif")>0){
			alert("<?php echo $lang_alert_please_hangup_first; ?>");
			return false;
		}
		...
		{
		if(confirm("<?php echo 目前为可用状态，请在暂停状态下进行外拨，点击确定切换到暂停状态并外拨，如不需要点击取消; ?>"))
		{
...
// Fast version of manual dial
		...
		if ( (MDDiaLCodEform.length < 1) || (MDPhonENumbeRform.length < 5) )
			{
			alert("<?php echo $lang_alert_quick_dial; ?>");
			}
		else
			{
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					//		alert(document.getElementById("MainStatuSSpan").innerHTML);
							...
							document.getElementById("MainStatuSSpan").innerHTML = "<font style=\"font-family:'宋体';font-size:12px;\"><?php echo $lang_dialing; ?>: " + status_display_number + " UID: " + MDnextCID + " &nbsp; <?php echo $lang_idle_state; ?>..." + "</font>";
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
                                ...
                                document.getElementById("MainStatuSSpan").innerHTML = " <font style=\"font-family:'宋体';font-size:12px;\"><?php echo $lang_dial_another; ?>: " + document.getElementById("xfernumber").value + " UID: " + CIDcheck + "</font>";


								if(Xfer_Dial_With_Customer_Display=='Y')
									{
								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_dialwithcustomer_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_dial_with_customer; ?>\" style=\"vertical-align:middle\">";
									}
                                //Mod by fnatic end
								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_parkcustomerdial_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_dial_del_cliente_del_parque; ?>\" style=\"vertical-align:middle\">";
							...
							if (MDalert == 'ERROR')
                                ...
                                var DiaLAlerTMessagE = "<center><br/><br/><b><?php echo $lang_customer_cannot_call; ?></b></center>";
								//Mod by fnatic end
								TimerActionRun("DiaLAlerT",DiaLAlerTMessagE);
								}
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								//Modified by Kelvin End
								document.getElementById("status_code").innerHTML="<?php echo $lang_talking; ?>";
                               ...
                               // alert(XD_live_customer_call);  answered call 
							    ...
							    document.getElementById("MainStatuSSpan").innerHTML = "<font style=\"font-family:'宋体';font-size:12px;\"><?php echo $lang_have_dialed; ?>: " + status_display_number + " UID: " + CIDcheck + " </font>";

								if(CheckOutboundChannelLine2 == 0){
									document.getElementById("out_line12").innerHTML = "<input type='text' size=\"15\" maxlength=\"20\" name=\"tel_out_line2\" id=\"tel_out_line2\" value=\"\" ><input type='button' onClick=\"outbound_dial_line2();return false;\" value=\"拨号\" />";
								}
								
								document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_cn.gif\" border=0 alt=\"<?php echo $lang_keep_phone; ?>\"></a>";
								document.getElementById("HangupControl").innerHTML = "<a href=\"#\" onclick=\"dialedcall_send_hangup();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_hangupcustomer_cn.gif\" border=0 alt=\"<?php echo $lang_hangup; ?>\"></a>";
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (UDresponse_status == 'GOOD')
						...
						document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormRefresH();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?>\"></a>\n";
						if (enable_second_webform > 0)
							{
							document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_two_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?>2\"></a>\n";
							}
						}
                        //Modified by Kelvin End
...
// Send the Manual 拨下一个号码 request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
	{
		...
		if (dial_method == "INBOUND_MAN")
		{
			...
			if(Dial_Next_Display=='Y')
			{				   
				document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_dial_next_number; ?>\">";
			}
			else
			{		
				document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\">";			
			}
			//Mod by fnaitc end
		...
		else
		{
		    ...
		    if(Dial_Next_Display=='Y')	
				{
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_dial_next_number; ?>\">";
				}
			else
				{
			document.getElementById("DiaLControl").innerHTML = "";
		...
		// Modify By Fnatic end
			...
			var man_preview = 'NO';
			var man_status = "<?php echo $lang_idle_state; ?>..."; 
			}
			...
			//		alert(xmlhttp.responseText);
					...
					if ( (MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) )
						...
						if (MDnextCID.match(regMNCvar))
							{alert("<?php echo $lang_alert_no_leads_in_campaign; ?>\n" + campaign + "!");   alert_displayed=1;}
						if (MDnextCID.match(regMDFvarDNC))
							{alert("<?php echo $lang_alert_tel_in_dnc; ?>\n" + mdnPhonENumbeR);   alert_displayed=1;}
						if (MDnextCID.match(regMDFvarCAMP))
							{alert("<?php echo $lang_alert_tel_not_in_campaign; ?>\n" + mdnPhonENumbeR);   alert_displayed=1;}
						...
						if (starting_dial_level == 0)
							...
							if(Dial_Next_Display=='Y')
								{											
							document.getElementById("DiaLControl").innerHTML = "<a style=\"display:none;\" href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif\" border=0 alt=\"<?php echo $lang_dial_next_number; ?>\"></a>";
							   }
							else
								{
							document.getElementById("DiaLControl").innerHTML = "";							
							...
							if (dial_method == "INBOUND_MAN")
                                ...
                                if(Dial_Next_Display=='Y')
									{
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif\" border=0 alt=\"<?php echo $lang_dial_next_number; ?>\"></a>";
								    }
							    else
									{
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a>";					
								    }
								//Mod by fnatic end
					...
					//	document.getElementById("MainStatuSSpan").innerHTML = "<font style=\"font-family:'宋体';font-size:12px;\">拨号中: " + status_display_number + " UID: " + MDnextCID + " &nbsp; " + man_status + "</font>";
						document.getElementById("MainStatuSSpan").innerHTML = "<font style=\"font-family:'宋体';font-size:12px;\"><?php echo $lang_dialing; ?>: " + status_display_number + " UID: " + MDnextCID + " &nbsp; " + man_status + "</font>";
						...
						if (LeaDPreVDispO == 'CALLBK')
							...
							document.getElementById("CBcommentsBoxC").innerHTML = "<b><?php echo $lang_agent_link; ?>:</b>" + CBuser;
					...
					//	alert(xmlhttp.responseText);
						...
						if (MDSnextCID == "LEAD NOT REVERTED")
							...
							if (dial_method == "INBOUND_MAN")
								...
								if(Dial_Next_Display=='Y')
									{
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a><a style=\"display:none;\" href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif\" border=0 alt=\"<?php echo $lang_dial_next_number; ?>\"></a>";
									}
								else
									{
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a>";								
			...
			//		alert(xmlhttp.responseText);
					...
					if (MDnextCID == " CALL NOT PLACED") //cn bug
						...
						//	document.getElementById("MainStatuSSpan").innerHTML = " 拨号中: " + status_display_number + " UID: " + MDnextCID + " &nbsp; 进入闲置状态...";
							document.getElementById("MainStatuSSpan").innerHTML = " <?php echo $lang_dialing; ?>: " + status_display_number + " UID: " + MDnextCID + " &nbsp; <?php echo $lang_idle_state; ?>...";
	...
	function AutoDial_ReSume_PauSe(taskaction,taskagentlog,taskwrapup,taskstatuschange,temp_reason)
		...
		if (taskaction == 'VDADready')
			...
			if (dial_method == "INBOUND_MAN")
                ...
                if(Dial_Next_Display=='Y')
					{
				document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"check_max_pause_count();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"></a><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a><a style=\"display:none;\" href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif\" border=0 alt=\"<?php echo $lang_hangup; ?>\"></a>";
				    }
				else
					{
				document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"check_max_pause_count();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"></a><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"></a>";								
				    }
				//Mod by fnatic end
			...
			document.getElementById("status_code").innerHTML="<?php echo $lang_agent_available; ?>";
			if(Pause_Code_Selected_Link_Display=='Y' && (agent_pause_codes_active=='FORCE' || agent_pause_codes_active=="Y")){
				document.getElementById("PauseCodeLinkSpan").style.display="none";
			}
			}
			...
			if(AutoDialReSumeStatus == 1){
				...
				document.getElementById("status_code").innerHTML="<?php echo $lang_agent_pause; ?>";
				if(Pause_Code_Selected_Link_Display=='Y' && (agent_pause_codes_active=='FORCE' || agent_pause_codes_active=="Y")){
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						if (check_VDIC_array[0] == '1')
							...
							if ( (VDIC_data_VDFR[1].length > 1) && (VDCL_fronter_display == 'Y') )
								{VDIC_fronter = " <?php echo $lang_from; ?>: " + VDIC_data_VDFR[0] + " " + VDIC_data_VDFR[1] + " " + VDIC_data_VDIG[24];}
						...
						//	DialLog("start");
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								//来电归属地
								<?php
									if($phone_place_enable == 'Y'){
								?>
								phone_location(callnum,"<?php echo $VD_campaign; ?>");
								<?php
									}
								?>
								document.getElementById("MainStatuSSpan").innerHTML = " <?php echo $lang_caller; ?>: " + dial_display_number + "<span id='location'></span> <?php echo $lang_technical_group; ?>: " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter + "&nbsp;&nbsp;" + extension_info; 
								}
							...
							document.getElementById("HangupControl").innerHTML = "<a href=\"#\" onclick=\"dialedcall_send_hangup();\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_hangupcustomer_cn.gif\" border=0 alt=\"<?php echo $lang_hangup; ?>\"></a>";
							if (VU_vicidial_transfers == '1'){
								document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('ON');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_cn.gif\" border=0 alt=\"<?php echo $lang_transfer; ?>\"></a>";
							}
							//Add by fnatic start
							if(Xfer_Local_Closer_Display=='Y')
								{
							...
							if (dial_method == "INBOUND_MAN")
								...
								if(Dial_Next_Display=='Y')
									{
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_hangup; ?>\">";
								    }
								else
									{
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_pause_OFF_cn.gif\" border=0 alt=\" <?php echo $lang_agent_pause; ?> \"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_resume_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_resume; ?>\">";							
							...
							if (alert_enabled=='ON')
								...
								<?php
									if($phone_place_enable == 'Y'){
								?>
								phone_location_str_alert(callnum," <?php echo $lang_caller; ?>: " + dial_display_number,"<br> <?php echo $lang_technical_group; ?>: " + VDIC_data_VDIG[1] + "<br> " + VDIC_fronter + "<br>" + extension_info,"<?php echo $VD_campaign; ?>");
								<?php
									}else{
								?>
								var str_temp = " <?php echo $lang_caller; ?>: " + dial_display_number + "<br> <?php echo $lang_technical_group; ?>: " + VDIC_data_VDIG[1] + "<br> " + VDIC_fronter;
...
// refresh the content of the web form URL
	function WebFormRefresH(taskrefresh,submittask,force_webvars_refresh) 
		...
		if (taskrefresh == 'OUT')
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormRefresH('IN');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?>\"></a>\n";
			}
		else 
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormRefresH('OUT');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?>\"></a>\n";
			}
		}
		}
		//Modified by Kelvin End
...
// refresh the content of the second web form URL
	function WebFormTwoRefresH(taskrefresh,submittask) 
		...
		if (enable_second_webform > 0 )
			...
			if (taskrefresh == 'OUT')
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH('IN');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_two_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?> 2\"></a>\n";
				}
			else 
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormTwoRefresH('OUT');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_two_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?> 2\"></a>\n";
				}
			}
		}
            //Modified by Kelvin End
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo,nodeletevdac) 
		...
		// 恢复话务员文字的可点击
		document.getElementById("agentdirectlink").innerHTML = '<b><a href="#" onClick="agent_check(this);XferAgentSelectLaunch();return false;" style="font-size:12px;" title="<?php echo $lang_agent_link; ?>"><?php echo $lang_agent_link; ?></a></b>';
		// 结束
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
            ...
            //Modified by Kelvin Begin
			if (WebForm_Button_Display !='NONE' ) {
			document.getElementById("WebFormSpan").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?>\">";
			if (enable_second_webform > 0)
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_two_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_web_form ; ?> 2\">";
				}
			}
            //Modified by Kelvin Begin
			document.getElementById("ParkControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_keep_phone; ?>\">";
			document.getElementById("HangupControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_hangupcustomer_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_hangup; ?>\">";
			if (VU_vicidial_transfers == '1'){
				document.getElementById("XferControl").innerHTML = "<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_transfer; ?>\">";
			}
			...
			if ( (auto_dial_level == 0) && (dial_method != 'INBOUND_MAN') )
				...
				if (document.vicidial_form.DiaLAltPhonE.checked==true)
					...
					if (hotkeysused == 'YES')
						...
						if(Dial_Next_Display=='Y')
							{
						document.getElementById("DiaLControl").innerHTML = "<a style=\"display:none;\" href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_dialnextnumber_cn.gif\" border=0 alt=\"<?php echo $lang_hangup; ?>\"></a>";
							}
						else
							{
							document.getElementById("DiaLControl").innerHTML = "";					
			...
			document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_leave3waycall_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_leave_3_way_call; ?>\" style=\"vertical-align:middle\">"; 
		}
		}
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//	document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_leave3waycall_OFF_cn.gif\" border=0 alt=\"离开三方通话\">";
            ...
            if(Xfer_Dial_With_Customer_Display=='Y')
				{
			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"DialWithCustomerClick(1);return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_dialwithcustomer_cn.gif\" border=0 alt=\"<?php echo $lang_dial_with_customer; ?>\" style=\"vertical-align:middle\"></a>";
			    }
            
			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"DialWithCustomerClick(0);return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_parkcustomerdial_cn.gif\" border=0 alt=\"<?php echo $lang_dial_del_cliente_del_parque; ?>\" style=\"vertical-align:middle\"></a>";
            //when project is ehsn image changed
            if(Fast_Hangup_Xferline_And_Grab_Custline=='N')
				{
			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupxferline_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_hangup_xfer_line; ?>\" style=\"vertical-align:middle\">";
				}
            else
				{
			document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_leave3waycall_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_leave_3_way_call; ?>\" style=\"vertical-align:middle\">";
            document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupxferline_OFF_cn_ehsn.gif\" border=0 alt=\"<?php echo $lang_hangup_xfer_line; ?>\" style=\"vertical-align:middle\">";
				}
	
			document.getElementById("HangupBothLines").innerHTML ="<IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupbothlines_OFF_cn.gif\" border=0 alt=\"<?php echo $lang_hangup_both_lines; ?>\" style=\"vertical-align:middle\">";
			}
		    //Add by fnatic start
			//$("#TransferMainDIV").dialog("close");
			//Add by fnatic end
...
// Generate the Call Disposition Chooser panel
	function DispoSelectContent_NA_create(taskDSgrp,taskDSstage)
		...
		var loop_ct = 0;
		var PauseCode_Default = "<? echo $Default_Pause_Code; ?>";
		var Default_Call_Result = "<? echo $default_call_result; ?>";
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			...
			*/
			if (agent_pause_codes_active=='Y')
				...
				{var Close_LinK = '';}
			PauseCode_HTML = PauseCode_HTML + "</td></tr><tr><td align=\"center\"><a href=\"#\" onclick=\"PauseCodeSelect_submit_by_radio();return false;\" style=\"font-size:14px;\"><?php echo $lang_submit; ?></a>" + Close_LinK + "</td></tr></td></tr></table>";
...
// Generate the 技能组代名 Chooser panel
	function GroupAliasSelectContent_create(task3way)
		...
		var Go_BacK_LinK = "<font size=3 style=\"BACKGROUND-COLOR: #FFFFCC\"><b><a href=\"#\" onclick=\"GroupAliasSelect_submit('');return false;\"><?php echo $lang_return_back; ?></a>";
...
// open web form, then submit disposition
	function WeBForMDispoSelect_submit()
	    ...
	    //Mod by fnatic end
		document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"DialWithCustomerClick(0);return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_parkcustomerdial_cn.gif\" border=0 alt=\"<?php echo $lang_dial_del_cliente_del_parque; ?>\" style=\"vertical-align:middle\"></a>";
		document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"confirm_bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupbothlines_cn.gif\" border=0 alt=\"<?php echo $lang_hangup_both_lines; ?>\" style=\"vertical-align:middle\"></a>";

		var DispoChoice = document.getElementById("DispoSelection").value;

		if (DispoChoice.length < 1) {alert("<?php echo $lang_alert_choose_dispo_content; ?>");}
		else
			{
	...
	function DispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("<?php echo $lang_alert_choose_dispo_content; ?>");}
			...
			if(document.getElementById("vtiger_activity_id").value != ""){
				...
				$.get("update_activity.php", { activityid: activityid, subject: DispoChoice,campaignid:'<? echo $VD_campaign; ?>'},
				   function(data){
      ...
      //Add by fnatic start
	   ...
	   if(Cortorl_Pausecode_Insert_Db=='N')
		...
		if (xmlhttp) 
			{
			//alert(newpausecode);
			VMCpausecode_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass  + "&ACTION=PauseCodeSubmit&format=text&status=" + newpausecode + "&agent_log_id=" + agent_log_id + "&campaign=" + campaign + "&extension=" + extension + "&prot111ocol=" + protocol + "&phone_ip=" + phone_ip + "&enable_sipsak_messages=" + enable_sipsak_messages + "&stage=" + pause_code_counter;
...
// 展现 message that there are no voice channels in the VICIDIAL session
	...
	//		var extendial = extension + "@" + ext_context;
		...
		if (xmlhttp) 
			{ 
			VMCoriginate_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass  + "&ACTION=OriginateVDRelogin&format=text&channel=" + originatevalue + "&queryCID=" + queryCID + "&exten=" + session_id + "&ext_context=" + ext_context + "&ext_priority=1" + "&extension=" + extension + "&prot111ocol=" + protocol + "&phone_ip=" + phone_ip + "&enable_sipsak_messages=" + enable_sipsak_messages + "&allow_sipsak_messages=" + allow_sipsak_messages + "&campaign=" + campaign + "&outbound_cid=" + campaign_cid;
			xmlhttp.open('POST', 'manager_send.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			xmlhttp.send(VMCoriginate_query); 
			xmlhttp.onreadystatechange = function() 
...
// Log the user out of the system, if active call or active dial is occuring, don't let them.
	function LogouT(tempreason)
		...
		if (MD_channel_look==1)
			{alert("<?php echo $lang_alert_dialing_but_logout; ?>");}
		else
			{
			if (VD_live_customer_call==1)
				{
				alert("<?php echo $lang_alert_talking_but_logout; ?>");
				}
			else
				{
				if (alt_dial_status_display==1)
					...
					if (xmlhttp) 
						{ 
						VDlogout_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=userLOGout&format=text&user=" + user + "&pass=" + pass + "&campaign=" + campaign + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&agent_log_id=" + agent_log_id + "&no_delete_sessions=" + no_delete_sessions + "&phone_ip=" + phone_ip + "&enable_sipsak_messages=" + enable_sipsak_messages + "&LogouTKicKAlL=" + LogouTKicKAlL + "&ext_context=" + ext_context +"&session_RemotePhone=" + session_RemotePhone;
						xmlhttp.open('POST', 'vdc_db_query.php'); 
						xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
				...
				//				alert(xmlhttp.responseText);
					...
					document.getElementById("LogouTBoxLink").innerHTML = logout_content + "<a href=\"" + agcPAGE + "?relogin=YES&session_epoch=" + epoch_sec + "&session_id=" + session_id + "&session_name=" + session_name + "&VD_login=" + user + "&VD_campaign=" + campaign + "&phone_login=" + original_phone_login + "&phone_pass=" + phone_pass + "&VD_pass=" + pass + "\"><?php echo $lang_click_to_relogin; ?></a>\n";
	...
	function refresh_agents_view(RAlocation,RAcount)
		...
		if (RAcount > 0)
			...
			if (even > 0)
				...
				if (xmlhttp) 
					{ 
					//RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=AGENTSview&format=text&user=" + user + "&pass=" + pass + "&user_group=" + VU_user_group + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&stage=" + agent_status_view_time + "&agentViewUsergroup=" + agentViewUsergroup + "&agentViewTechgroup=" + agentViewTechgroup  + "&campaign=" + campaign + "&comments=" + RAlocation;
					RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=AGENTSview&format=text&user=" + user + "&pass=" + pass + "&user_group=" + VU_user_group + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&stage=" + agent_status_view_time + "&agentViewUsergroup=" + agentViewUsergroup + "&redirectcalltoagentid=" + redirectcalltoagentid + "&agentViewTechgroup=" + agentViewTechgroup  + "&campaign=" + campaign + "&comments=" + RAlocation;
					...
					xmlhttp.onreadystatechange = function() 
						...
						if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
							...
							if (RAlocation == 'AgentXferViewSelect') 
								{
								document.getElementById(RAlocation).innerHTML = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b><a href=\"#\" onclick=\"AgentsXferSelect('0','AgentXferViewSelect');return false;\"><?php echo $lang_return_back; ?></a></b>&nbsp;&nbsp;&nbsp;&nbsp;"+newRAlocationHTML;
								}
							else
								{
...
// Grab the call in queue and bring it into the session
		...
		if (CQauto_call_id > 0)
			{
				...
				if (xmlhttp) 
					{ 
					RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=CALLSINQUEUEgrab&format=text&user=" + user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&campaign=" + campaign + "&stage=" + CQauto_call_id;
					xmlhttp.open('POST', 'vdc_db_query.php'); 
					xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
					xmlhttp.send(RAview_query); 
					xmlhttp.onreadystatechange = function() 
	...
	function check_for_grab_outcoming(){
		...
		if (xmlhttp) 
			{ 
			RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=CALLSOUTBOUNDECHECK&format=text&user=" + user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&campaign=" + campaign + "&stage=" + CQauto_call_id;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			xmlhttp.send(RAview_query); 
			xmlhttp.onreadystatechange = function() 
	...
	function check_for_grab_incoming(){
		...
		if (xmlhttp) 
			{ 
			RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=CALLSINQUEUECHECK&format=text&user=" + user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&campaign=" + campaign + "&stage=" + CQauto_call_id;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			xmlhttp.send(RAview_query); 
			xmlhttp.onreadystatechange = function() 
		...
		if (CQauto_call_id > 0)
			{
				...
				if (xmlhttp) 
					...
					RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=CALLSINQUEUEgrab2&format=text&user=" + user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&user_group=" + VU_user_group + "&last_name=" + recording_exten +"&campaign=" + campaign + "&stage=" + CQauto_call_id;
					xmlhttp.open('POST', 'vdc_db_query.php'); 
					xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
					xmlhttp.send(RAview_query); 
					xmlhttp.onreadystatechange = function() 
						...
						if (xmlhttp.readyState == 4 && xmlhttp.status == 200) 
							{
								...
								document.getElementById("line12").innerHTML = "<input type='button' onclick=\"callinqueuehangup(0);return false;\" value=\"<?php echo $lang_hangup; ?>\" />";
								xmlhttp = null;
								CollectGarbage();
							}
						}
	...
	function callinqueuehangup(type){
		...
		if (xmlhttp) 
			{ 
			RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=CALLSINQUEUEHANGUP&format=text&user=" + user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&campaign=" + campaign + "&stage=" + CQauto_call_id + "&agent_log_id_line2=" + agent_log_id_line2;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			xmlhttp.send(RAview_query); 
			xmlhttp.onreadystatechange = function() 
	...
	function refresh_calls_in_queue(CQcount)
		...
		if (CQcount > 0)
			...
			if (even > 0)
				...
				if (xmlhttp) 
					{ 
					RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=CALLSINQUEUEview&format=text&user=" + user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&campaign=" + campaign + "&stage=100%";
					xmlhttp.open('POST', 'vdc_db_query.php'); 
					xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
					xmlhttp.send(RAview_query); 
					xmlhttp.onreadystatechange = function() 
...
// Open or close the callsinqueue view bottombar
	function show_calls_in_queue(CQoperation)
		...
		if (CQoperation=='SHOW')
			...
			document.getElementById("callsinqueuelink").innerHTML = "<a href=\"#\"  onclick=\"show_calls_in_queue('SHOW');\"><?php echo $lang_link_grap_queue; ?></a>";
	...
	function AgentsViewOpen(AVlocation,AVoperation)
		...
		if (AVoperation=='open')
			...
			if (AVlocation=='AgentViewSpan')
				...
				//document.getElementById("AgentViewLink").innerHTML = "<a href=\"#\" onclick=\"AgentsViewOpen('AgentViewSpan','close');return false;\"><IMG SRC=\"../agc/images/cn/lgq_ss_see.gif\" BORDER=0> </a>";
				document.getElementById("AgentViewLink").innerHTML = "<a href=\"#\" onclick=\"redirectcalltoagentid = '';AgentsViewOpen('AgentViewSpan','open');return false;\"><IMG SRC=\"../agc/images/cn/lgq_ss_see.gif\" BORDER=0> </a>";
	...
	function TimerActionRun(taskaction,taskdialalert)
	    ...
	    //  Mod by fnatic end  
			...
			if ( (timer_action_message.length > 0) || (timer_action == 'MESSAGE_ONLY') )
				{
				document.getElementById("TimerContentSpan").innerHTML = "<b>定时器通知: " + timer_action_seconds + " <?php echo $lang_seconds; ?><BR><BR>" + timer_action_message + "</b>";
...
// GLOBAL FUNCTIONS
	function begin_all_refresh()
		...
		//alert(0);
		<?php if($IM_Enable=='Y'){ ?>
		imOpener = openWindow('../chat/index.php?userName=<?php echo $VD_login;?>&talk_level=<?php echo $IM_Talk_Level;?>&admin_level=<?php echo $IM_Admin_Level;?>');
		<?php } ?>
		...
		socket.on('outbound', function (phone) {
	    //startCall(phone);
	    NeWManuaLDiaLCalL('NO');
	   	document.getElementById('MDPhonENumbeR').value = phone;  
	  }); 
	  
 		socket.on('inbound', function (agent) {
	    console.log(agent + " AutoDialWaiting: "+ AutoDialWaiting + " AgentDispoing:" + AgentDispoing);
     ...
     // alert("in:" + agent);
	  ...
	  socket.on('ready', function (agent) {
	    AutoDial_ReSume_PauSe('VDADready');
	  }); 
	  
	  socket.on('pause', function (agent) {
	    check_max_pause_count();
	  }); 		
	  ...
	  socket.on('logout', function (agent) {
	    ...
	    $url_ws = "vicidial.php?VD_campaign="+$data.campaign+"&VD_login="+$data.user+"&VD_pass="+$data.pass+"&phone_login="+$data.user+"&phone_pass="+$data.pass;
	    location.href=$url_ws; 			

	  }); 		  
	  
	  
	  var $agentid=<?php echo $VD_login;?>;
	...
	function start_all_refresh()
		...
		if(ParkControlCount>=1){
			...
			if(ParkControlCount ==6){
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('FROMParK','" + ParkControlredirectdestination + "','" + ParkControlredirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_grabparkedcall_cn.gif\" border=0 alt=\"<?php echo $lang_grab_park_call; ?>\"></a>";
				ParkControlCount = 0;
			}
		}
		...
		//	if ( (agentcall_manual != '1') && (starting_dial_level > 0) )
			...
			if ( (agent_pause_codes_active=='Y') || (agent_pause_codes_active=='FORCE') )
				...
				if(Pause_Code_Selected_Link_Display=='Y')
					{
				     document.getElementById("PauseCodeLinkSpan").innerHTML = "<a href=\"#\" onclick=\"changePauseCode();\"><?php echo $lang_modify; ?></a>";			   
				    }
				//Mod by fnatic end
				//Mod by yanson start
                if(agent_pause_codes_active=='FORCE' && Default_Pause_Code_Enable == 'Y'){
			...
			if ( (crm_popup_login == 'Y') && (crm_login_address.length > 4) )
				...
				if(crm_target=='CCMS')
					{
					//document.getElementById("ScriptContents").innerHTML = "<iframe onload=\"SetCwinHeight(this)\"  src=\"" + TEMP_crm_login_address + "\" style=\"background-color:transparent;\"  frameborder=\"0\"  scrolling=\"yes\" allowtransparency=\"true\" id=\"popupFrame\" name=\"popupFrame\" width=\"99%\" height=\"" + script_height + "\" STYLE=\"z-index:18\"></iframe>";	
					}
				// end
			...
			if (open_dispo_screen==1)
				{
				var Auto_Submit_Dispo = "<? echo $auto_submit_dispo; ?>";
	...
	function agent_grab_line2(){
		...
		if (xmlhttp) 
			{ 
			RAview_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=agent_grab_line2&format=text&user=" + window.user + "&pass=" + pass + "&conf_exten=" + session_id + "&extension=" + extension + "&prot111ocol=" + protocol + "&campaign=" + campaign + "&stage=100%";
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			xmlhttp.send(RAview_query); 
			xmlhttp.onreadystatechange = function() 
		...
		if (document.getElementById(divvar))
			...
			if (divvar == 'DiaLLeaDPrevieW')
				...
				if(Lead_Preview_Display=='Y')
					{			    
				var buildDivHTML = "<font class=\"preview_text\"> <input type=checkbox name=LeadPreview size=1 value=\"0\"> <?php echo $lang_review_lead; ?><BR></font>";
				    }
				else
					{
				var buildDivHTML ="";
	...
	function conf_channels_detail(divvar) 
		...
		if (divvar == 'SHOW')
			...
			if(Conference_Channel_Display=='Y')
			   ...
			   document.getElementById("busycallsdisplay").innerHTML = "<a href=\"#\"  onclick=\"conf_channels_detail('SHOW');\"><?php echo $lang_link_display_channel_info; ?></a>";
			    }
		   ...
		   $('#MaiNfooterspanDIV').dialog
				({ 
			       draggable: false,
				   resizable:false ,
				   title:'<?php echo $lang_link_display_channel_info; ?>',
				   width:950,
			       height:350,
			       modal: true
			    });
   ...
   */
	function client_phone_manager(taskvar)
		...
		if (xmlhttp) 
			{ 
			manager_request_parameter = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=" + taskvar + "&format=text&conf_exten=" + session_id + "&prot111ocol=" + protocol  + "&extension=" + extension + "&SIP_user_DiaL=" + SIP_user_DiaL + "&ext_context=" + ext_context + "&campaign_cid=" + campaign_cid ;
...
<!--
	...
	div.scroll_script {height: <?=$SSheight-30 ?>px; width: <?=$SDwidth-50 ?>px; background:#FFCC99; overflow: scroll; font-size: 12px;  font-family:"宋体";}
   ...
   #button_pannel{ float:left;margin-top:0px;}
   #pannel_img{ cursor:pointer;}
   div.button_link{text-align:left;margin:5px 0px 0px 0px;float:right;}
   ...
   #MaiNfooterspan{top:<?php echo $HKheight+31; ?>px !important;/*ff*/ *top:<?php echo $HKheight+11; ?>px;/*ie6 7 8*/}
...
<div id='divtest' style='position: absolute;display:none; z-index: 9999;width:100px;height:20px;'><div class="demo"><div id="slider-vertical" style="height:100px;"></div></div></div>
<FORM name=vicidial_form>
<input type=HIDDEN NAME=vtiger_activity_id ID=vtiger_activity_id value="">
<input type=HIDDEN NAME=vtiger_inbound_number ID=vtiger_inbound_number value="">
<span style="width:100%;position:absolute;left:0px;top:0px;z-index:2; " id="Header">
<TABLE style="display:none" width="100%" BORDER=0 CELLPADDING=0 CELLSPACING=0 BGCOLOR=white height="30px" MARGINWIDTH=0 MARGINHEIGHT=0 LEFTMARGIN=0 TOPMARGIN=0 VALIGN=TOP ALIGN=LEFT>
  ...
  <TD class="banner_text_2" COLSPAN=4  VALIGN=TOP ALIGN=RIGHT style="background:url(../agc/images/logo_r1_c3.jpg); background-repeat:repeat-x;text-algin:left;">
    <b><span style="color:#FFFFFF; font-size:12px; font-family:Arial, Helvetica, sans-serif;" id="status">LIVE</span></b>&nbsp;
     <font class="banner_text">
       <span id="SecondSspan">
	    <font class="body_text"><?php echo $lang_talk_time; ?>: 
      ...
      <?php if ($INgrpCT > 0 && $Ingroup_Change_Enable=='Y') {echo "<a href=\"#\" onclick=\"OpeNGrouPSelectioN();return false;\">转技能组</a> &nbsp; &nbsp; \n";} ?>
	  <?php if($user_level>=5){ ?><a href="../admin.php" target="_blank">CCMS Admin</a>&nbsp;&nbsp;<?php } ?>
	  <a href="#" style="display:none" onClick="changepassword();return false;"><?php echo $lang_link_change_password; ?></a>&nbsp;&nbsp;
      <?php	echo "<a href=\"#\"  onclick=\"NormalLogout();return false;\">$lang_link_logout</a>\n"; ?><img src="../agc/images/logo_r1_c5.jpg" align="middle" />
    </font>
  </TD>
...
<div style="display:none" id="dialog-password" title="<?php echo $lang_link_change_password; ?>">
	<table>
<tr><td><label for="name"><?php echo $lang_old_password; ?>:</label></td><td><input type="password" name="old_password" id="old_password" value="" class="text ui-widget-content ui-corner-all"/></td></tr>
<tr><td><label for="email"><?php echo $lang_new_password; ?>:</label></td><td><input type="password" name="new_password1" id="new_password1" value="" class="text ui-widget-content ui-corner-all"/></td></tr>
<tr><td><label for="password"><?php echo $lang_new_password2; ?>:</label></td><td><input type="password" name="new_password2" id="new_password2" value="" class="text ui-widget-content ui-corner-all"/></td></tr>
<tr><td colspan="2"  align="center"><button onClick="change_pass();"><?php echo $lang_submit; ?></button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button onClick="closedialog();"><?php echo $lang_cancel; ?></button></td></tr>
</table>
</div>
...
<TD WIDTH=<?php //echo $HSwidth ?> VALIGN=MIDDLE ALIGN=CENTER><font class="body_text">&nbsp;</TD>
<TD WIDTH=109></TD>
</TR></TABLE>-->
</span>

<span style="position:absolute;left:0px;top:0px;z-index:3;" id="WelcomeBoxA">
    <!--<table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $HKwidth ?>><TR><TD align=center><BR><span id="WelcomeBoxAt">Agent Screen</span></TD></TR>-->
    <table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> ><TR><TD align=center><BR><span id="WelcomeBoxAt">Agent Screen</span></TD></TR></TABLE>
</span>
...
<!--<span style="position:absolute;left:430px;top:<?php //echo $CBheight ?>px;z-index:14;" id="PauseCodeButtons"><font class="body_text">
...
<span style="display:none;position:absolute;left:<?php echo $AMwidth ?>px;top:<?php echo $AMheight ?>px;z-index:22;" id="AgentMuteANDPreseTDiaL"><font class="body_text">
	<?php
	if ($PreseT_DiaL_LinKs)
		{
		echo "<a href=\"#\" onclick=\"DtMf_PreSet_a_DiaL();return false;\"><font class=\"body_tiny\">D1 - W?HLEN</font></a>\n";
		echo "<BR>\n";
		echo "<a href=\"#\" onclick=\"DtMf_PreSet_b_DiaL();return false;\"><font class=\"body_tiny\">D2 - W?HLEN</font></a>\n";
		}
	else {echo "<BR>\n";}
	?>
<BR><BR> &nbsp; <BR>
...
<div id="CallBacKsLisTBoxDIV" style="display:none">
<span id="CallBacKsLisTBox">
    <table border="0" cellspacing="0" cellpadding="0" width="100%">
	<TR><TD align="center" valign="top">
	<!--话务员的回拨电话 <?php //echo $VD_login ?>:<BR>点选以下回拨记录进行客户回拨，如果点选记录进行拨号，本次记录将会从名单内移除.-->
	<!--<BR>-->
...
<!--<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top><!-- 新的人工拨号纪录给 <?php //echo "$VD_login in campaign $VD_campaign" ?>:<BR><BR>请输入下面的信息给您想要外拨的新记录.
	<BR>
	<?php 
	if (eregi("X",$dial_prefix))
		{
		//echo "Note: a dial prefix of $dial_prefix 将会加在此号码之前<BR>\n";
		}
	?>
...
<!--Add by fnatic start-->
<div id="ManualDialDIV" style="display:none">
<span id="NeWManuaLDiaLBox">
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
	   <td height="37"><input type='checkbox' name='manudial_dial_type' id='manudial_dial_type' title='<?php echo $lang_inline_dial_title; ?>' /> <?php echo $lang_inline_dial; ?><!-- + fnatic 20110627 -->
	       <input type='text' size="15" maxlength="20" name="MDPhonENumbeR" id="MDPhonENumbeR" value="" >
		   <input type='button' onClick="NeWManuaLDiaLCalLSubmiT('NOW');return false;" value="<?php echo $lang_dial; ?>" />
        <table border="1">
</table>
<tr>
...
<span style="display:none;position:absolute;left:35px;top:<?php echo $CBheight ?>px;z-index:20;" id="AgentStatusSpan" ><font class="body_text">
...
<span id="TransferMain">
 ...
 <table cellpadding="0" cellspacing="0" border="0" class="transfer_table" align="center">
  <!-- <tr>
    ...
    <td>
       <span style="position:absolute;left:0px;top:0px;width:360px;height:300px;background-color:#FFFFFF" id="AgentXferViewSpan">
			 <span style="margin-top:5px;margin-bottom:5px; padding-bottom:5px; text-align:center;width:100%;border-bottom:1px solid #eee;">
				 <?php echo $lang_user_group; ?>：
				 <select name="AgentViewUsergroup2" id="AgentViewUsergroup2" style="width:120px;">
				   <?php echo getAgentViewUsergroup($VD_login,$VD_campaign); ?>
		     </select>
			 <?php echo $lang_technical_group; ?>：
			 <select name="AgentViewTechgroup2" id="AgentViewTechgroup2" style="width:120px;"><?php echo getAgentViewTechgroup($VD_campaign); ?></select>
			 </span>
   ...
   <td>
   <fieldset>
	<legend style="color:#000"><?php echo $lang_transfer_target; ?></legend>
	<div style="margin:2px 2px">
	 <input type="radio" name="btnTransferTarget" id="consultativexfer" value="0" checked="checked" style="vertical-align:text-bottom" onClick="agentclick()"><span id="agentdirectlink" style="width:45px;"><b><a href="#" onClick="agent_check(this);XferAgentSelectLaunch();return false;" style="font-size:12px;" title="<?php echo $lang_agent_link; ?>"><?php echo $lang_agent_link; ?></a></b>
     </span>
	 <span id="agentinput"><input type="text" size="12" name="xfernumber" id="xfernumber" maxlength="25" value="<?php echo $preset_populate ?>" style="margin-left:19px"></span>	
	</div>

    <div style="margin:2px 2px">
	 <input type="radio" name="btnTransferTarget" id="xferoverride" size="1" value="0" style="vertical-align:text-bottom" onClick="overrideclick()"><?php echo $lang_outline_phone; ?>
	 <span id="overrideinput"><input type="text" size="12"  maxlength="25" value="" style="margin-left:10px"></span>
	...
	<div style="margin:4px 2px">
	 <input type="radio" name="btnTransferTarget" id="xferoverrideaaa" size="1" value="0" style="vertical-align:text-bottom" onClick="trasfertargetclick()"><?php echo $lang_technical_group; ?>
	 <span id="XfeRGrouPLisT" style="margin-left:22px;">	    
		<select size=1 name="XfeRGrouP" id="XfeRGrouP" onChange="XferAgentSelectLink();return false;" ><option>-- 选择转接技能组--</option></select>
	 </span>
      <span STYLE="background-color: <?php echo $MAIN_COLOR ?>;display:none" id="LocalCloser">
       <?php if($Xfer_Local_Closer_Display=='Y'){ ?>
	 ...
	 <?php if($Xfer_Answer_Machine_Message_Display=='Y'){?>
	...
	<?php if($Xfer_Answer_Machine_Message_Display=='Y'){?>
	<div style="margin:2px 2px">
	 <input type="radio" name="btnTransferTarget" id="xferoverridebbb" size="1" value="0" style="vertical-align:text-bottom" onClick="vmclick()"><?php echo $lang_voice_info; ?>	</div>
	<?php } ?>
 </fieldset>   </td>
  </tr>
  ...
  <tr>
   <td>
    <fieldset>
	  <legend style="color:#000"><?php echo $lang_transfer_mode; ?></legend>
     <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialBlindTransfer">	 </span>&nbsp; 

     <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialWithCustomer">
	     <a href="#" onClick="DialWithCustomerClick(1);return false;">
		  <?php if($Xfer_Dial_With_Customer_Display=='Y'){ ?>
		  <IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_dialwithcustomer_cn.gif" border=0 alt="<?php echo $lang_dial_with_customer; ?>" style="vertical-align:middle">
		  <?php } ?>
		 </a>	</span>&nbsp;

    <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkCustomerDial">
	  <a href="#" onClick="DialWithCustomerClick(0);return false;">
	     <IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_parkcustomerdial_cn.gif" border=0 alt="<?php echo $lang_dial_del_cliente_del_parque; ?>" style="vertical-align:middle">	  </a>	</span>
   </fieldset>   </td>
  </tr>
  ...
  <tr>
   <td>
     <fieldset>
	  <legend style="color:#000"><?php echo $lang_hangup_mode; ?></legend>
      <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupXferLine">
	   <!--when project is ehsn image changed-->
      <IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupxferline_OFF_cn_ehsn.gif" border=0 alt="<?php echo $lang_hangup_xfer_line; ?>" style="vertical-align:middle">	  </span>    
     <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupBothLines">
	  
	     <IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupbothlines_OFF_cn.gif" border=0 alt="<?php echo $lang_hangup_both_lines; ?>" style="vertical-align:middle">	 </span>
    <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="Leave3WayCall">
	  
	    <IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_leave3waycall_OFF_cn.gif" border=0 alt="<?php echo $lang_leave_3_way_call; ?>" style="vertical-align:middle">	</span>
  </fieldset>   </td>
  </tr>
 </table>
...
<div id="AgentViewSpanDIV">
 <span style="margin-bottom:5px; padding-bottom:5px; text-align:center;width:100%;border-bottom:1px solid #eee;">
	 <?php echo $lang_user_group; ?>：<select name="AgentViewUsergroup" id="AgentViewUsergroup" style="width:150px;"><?php echo getAgentViewUsergroup($VD_login,$VD_campaign); ?></select>
	 <?php echo $lang_technical_group; ?>：<select name="AgentViewTechgroup" id="AgentViewTechgroup" style="width:150px;"><?php echo getAgentViewTechgroup($VD_campaign); ?></select>
 </span>
...
<span style="display:none;position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:34;" id="HotKeyActionBox" >
...
<span style="display:none;position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:35;" id="HotKeyEntriesBox">
    <table border=0 bgcolor="#FFDD99" width=<?php echo $HCwidth ?> height=70>
...
<span style="display:none;position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:36;" id="CBcommentsBox">
...
<span style="display:none;position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:37;" id="EAcommentsBox">
    <table border=0 bgcolor="#FFFFCC" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFFF66">
...
<span style="display:none;position:absolute;left:695px;top:<?php echo $HTheight ?>px;z-index:38;" id="EAcommentsMinBox">
...
<span style="display:none;position:absolute;left:0px;top:0px;z-index:39;width:644px;height:200px" id="NoneInSessionBox">
    <table border=1 bgcolor="#CCFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> <?php echo $lang_nobody_in_conference; ?>: <span id="NoneInSessionID"></span><BR>
	<a href="#" onClick="NoneInSessionOK();return false;"><?php echo $lang_return_back; ?></a>
	<BR><BR>
	<a href="#" onClick="NoneInSessionCalL();return false;"><?php echo $lang_ring_the_agent_phone_again; ?></a>
	</TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:0px;z-index:40;width:100%;" id="CustomerGoneBox">
    <table border=1 bgcolor="#CCFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> 客户已挂断电话: <span id="CustomerGoneChanneL"></span><BR>
	<a href="#" onClick="CustomerGoneOK();return false;"><?php echo $lang_return_back; ?></a>
	<BR><BR>
	<a href="#" onClick="CustomerGoneHangup();return false;">结束通话,填写小结</a>
	</TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:0px;z-index:41;width:100%;" id="WrapupBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> 电话文书: <span id="WrapupTimer"></span> 文书状态尚余秒数<BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:43;width:100%;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center>你的会话已不可用<BR><a href="#" onClick="LogouT('DISABLED');return false;">注销2</a><BR><BR><a href="#" onClick="hideDiv('AgenTDisablEBoX');return false;"><?php echo $lang_return_back; ?></a>
</TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:0px;z-index:44;width:100%;" id="SysteMDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center><?php echo $lang_alert_system_time_synchronization; ?><BR><BR><BR><a href="#" onClick="hideDiv('SysteMDisablEBoX');return false;"><?php echo $lang_return_back; ?></a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:45;width:100%" id="LogouTBox">
    <table  border=1 bgcolor="#FFFFFF" width=200px height=100px><TR><TD align=center ><BR><span id="LogouTBoxLink">注销</span></TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:70px;z-index:46;" id="DispoButtonHideA">
    <table border=0 bgcolor="#F4F4F4" width=195 height=70><TR><TD align=center VALIGN=top></TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:138px;z-index:47;" id="DispoButtonHideB">
    <table border=0 bgcolor="#F4F4F4" width=195 height=250><TR><TD align=center VALIGN=top>&nbsp;</TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:0px;z-index:48;" id="DispoButtonHideC">
    <table border=0 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=47><TR><TD align=center VALIGN=top>你必须先更改客户数据，再挂断电话。否则所做变更将不会生效！. </TD></TR></TABLE>
</span>
...
<div id="DispoSelectBox">
	...
	<input type="checkbox" name="DispoSelectStop" id="DispoSelectStop" size=1 value="0"  onClick="change_pause_code(this);"><?php echo $lang_agent_pause; ?>
	<BR>
	<span id="DispoPauseHTMLContent"></span>
	<?php
	if($Dial_Next_Display == 'Y'){
	?>
	<span style="width:100%;height:30px;"><span id="DispoDialNextContent"><input type="checkbox" name="DispoDialNext" id="DispoDialNext" size=1 value="0"><?php echo $lang_dial_next; ?></span>&nbsp;</span>
	<?php
	}
	?>
	<span style="width:100%;height:30px;">
	<p align="center"><a href="#" onClick="DispoSelect_submit2();return false;" style="font-size:14px;"><?php echo $lang_submit; ?></a></p>
	</span>
	<? } ?>
...
<span style="display:none;position:absolute;left:0px;top:0px;z-index:65;width:100%;" id="GroupAliasSelectBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> 选择技能组代名 :<BR>
...
<!--Edit by fnatic-->
<span style=" display:none" id="GENDERhideFORieALT"></span>

<!--End-->
<div id="CallBackSelectBox">
    <!--<table border=1 bgcolor="#F4F4F4" width=<?php //echo $CAwidth ?> height=<?php //echo $WRheight ?>><TR><TD align=center VALIGN=top> 选择回拨日期 :<span id="CallBackDatE">-->
    <table width=<?php echo $CAwidth ?> border=0 cellpadding="5"><TR><td bgcolor="#FEFBE2" style="border:1px solid #FEEB84;border-collapse:collapse;">
	...
	if ($agentonly_callbacks)
		{echo "<input type=checkbox checked=checked name=CallBackOnlyMe id=CallBackOnlyMe size=1 value=\"0\">自己跟进<BR>";}
	?>
	</TD></TR><TR><td bgcolor="#FEFBE2" style="border:1px solid #FEEB84;border-collapse:collapse;">
	...
	<a href="#" onClick="CallBackDatE_submit();return false;" style="font-size:14px;"><?php echo $lang_submit; ?></a>       <a href="#" onClick="fall_CallBack();";style="font-size:14px;">返回</a><BR>

	</TD></TR></TABLE>
</div>

<span style="display:none;position:absolute;left:0px;top:0px;z-index:53;width:100%;" id="CloserSelectBox">
    <table border=1 bgcolor="#F4F4F4"  height=200px width=200px><TR><TD align=center VALIGN=top>请选择技能组 <BR>
	...
	if ( ($outbound_autodial_active > 0) and ($disable_blended_checkbox < 1) and ($dial_method != 'INBOUND_MAN') )
		{
		?>
		<input type=checkbox name=CloserSelectBlended size=1 value="0">呼入呼出混合模式 <BR>
		<?php
		}
	?>
	<a href="#" onClick="CloserSelectContent_create();return false;"> 重置 </a> | 
	<a href="#" onClick="CloserSelect_submit();return false;"><?php echo $lang_submit; ?></a>
	<BR>
	</TD></TR></TABLE>
</span>

<span style="display:none;position:absolute;left:0px;top:0px;z-index:54;" id="TerritorySelectBox">
    <table border=1 bgcolor="#F4F4F4" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> TERRITORY SELECTION <BR>
	...
	<a href="#" onClick="TerritorySelect_submit();return false;"><?php echo $lang_submit; ?></a>
	<BR><BR><BR><BR> &nbsp; 
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
	...
	<?php
	if (!$agentonly_callbacks)
		{echo "<input type=checkbox name=CallBackOnlyMe size=1 value=\"0\"> 只有个人回拨 <BR>";}
	if ( ($outbound_autodial_active < 1) or ($disable_blended_checkbox > 0) or ($dial_method == 'INBOUND_MAN') )
		{echo "<input type=checkbox name=CloserSelectBlended size=1 value=\"0\"> 呼入/呼出 混合模式<BR>";}
	?>
</span>
...
<span style="position:absolute;left:<?php echo $AMwidth ?>px;top:60px;z-index:31;" id="ScriptRefresH">
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF_cn.gif" border=0 alt="关闭快捷键"></a></span>
<?php } ?>

<div id="MaiNfooterspanDIV">
<span id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>>
<tr>
 <td>
...
<span style="position:absolute;left:0px;top:0px;z-index:10;width:100%;" id="MainPanel">
<!--<TABLE border=0 BGCOLOR="<?php //echo $MAIN_COLOR ?>" width=<?php //echo $MNwidth ?> id="MainTable" >-->
<TABLE border=0 BGCOLOR="<?php echo $MAIN_COLOR ?>" width="100%" id="MainTable" >
<TR>
  <TD colspan="2">
   <table border=0 width="100%" cellspacing="0" cellpadding="0">
    <tr>
	 <td width="175px" align="center">
	  <IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>agc_live_call_OFF_cn.gif" NAME=livecall  WIDTH=109 HEIGHT=30 BORDER=0>
	  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  <IMG SRC="../agc/images/pannel_control_img.jpg" style="margin-bottom:10px;" id="pannel_img" onClick="javascript:show_pannel_div();" ALT="<?php echo $lang_hide_panel; ?>" border="0">
	 </td>
	 <td align="left" valign="middle">
	   <div class="queuediv">
		 <span style="display:none" id="out_line12">
		 </span>
		 <span id="line12"></span>
...
<tr>
<td id="button_pannel_td" nowrap="nowrap">
<div id="button_pannel">
<span id="HangupControl"><IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_hangupcustomer_OFF_cn.gif" border=0 alt="<?php echo $lang_hangup; ?>"></span>


<span id="DiaLControl">
<?php if($Dial_Next_Display=='Y'){?>
<a  href="#" onClick="ManualDialNext('','','','','','0');" style="display:none"><IMG SRC="../agc/images/vdc_LB_dialnextnumber_OFF_cn.gif" border=0 alt="拨下一个号码"></a>
<?php } ?>
</span>

<span id="ParkControl"><IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_parkcall_OFF_cn.gif" border=0 alt="<?php echo $lang_keep_phone; ?>"></span>
<span id="XferControl"><?php if ($VU_vicidial_transfers == '1'){?><IMG SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_transferconf_OFF_cn.gif" border=0 alt="转接"><?php }?></span>

<span id="DiaLDiaLAltPhonE"><input type=checkbox name=DiaLAltPhonE size=1 value="0">拨打其它电话</span>
<span id="DiaL1111"><IMG SRC="../agc/images/cn/lgq_ss_vertical_line.gif" BORDER=0></span>
<span id="ManuaLDiaLButtons"><span id="MDstatusSpan">
		<a href="#" onClick="NeWManuaLDiaLCalL('NO');return false;"><IMG SRC="../agc/images/cn/lgq_ss_call.gif" BORDER=0><?php //echo $lang_link_manual_dial; ?></a>
	 </span></span>    
<span id="AgentViewLinkSpan">
	  <span id="AgentViewLink">
	  
	   <a href="#" onClick="redirectcalltoagentid = '';AgentsViewOpen('AgentViewSpan','open');return false;"><IMG SRC="../agc/images/cn/lgq_ss_see.gif" BORDER=0> <?php //echo $lang_link_agent_status; ?></a>
	  </span>
	 </span>
<span id="AgentAlertSpan">
	<?php
	if ( (ereg('ON',$VU_alert_enabled)) and ($AgentAlert_allowed > 0) )
	{echo "<a href=\"#\" onclick=\"alert_control('OFF');return false;\">";
	 echo "<IMG SRC=\"../agc/images/cn/" ."lgq_ss_off.gif\" border=0> </a>";
	
	}
	else
	{echo "<a href=\"#\" onclick=\"alert_control('ON');return false;\">";
	 echo "<IMG SRC=\"../agc/images/cn/" ."lgq_ss_on.gif\" border=0></a>";
	}
	?>
	</span>

<span id="CallbacksButtons"></span><span id="CBstatusSpan"><a href="#" onClick="CalLBacKsLisTCheck();return false;"><IMG SRC="../agc/images/cn/lgq_ss_callback.gif" BORDER=0><?php //echo $lang_link_agent_callback_notice; ?></a></span>

<span style="" id="VolumeControlSpan"><span id="VolumeUpSpan"><IMG SRC="../agc/images/vdc_volume_up_off.gif" BORDER=0></span><span id="VolumeDownSpan"><IMG SRC="../agc/images/vdc_volume_down_off.gif" BORDER=0></span>
</font></span>
<span id="DiaLLeaDPrevieW" style="display:none">
<?php if($Lead_Preview_Display=='Y'){?>
<input type=checkbox name=LeadPreview size=1 value="0"><?php echo $lang_review_lead; ?>
<?php } ?>
</span>
...
if ( ($manual_dial_preview) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=LeadPreview size=1 value=\"0\"> 预览 lead<BR></font>";}
if ( ($alt_phone_dialing) and ($auto_dial_level==0) )
	{echo "<font class=\"preview_text\"> <input type=checkbox name=DiaLAltPhonE size=1 value=\"0\"> 拨打其它电话<BR></font>";}
...
<div style="display:none"> 
  ...
  <span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="RecorDControl"><a href="#" onClick="conf_send_recording('MonitorConf',session_id,'');return false;"><IMG SRC="../agc/images/vdc_LB_startrecording_cn.gif" border=0 alt="启动录音"></a></span>
  <!--// fix vicidial record bug + fantic-->
...
<span STYLE="background-color: #FFFFFF;display:none" id="WebFormSpan"><IMG  SRC="../agc/images/<?php echo $lang_images_dir; ?>vdc_LB_webform_OFF_cn.gif" border=0 alt="<?php echo $lang_web_form ; ?>"></span>
<?php
if ($enable_second_webform > 0)
	{echo "<span STYLE=\"background-color: #FFFFFF;display:none\" id=\"WebFormSpanTwo\"><IMG SRC=\"../agc/images/" . $lang_images_dir . "vdc_LB_webform_two_OFF_cn.gif\" border=0 alt=\"" . $lang_web_form . " 2\"></span>\n";}
?>
...
<?php
if ($quick_transfer_button_enabled > 0)
	{echo "<span STYLE=\"background-color: $MAIN_COLOR\" id=\"QuickXfer\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer_OFF_cn.gif\" border=0 alt=\"Quick Transfer\"></span>\n";}
?>
...
<tr>
	<td>
	   <span id="ManuaLDiaLButtons"><span id="MDstatusSpan">
		<a href="#" onClick="NeWManuaLDiaLCalL('NO');return false;"><?php echo $lang_link_manual_dial; ?></a>
	 </span><!--<a href="#" onClick="NeWManuaLDiaLCalL('FAST');return false;">快速拨号</a>--></span>   
	</td>
</tr>
<tr <?php if($VU_view_agent_status == 'N'){ echo " style=\"display:none;\""; }?>>
	<td>
	 <span id="AgentViewLinkSpan">
	  <span id="AgentViewLink">
	  
	   <a href="#" onClick="redirectcalltoagentid = '';AgentsViewOpen('AgentViewSpan','open');return false;"><?php echo $lang_link_agent_status; ?></a>
	  </span>
	 </span>
	</td>
</tr>
<?php if($AgentAlert_allowed> 0){ ?>
<tr>
	<td>
	<span id="AgentAlertSpan">
	<?php
	if ( (ereg('ON',$VU_alert_enabled)) and ($AgentAlert_allowed > 0) )
	{echo "<a href=\"#\" onclick=\"alert_control('OFF');return false;\">$lang_link_close_inbound_notice </a>";}
	else
	{echo "<a href=\"#\" onclick=\"alert_control('ON');return false;\">$lang_link_open_inbound_notice</a>";}
	?>
	</span>
	</td>
</tr>
<?php } ?>
<?php if($grab_calls_in_queue=='Y'){ ?>
<tr>
	<td>
	<span  id="callsinqueuelink">
	<?php 
	//echo $view_calls_in_queue."|".$view_calls_in_queue_launch;
	if ($view_calls_in_queue > 0)
		{ 
		if ($view_calls_in_queue_launch > 0) 
			{echo "<a href=\"#\" onclick=\"show_calls_in_queue('HIDE');\">$lang_link_hide_grqp_queue</a>\n";}
		else 
			{echo "<a href=\"#\" onclick=\"show_calls_in_queue('SHOW');\">$lang_link_grap_queue</a>\n";}
		}
	?>
	</span>   
	</td>
...
<?php } ?>
<?php if($Conference_Channel_Display=='Y') { ?>
<tr>
	<td colspan="2">
	<span id="busycallsdisplay">
	<a href="#"  onclick="conf_channels_detail('SHOW');"><?php echo $lang_link_display_channel_info; ?></a>
	</span>   
	</td>
</tr>
<?php } ;?>
...
<tr>
	...
	function small_menu_openwindow(url)
		...
		var wd_width='800';
		var wd_height='600';
		//var wd_width=screen.availWidth-120;
		//var wd_height=screen.availHeight-150;
		var wd_top=15;
		var wd_left=15;
		var wd_toolbar=0;
		var wd_menubar="no";
	...
	<a onClick="imOpener = openWindow(this.href);this.blur();return false;" href="../chat/index.php?userName=<?php echo $VD_login;?>&talk_level=<?php echo $IM_Talk_Level;?>&admin_level=<?php echo $IM_Admin_Level;?>">即时信息</a>

	</td>
</tr>
<?php } ?>
<?php if($scheduled_callbacks=='1'){ ?>
<tr>
	<td>
	<span id="CallbacksButtons"></span><span id="CBstatusSpan"><a href="#" onClick="CalLBacKsLisTCheck();return false;"><?php echo $lang_link_agent_callback_notice; ?></a></span>
	</td>
</tr>
<?php } ?>
</table>
...
<tr><td><?php echo $lang_notice; ?></td></tr>
<tr><td>
<marquee height=100 width=100% direction=up scrollamount=2 onmouseover=this.stop() onmouseout=this.start() >
<table  width="100%">
<?php
$now = date("Y-m-d H:i:s");
$stmt = "SELECT id,title,DATE FROM ccms_notice where campaign_id='$VD_campaign' and active_time>'$now'";
//echo $stmt;
$rslt=mysql_query($stmt, $link);
//$row=mysql_fetch_row($rslt);
while($row=mysql_fetch_row($rslt)){
    echo "<tr><td><a href=\"#\" onClick=\"showNotice($row[0])\" > $row[1] </a></td></tr>";
}
?>
</table>
</marquee>
...
<!--<hr>-->
 <!-- version: <?php //echo $version ?> 
  版次: <?php //echo $build ?> 
  主机: <?php //echo $server_ip ?>  -->

  <?php	//echo "<b>话务员帐号:</b> $VD_login<br/><b>分机号码:</b> $SIP_user <br/> <b>登陆Campaign:</b> $VD_campaign&nbsp; \n"; ?>
 <!-- <br/>
  <b>暂停原因</b>
...
<table class="agent_info_table" border="0" cellspacing="0" cellpadding="0">
 
 <tr style="display:none" >
  <td class="head" style="display:none" colspan=2><?php echo $lang_account; ?>：<?php echo $VD_login;?>
  </td>
 </tr>
  <tr >
  <td class="head" ><font size="2px"><?php echo $lang_phone_number; ?>：</font>
  </td>
  <td ><font size="2px"><?php echo $SIP_user;?></font>
  </td>
  <td class="head" colspan=2><font size="2px"><?php echo $lang_agent_status; ?>：<span id="status_code"></span></font>
  </td>
 </tr>
  <tr style="display:none">
  ...
  <td><?php echo $VD_campaign;?><input type="hidden" name="vicidial_campaign_id" id="vicidial_campaign_id"  value="<?php echo $VD_campaign;?>">
  </td>
 </tr>
 <tr style="display:none">
  <td class="head"><?php echo $lang_today_login; ?>：
  </td>
  <td><?php echo $Agent_First_Login_Time;?>
  </td>
 </tr>
 <tr>
 ...
 ?>
  <tr>
  <td class="head"><?php echo $lang_pause_reason; ?>：
  </td>
  <td><span id="last_pause_code"></span><span id="PauseCodeLinkSpan"></span>
  </td>
 </tr>
 <?php 
 }	 
 ...
 <span id="debugbottomspan"></span>
   ...
   <tr style="display:none">
 <td colspan=2>
 <?php echo $lang_version; ?> ：<?php echo $version ;?>
 </td>
 </tr>
</table>
</div>
...
<td width=<?php //echo $SDwidth ?> align=left valign=top style="display:none">
...
if (window.WebForm_Button_Display=='NONE') {
...
function confirm_xfercall_send_hangup(){
	...
	//xfercall_tmp = 1;
    var confirm_value="<?php echo $lang_alert_confirm_hangup_transfer; ?>";
	...
	if (document.getElementById("xferoverride").checked==true && (document.getElementById("xferchannel").value == "" || document.getElementById("xferchannel").value == null) )
		{
		alert("<?php echo $lang_alert_outline_channel; ?>");
		return false;
		}
	...
	{
		alert("<?php echo $lang_alert_inline_channel; ?>");
		return false;
	}
	...
	if (document.getElementById("xferoverride").checked==true && Xfer_Blind_Display=='Y' )
		{
		document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_blindtransfer_cn.gif\" border=0 alt=\"<?php echo $lang_blind_transfer; ?>\" style=\"vertical-align:middle\"></a>";
...
function confirm_bothcall_send_hangup(){
	//yanson@20101027 start
	if(!confirm("<?php echo $lang_alert_hangup_all; ?>")){
		return false;
	}
...
function confirm_leave_3way_call(){
	...
	if(!confirm("<?php echo $lang_alert_leave_3way_confirm; ?>")){
			return false;
	}
...
{   
	 ...
	 pannel_img.alt=(pannel_img.alt=="<?php echo $lang_hide_panel; ?>")?'<?php echo $lang_expand_panel; ?>':'<?php echo $lang_hide_panel; ?>';
}
//TransferMain@20110104 start
function agentclick(){
	loop_ct = 0;
	...
	document.getElementById("agentinput").innerHTML = '<input type="text" size="12" name="xfernumber" id="xfernumber" readonly="true" maxlength="25" value="<?php echo $preset_populate ?>" style="margin-left:19px">';
	while (loop_ct < XFgroupCOUNT){
...
function overrideclick(){
	...
	document.getElementById("overrideinput").innerHTML = '<input type="text" size="12" name="xfernumber" id="xfernumber" maxlength="25" value="<?php echo $preset_populate ?>" style="margin-left:10px">';
	if(Xfer_Blind_Display=='Y')
	{
		document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_blindtransfer_cn.gif\" border=0 alt=\"<?php echo $lang_blind_transfer; ?>\" style=\"vertical-align:middle\"></a>";
	}
...
function trasfertargetclick(){
	document.getElementById("overrideinput").innerHTML = '<input type="text" size="12"  maxlength="25" value="" style="margin-left:10px">';
	document.getElementById("agentinput").innerHTML = '<input type="text" size="12" name="xfernumber" id="xfernumber" maxlength="25" value="<?php echo $preset_populate ?>" style="margin-left:19px">';
 
	if(Xfer_Blind_Display=='Y')
	{
		document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_blindtransfer_cn.gif\" border=0 alt=\"<?php echo $lang_blind_transfer; ?>\" style=\"vertical-align:middle\"></a>";
	}
...
function threeCall(){
	if(Fast_Hangup_Xferline_And_Grab_Custline=='N')
	{
		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"confirm_xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupxferline_cn.gif\" border=0 alt=\"<?php echo $lang_hangup_xfer_line; ?>\" style=\"vertical-align:middle\"></a>";
    }
	else
	{
		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"confirm_xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupxferline_cn_ehsn.gif\" border=0 alt=\"<?php echo $lang_hangup_xfer_line; ?>\" style=\"vertical-align:middle\"></a>";
	}
    document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"confirm_bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_hangupbothlines_cn.gif\" border=0 alt=\"<?php echo $lang_hangup_both_lines; ?>\" style=\"vertical-align:middle\"></a>";
	document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"confirm_leave_3way_call();return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_leave3waycall_cn.gif\" border=0 alt=\"<?php echo $lang_leave_3_way_call; ?>\" style=\"vertical-align:middle\"></a>";
}
...
function vmclick(){
	...
	if(Xfer_Blind_Display=='Y')
	{
		document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRVMAIL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/<?php echo $lang_images_dir; ?>vdc_XB_blindtransfer_cn.gif\" border=0 alt=\"<?php echo $lang_blind_transfer; ?>\" style=\"vertical-align:middle\"></a>";
	}
...
if(window.CalL_XC_a_NuMber != ""){
	document.getElementById("preset_num").innerHTML = '&nbsp;<a href="#" onClick="target_check();DtMf_PreSet_a();return false;"><?php echo $xfer_a_name; ?></a> &nbsp;';
}
if(window.CalL_XC_b_NuMber != ""){
	document.getElementById("preset_num").innerHTML = document.getElementById("preset_num").innerHTML + '<a href="#" onClick="target_check();DtMf_PreSet_b();return false;"><?php echo $xfer_b_name; ?></a> &nbsp;';
}
if(window.CalL_XC_c_NuMber != ""){
	document.getElementById("preset_num").innerHTML = document.getElementById("preset_num").innerHTML + '<a href="#" onClick="target_check();DtMf_PreSet_c();return false;"><?php echo $xfer_c_name; ?></a> &nbsp;';
}
if(window.CalL_XC_d_NuMber != ""){
	document.getElementById("preset_num").innerHTML = document.getElementById("preset_num").innerHTML + '<a href="#" onClick="target_check();DtMf_PreSet_d();return false;"><?php echo $xfer_d_name; ?></a> &nbsp;';
}
if(window.CalL_XC_e_NuMber != ""){
	document.getElementById("preset_num").innerHTML = document.getElementById("preset_num").innerHTML + '<a href="#" onClick="target_check();DtMf_PreSet_e();return false;"><?php echo $xfer_e_name; ?></a> &nbsp;';
}
if(window.CalL_XC_a_NuMber != "" || CalL_XC_b_NuMber != "" || CalL_XC_c_NuMber != "" || CalL_XC_d_NuMber != "" || CalL_XC_e_NuMber != "" ){
...
$( "#dialog-password" ).dialog({
	...
	function changepassword(){
		...
		if (MD_channel_look==1)
			{alert("<?php echo $lang_alert_can_not_change_password_when_dial; ?>");}
		else
			{
			if (VD_live_customer_call==1)
				{
				alert("<?php echo $lang_alert_tel_is_dialing; ?>");
				}
			else
				{
				if (alt_dial_status_display==1)
					...
					{
						if (VDRP_stage != 'PAUSED'){
							alert("<?php echo $lang_alert_pause_to_change_password; ?>");
						}else{
							try{
...
function change_pass(){
	if($( "#old_password" ).val()=="" || $( "#new_password1" ).val()=="" || $( "#new_password2" ).val()==""){
		alert("<?php echo $lang_alert_password_cannt_empty; ?>");
	}else{
		var para = "name=" + window.user + "&old_password=" + $( "#old_password" ).val() + "&new_password1=" + $( "#new_password1" ).val() + "&new_password2=" + $( "#new_password2" ).val() + "&action=modify_password&campaign_id=" + campaign;
		//alert(para);
		$.post("check_user.php", para,
		function(data){
			if(data == "1"){
				alert("<?php echo $lang_alert_old_password_wrong; ?>");
			}else if(data == "2"){
				alert("<?php echo $lang_alert_new_password_not_the_same; ?>");
			}else if(data == "3"){
				alert("<?php echo $lang_alert_password_length; ?>");
			}else if(data == "4"){
				alert("<?php echo $lang_alert_password_type; ?>");
			}else{
				alert("<?php echo $lang_alert_password_relogin; ?>");
				$( "#dialog-password" ).dialog( "close" );
				NormalLogout();
			}
		});
...
var para = "name="+window.user+"&action=check_user_login";
//alert(para);
$.post("check_user.php", para,
function(data){
    if(data == "0"){
...
<div id="dialog-confirm" title="详细信息" style="display:none">
<iframe id="webaddress" src="" width="500px" height="377px"></iframe>
</div> 
</body>
</html>
...
if ( ($mel > 0) or ($one_mysql_log > 0) )
	...
	if ( ($errno > 0) or ($mel > 1) or ($one_mysql_log > 0) )
		...
		fwrite ($efp, "$NOW_TIME|CCMS    |$query_id|$errno|$error|$stmt|$user|$server_ip|$session_name|\n");
		fclose($efp);
		}
	}
$one_mysql_log=0;
