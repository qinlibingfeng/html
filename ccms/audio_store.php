...
if ( (!preg_match("/\|$ip\|/", $server_ips)) and ($formIPvalid < 1) )
	...
	$stmt="SELECT count(*) from vicidial_users where user='$PHP_AUTH_USER' and pass='$PHP_AUTH_PW'";
	if ($DB) {echo "|$stmt|\n";}
	$rslt=mysql_query($stmt, $link);
	$row=mysql_fetch_row($rslt);
	$auth=$row[0];
	...
	if($auth>0)
		{
		$stmt="SELECT full_name,change_agent_campaign,modify_timeclock_log,user,user_level,modify_users,ast_admin_access from vicidial_users where user='$PHP_AUTH_USER' and pass='$PHP_AUTH_PW'";
		$rslt=mysql_query($stmt, $link);
		$row=mysql_fetch_row($rslt);
		$LOGfullname =				$row[0];
