...
if ( (strlen($group[0]) < 1) or (strlen($user_group[0]) < 1) )
	...
	if ($file_download < 1)
		...
		echo "| <a href=\"$LINKbase&stage=NAME\">坐席</a>       | <a href=\"$LINKbase&stage=ID\">ID</a>       | <a href=\"$LINKbase&stage=LEADS\">呼叫中</a>    | <a href=\"$LINKbase&stage=TCLOCK\">时间锁</a> | <a href=\"$LINKbase&stage=TIME\">AGENT TIME</a> | 等待       | 通话时长       | DISPO      | 暂停时间      | DEAD       | CUSTOMER   |   |$sub_statusesHTML\n";
		echo "+-----------------+----------+----------+------------+------------+------------+------------+------------+------------+------------+------------+   +$sub_statusesHEAD\n";
		}
	else
		{
...
#		echo "$Suser[$m]|$Sname[$m]|$Swait[$m]|$Stalk[$m]|$Sdispo[$m]|$Spause[$m]|$Scalls[$m]\n";
	...
	if ($file_download < 1)
		{
		echo "+-----------------+----------+----------+------------+------------+------------+------------+------------+------------+------------+------------+   +$sub_statusesHEAD\n";
		echo "|  总计        AGENTS:$TOT_AGENTS | $TOTcalls |$TOTtimeTC |$TOTALtime |$TOTwait |$TOTtalk |$TOTdispo |$TOTpause |$TOTdead |$TOTcustomer |   |$SUMstatusesHTML\n";
		echo "+-----------------+----------+----------+------------+------------+------------+------------+------------+------------+------------+------------+   +$sub_statusesHEAD\n";
		if ($AUTOLOGOUTflag > 0)
