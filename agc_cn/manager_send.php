...
if (($ACTION == "Monitor") || ($ACTION == "StopMonitor")) {
    ...
    if ((strlen($channel) < 3) or (strlen($queryCID) < 15) or (strlen($filename) < 8)) {
        ...
        if ($channel_live == 1) {
            ...
            if ($ACTION == "Monitor") {
                //$phones         = explode("_", $filename);
                //$phone_number_i = $phones[1];
...
if (($ACTION == "MonitorConf") || ($ACTION == "StopMonitorConf")) {
    ...
    if ((strlen($exten) < 3) or (strlen($channel) < 4) or (strlen($filename) < 8)) {
        ...
        ##### StopMonitorConf steps #####
            ...
            $stmt = "SELECT recording_id,start_epoch FROM recording_log where filename='$filename' and vicidial_id is null";
            $rslt = mysql_query($stmt, $link);
            if ($mel > 0) {
            ...
            $i = 0;
            while ($h > $i) {
                //$stmt = "INSERT INTO vicidial_manager select '','','$NOW_TIME','NEW','N','$server_ip','','Hangup','RH12345$StarTtime$i','Channel: $HUchannel[$i]','','','','','','','','','' from (select 'RH12345$StarTtime$i' as caller)datas where not exists(select callerid from vicidial_manager where datas.caller=vicidial_manager.callerid);";
...
if ($ACTION == "VolumeControl") {
    if ((strlen($exten) < 1) or (strlen($channel) < 1) or (strlen($stage) < 1) or (strlen($queryCID) < 1)) {
        ...
        if (eregi('UNMUTE', $stage)) {
            $vol_prefix = '2';
        }
        if (eregi('MUTING', $stage)) {
            $vol_prefix = '1';
        }
        $local_DEF            = 'Local/';
        $local_AMP            = '@';
