...
if (isset($_GET["is_manual_dialed"])) {
    $is_manual_dialed = $_GET["is_manual_dialed"];
} elseif (isset($_POST["is_manual_dialed"])) {
    $is_manual_dialed = $_POST["is_manual_dialed"];
}
...
if (isset($_GET["call_des"])) {
    $call_des = $_GET["call_des"];
} elseif (isset($_POST["call_des"])) {
    $call_des = $_POST["call_des"];
}
...
$hangup_cause_dictionary = array(
    ...
    96 => "必选信息单元丢失.",
    97 => "消息类型不存在或未实施移.",
    98 => "消息与呼叫状态不符或消息类型不存在或未实施.",
    99 => "信息单元不存在或未实施.",
    100 => "无效的信息单元内容.",
    101 => "消息与呼叫状态不符.",
    102 => "定时器超时的恢复.",
    103 => "参数不存在或未实施，通过（国内使用）.",
    111 => "协议差错，未规定.",
    127 => "互通，未规定."
);
...
if ($ACTION == 'LogiNCamPaigns') {
    if ((strlen($user) < 1)) {
        ...
        echo "<option value=\"\">-- 未找到您适用的业务活动 --</option>\n";
        echo "</select>\n";
        exit;
    } else {
    ...
    if (((ereg("START|ALL", $shift_enforcement)) and (!ereg("OFF", $VU_agent_shift_enforcement_override))) or (ereg("START|ALL", $VU_agent_shift_enforcement_override))) {
        ...
        if ((strlen($LOGgroup_shiftsSQL) < 3) and ($VU_shift_override_flag < 1)) {
            $VDdisplayMESSAGE = "<B>错误： There are no Shifts enabled for your user group</B>\n";
            $VDloginDISPLAY   = 1;
        } else {
            $HHMM = date("Hi");
            $wday = date("w");
            ...
            if (($shift_ok < 1) and ($VU_shift_override_flag < 1)) {
                $VDdisplayMESSAGE = "<B>错误： You are not allowed to log in outside of your shift</B>\n";
                $VDloginDISPLAY   = 1;
            }
        }
        if ($VDloginDISPLAY > 0) {
    ...
    if ($show_campaign_list > 0) {
        ...
        echo "<option value=\"\">-- 请选择业务活动 --</option>\n";
...
if ($ACTION == 'UpdateFields') {
    ...
    if ($vla_records > 0) {
        ...
        if ($list_lead_ct > 0) {
            ...
            echo "错误： no lead info in system: $lead_id\n";
        }
    } else {
        echo "错误： no lead active for this agent\n";
    }
}
...
if ($ACTION == 'manDiaLnextCaLL') {
 ...
 //add custom fields                       
												...
												$sql="select list_id,field_id,field_name,field_default from list_fields  where list_id='".$list_id."'order by field_id";
												
												
												$rslt=mysql_query($sql, $link);							
												$row_counts_list=mysql_num_rows($rslt);			
																							
										 		if ($row_counts_list!=0) {
													while($rs= mysql_fetch_array($rslt)){ 

															$list=array( "list_id"=>$rs['list_id'],"field_id"=>$rs['field_id'],"field_name"=>$rs['field_name'],"field_default"=>$rs['field_default']);
												...
												foreach($field_arr as $list){
													$sql_insert_fields .= ",".$list['field_name'];
													$sql_insert_values .= ", '".$list['field_default']."'";
								
												}
												$sql_insert_fields .= ")";
												$sql_insert_values .= ")";
												//echo $sql_insert_fields.$sql_insert_values." -------------------";
										
												mysql_query($sql_insert_fields.$sql_insert_values, $link);							
...
//add custom fields                       
												...
												$sql_insert_fields .= ")";
												$sql_insert_values .= ")";
												//echo $sql_insert_fields.$sql_insert_values."222<br/>";  
 ...
 //end                      
                ...
                if (eregi("N", $no_hopper_dialing)) {
                    ...
                    if (strlen($dial_statuses) > 2) {
                        ...
                        $rslt              = mysql_query($stmt, $link);
                        if ($mel > 0) {
                            mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00258', $user, $server_ip, $session_name, $one_mysql_log);
                        }
                        $userterrVLA_ct = mysql_num_rows($rslt);
                        if ($DB) {
                            echo "$userterrVLA_ct|$stmt\n";
                        }
                        if ($userterrVLA_ct > 0) {
                            $row               = mysql_fetch_row($rslt);
                            $agent_territories = $row[0];
                        ...
                        if (eregi("TERRITORY", $agent_dial_owner_only)) {
                            $adooSQL = "and owner IN('$user','$territory')";
                        }
                        ...
                        if (eregi("UP LAST NAME", $lead_order)) {
                            $order_stmt = 'order by last_name desc, lead_id asc';
                        }
                        if (eregi("DOWN LAST NAME", $lead_order)) {
                            $order_stmt = 'order by last_name, lead_id asc';
                        }
                        if (eregi("UP PHONE", $lead_order)) {
                            $order_stmt = 'order by phone_number desc, lead_id asc';
                        }
                        if (eregi("DOWN PHONE", $lead_order)) {
                            $order_stmt = 'order by phone_number, lead_id asc';
                        }
                        if (eregi("UP COUNT", $lead_order)) {
                            $order_stmt = 'order by called_count desc, lead_id asc';
                        }
                        if (eregi("DOWN COUNT", $lead_order)) {
                            $order_stmt = 'order by called_count, lead_id asc';
                        }
                        if (eregi("UP LAST CALL TIME", $lead_order)) {
                            $order_stmt = 'order by last_local_call_time desc, lead_id asc';
                        }
                        if (eregi("DOWN LAST CALL TIME", $lead_order)) {
                            $order_stmt = 'order by last_local_call_time, lead_id asc';
                        }
                        if (eregi("RANDOM", $lead_order)) {
                            $order_stmt = 'order by RAND()';
                        }
                        if (eregi("UP RANK", $lead_order)) {
                            $order_stmt = 'order by rank desc, lead_id asc';
                        }
                        if (eregi("DOWN RANK", $lead_order)) {
                            $order_stmt = 'order by rank, lead_id asc';
                        }
                        if (eregi("UP OWNER", $lead_order)) {
                            $order_stmt = 'order by owner desc, lead_id asc';
                        }
                        if (eregi("DOWN OWNER", $lead_order)) {
                            $order_stmt = 'order by owner, lead_id asc';
                        }
                        if (eregi("UP TIMEZONE", $lead_order)) {
                            $order_stmt = 'order by gmt_offset_now desc, lead_id asc';
                        }
                        if (eregi("DOWN TIMEZONE", $lead_order)) {
                            $order_stmt = 'order by gmt_offset_now, lead_id asc';
                        }
                        if ($agent_dial_owner_only != "none") {
                            $up_owner_sql = ",owner='$user'";
                        }
                        $stmt = "UPDATE vicidial_list SET status='QUEUE',user='$user' $up_owner_sql where called_since_last_reset='N' and status IN($Dsql) and list_id IN($camp_lists) and ($all_gmtSQL) $DLTsql $fSQL $adooSQL $order_stmt LIMIT 1;";
                        if ($DB) {
                            echo "$stmt\n";
                        }
                        $rslt = mysql_query($stmt, $link);
 ...
 //add end           
            ...
            if (($WeBRooTWritablE > 0) and ($enable_agc_xfer_log > 0)) {
                ...
                while (strlen($PADlead_id) > 9) {
                    $PADlead_id = substr("$PADlead_id", 0, -1);
                }
            ...
            $json_data="{";
				 		$json_data.="\"values\":".json_encode($LeaD_InfO).",";
           ...
           // echo "HOPPER EMPTY\n";
            ...
            $json_data="{";
				 		$json_data.="\"values\":".json_encode("HOPPER EMPTY\n").",";
				 		$json_data.="\"fields\":".json_encode($field_value_arr)."";
				 		$json_data.="}";
                ...
                if (strlen($rec_channels[$loop_count]) > 5) {
                    ...
                    if (strlen($filename) > 2) {
                        ...
                        if ($fn_count) {
							...
							$stmt = "UPDATE recording_log set end_time='$NOW_TIME',end_epoch='$StarTtime',length_in_sec=$length_in_sec,length_in_min='$length_in_min' $vidSQL $lidSQL where recording_id='$recording_id';";
                            if ($format == 'debug') {
                                echo "\n<!-- $stmt -->";
                            }
                            $rslt = mysql_query($stmt, $link);
        ...
        if ($queue_leadID_ct > 0) {
            ...
            if (($call_type == 'OUT') or ($call_type == 'OUTBALANCE')) {
                ...
                if ((strlen($VDCL_group_web) > 5) or (strlen($VDCL_group_name) > 0)) {
                    ...
                    echo "X|$VDCL_group_name|$VDCL_group_color|$VDCL_fronter_display|$VDADchannel_group|$VDCL_ingroup_script|$VDCL_get_call_launch|$VDCL_xferconf_a_dtmf|$VDCL_xferconf_a_number|$VDCL_xferconf_b_dtmf|$VDCL_xferconf_b_number|$VDCL_default_xfer_group|$VDCL_ingroup_recording_override|$VDCL_ingroup_rec_filename|$VDCL_default_group_alias|$VDCL_caller_id_number|$VDCL_group_web_vars|$VDCL_group_web_two|$VDCL_timer_action|$VDCL_timer_action_message|$VDCL_timer_action_seconds|$VDCL_xferconf_c_number|$VDCL_xferconf_d_number|$VDCL_xferconf_e_number|\n";
                }
...
if ($ACTION == 'updateDISPO') {
    ...
    if ((strlen($dispo_choice) < 1) || (strlen($lead_id) < 1)) {
		...
		$stmt = "UPDATE vicidial_log set status='$dispo_choice',call_des='$call_des' where lead_id='$lead_id' and user='$user' order by call_date desc limit 1;";
        if ($format == 'debug') {
            echo "\n<!-- $stmt -->";
        }
        ...
        if ($row[0] > 0) {
            ...
            $rslt = mysql_query($stmt, $link);
            if ($mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00310', $user, $server_ip, $session_name, $one_mysql_log);
            }
            ...
            if (($auto_dial_level < 1) or (preg_match('/^M/', $MDnextCID))) {
                ...
                $stmt = "SELECT count(*) from vicidial_log where lead_id='$lead_id' and user='$user' ";
                if ($format == 'debug') {
                    echo "\n<!-- $stmt -->";
                }
                $rslt = mysql_query($stmt, $link);
                ...
                if ($row[0] > 0) {
                    ...
                    /*$stmt = "UPDATE vicidial_log set status='$dispo_choice',call_des='$call_des' where lead_id='$lead_id' and user='$user' order by uniqueid desc limit 1;";
                    if ($format == 'debug') {
                        echo "\n<!-- $stmt -->";
                    }
                    $rslt = mysql_query($stmt, $link);
                    ...
                    if ($mel > 0) {
                        mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00145', $user, $server_ip, $session_name, $one_mysql_log, $affectedRows);
                    }*/
                    ...
                    $stmt        = "INSERT INTO vicidial_log set uniqueid='$FAKEcall_id',lead_id='$lead_id',list_id='$VLlist_id',campaign_id='$campaign',call_date='$NOW_TIME',start_epoch='$StarTtime',end_epoch='$StarTtime',length_in_sec='0',status='$dispo_choice',phone_code='$VLphone_code',phone_number='$VLphone_number',user='$user',comments='MANUAL',processed='N',user_group='$user_group',term_reason='AGENT',alt_dial='$VLalt',call_des='$call_des';";
					...
					$stmtb        = "update recording_log set vicidial_id='$FAKEcall_id',lead_id='$lead_id',end_epoch='$StarTtime',length_in_sec='0' where lead_id='$lead_id' and user='$user' order by recording_id desc limit 1;";
                    ...
                    if ($mel > 0) {
                        mysql_error_logging($NOW_TIME, $link, $mel, $stmtb, '002152', $user, $server_ip, $session_name, $one_mysql_log);
                    }
                    $MAN_vl_insert++;
                }
                ...
                if ($mel > 0) {
                    mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00145-1', $user, $server_ip, $session_name, $one_mysql_log, $affectedRows);
                }*/
            }
        }
    ...
    $stmt = "UPDATE vicidial_agent_log set dispo_sec='$dispo_sec',status='$dispo_choice',vicidial_id='$uniqueid' $dispo_epochSQL $lead_id_commentsSQL where agent_log_id='$agent_log_id';";
    if ($format == 'debug') {
        echo "\n<!-- $stmt -->";
    }
    $rslt = mysql_query($stmt, $link);
    ...
    if (($auto_dial_method=="Y" && $is_manual_dialed=="N" && $auto_dial_level < 1) || $auto_dial_method=="N" ) {
        ...
        if ($DO_NOT_UPDATE < 1) {
            ...
            if ($mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00163', $user, $server_ip, $session_name, $one_mysql_log);
						...
						$rslt=mysql_query("select field_name from list_fields  where list_id=".$list_id, $link);
						
						while($con=mysql_fetch_assoc($rslt)){//通过循环读取数据内容				
							$sql_2 .= ",".$con['field_name']." ='".trim($_REQUEST[$con['field_name']])."'";
						}
						$sql_2 .= " where lead_id='".$lead_id."'";

 			 			mysql_query($sql_2, $link);
...
if ($ACTION == 'PauseCodeSubmit') {
    ...
    if ((strlen($status) < 1) || (strlen($agent_log_id) < 1)) {
        ...
        ### this is not the first pause code entry, insert new vicidial_agent_log entry
            ...
            $rslt = mysql_query($stmt, $link);
            if ($mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00305', $user, $server_ip, $session_name, $one_mysql_log);
            }
            $VDpr_ct = mysql_num_rows($rslt);
            if ($VDpr_ct > 0) {
                $row       = mysql_fetch_row($rslt);
            ...
            $rslt = mysql_query($stmt, $link);
            if ($mel > 0) {
                mysql_error_logging($NOW_TIME, $link, $mel, $stmt, '00308', $VD_login, $server_ip, $session_name, $one_mysql_log);
            }
            $VLAaffected_rows_update = mysql_affected_rows($link);
        }
...
if ($ACTION == 'CALLSINQUEUEview') {
    ...
    if (eregi('NONE', $view_calls_in_queue)) {
        ...
        echo "<TR>";
        echo "<TD> &nbsp; </TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 电话号码 &nbsp; </font></TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 客户名称 &nbsp; </font></TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 等待 &nbsp; </font></TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 坐席 &nbsp; </font></TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"> &nbsp; &nbsp; &nbsp; </font></TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 电话组 &nbsp; </font></TD>";
        echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; 类型 &nbsp; </font></TD>";
        echo "</TR>";
...
if ($ACTION == 'CALLSINQUEUEgrab') {
    ...
    if ((eregi('NONE', $view_calls_in_queue)) or (eregi('N', $grab_calls_in_queue))) {
        echo "错误： Calls in Queue View Disabled for this campaign\n";
        exit;
    } else {
        ...
        if ($affected_rows > 0) {
            ...
            echo "错误： Call $stage could not be grabbed for $user\n";
            exit;
        }
    }
}
...
if ($ACTION == 'VDADpause_dead') {
	...
	$rslt = mysql_query($stmt, $link);
	if ($mel > 0) {
		$errno = mysql_error_logging($NOW_TIME, $link, $mel, $stmt, 'D0166', $user, $server_ip, $session_name, $one_mysql_log);
	}
	echo $stage;
}
...
##### MySQL Error Logging #####
function mysql_error_logging($NOW_TIME, $link, $mel, $stmt, $query_id, $user, $server_ip, $session_name, $one_mysql_log, $rows)
{
    $NOW_TIME = date("Y-m-d H:i:s");
    #	mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00001',$user,$server_ip,$session_name,$one_mysql_log);
    $errno    = '';
    ...
    if (($mel > 0) or ($one_mysql_log > 0)) {
        ...
        if (($errno > 0) or ($mel > 1) or ($one_mysql_log > 0)) {
            ...
            fwrite($efp, "$NOW_TIME|vdc_db_query|$query_id|$errno|$error|$stmt|$user|$server_ip|$session_name|$rows\n");
            fclose($efp);
        }
    }
    $one_mysql_log = 0;
