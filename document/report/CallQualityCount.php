...
function check_form(actions){
	...
	if(data_type=="data_8"||data_type=="data_9"||data_type=="data_9_month"||data_type=="data_10"||data_type=="data_11"){
		action_send="quality_user_report";
	}else{
		action_send="quality_report";	
	}
	...
	if (actions == "search") {
		...
		var url="action="+action_send+times+"&"+$('#form1').serialize();
		//alert(url);
		$.ajax({
				...
				$("#datatable thead tr").append(th);
 				...
 				if(parseInt(json.counts)>0){
					...
					$.each(json.datalist, function(index,con){
						...
						for(var key in con){
							nowrap="";
							if(key=="工号"||key=="日期"||key=="月份"||key=="姓名"||key=="业务"||key=="质检人"){
								nowrap="nowrap";
							}
	...
	if (actions == "excel") {
		...
		var url="action="+action_send+"&do_actions=excel"+times+"&"+$('#form1').serialize();
		$.ajax({
      ...
      <form action="" onSubmit=""  method="post" name="form1" id="form1">       
        <table width="100%" border="0" align="center"  cellspacing="0" id="search_list" class="search_table" >
               ...
               <tr>
                 ...
                 <td >
                 	<select name="data_type" class="s_option" id="data_type">
                        <option value="data_1" chart_x="日期" title="按日期统计">按日期统计</option>
                        <option value="data_1_month" chart_x="月份" title="按月份统计">按月份统计</option>
                        <option value="data_2" chart_x="工号、姓名" title="按工号统计">按工号统计</option>
                        <option value="data_3" chart_x="业务" title="按业务统计">按业务统计</option>
                        
                        <option value="data_5" chart_x="日期、工号、姓名" title="按日期、工号统计">按日期、工号统计</option>
                        <option value="data_5_month" chart_x="月份、工号、姓名" title="按月份、工号统计">按月份、工号统计</option>
                        <option value="data_6" chart_x="日期、业务" title="按日期、业务统计">按日期、业务统计</option>
                        <option value="data_6_month" chart_x="月份、业务" title="按月份、业务统计">按月份、业务统计</option>
                        <option value="data_7" chart_x="业务、工号、姓名" title="按业务、工号统计">按业务、工号统计</option>
                        
                        <option value="data_8" chart_x="质检人" title="按质检人统计">按质检人统计</option>
                        <option value="data_9" chart_x="质检人、日期" title="按质检人、日期统计">按质检人、日期统计</option>
                        <option value="data_9_month" chart_x="质检人、月份" title="按月份、质检人统计">按质检人、月份统计</option>
                        <option value="data_10" chart_x="质检人、工号、姓名" title="按质检人、工号统计">按质检人、工号统计</option>
                        <option value="data_11" chart_x="质检人、业务" title="按质检人、业务统计">按质检人、业务统计</option> 
                      </select>
                   </td>
                 ...
                 <!--<td width="8%" align="right">质检人员：</td>
                 ...
                 <td width="8%"  align="right" id="">坐席工号：</td>
     <td  nowrap="nowrap"><input name="agent_name_list" type="text" class="input_text2" id="agent_name_list"  title="双击选择坐席工号" size="16" readonly="readonly"  onDblClick="c_agent_list('get_agent_list');"/><a class="sel_" hidefocus="true" href="javascript:void(0);" title="点击选择坐席工号" onClick="c_agent_list('get_agent_list');"></a></td>
               ...
               <tr>
                 ...
                 <!--<ul class="nav nav-pills">
                  <li class="dropdown"> <a class="dropdown-toggle" id="ch_sec_type" href="javascript:void(0)" title="点击设定时间精确度类型"> <span>质检时间(小时)</span> <b class="caret"></b> </a>
     ...
     <div id="chart_con"></div>
        ...
        <div id="chart_f_list">
            ...
            <div class="head"><span class="green">&nbsp;选择图表数据值</span><span id="chart_tip" style="margin-left:6px" class="red"></span><a href='javascript:void(0)' hidefocus='true' class="close" title="关闭" onclick="javascript:$('#chart_f_list').fadeOut()"></a></div>
              ...
              <div class="bottom"><input type="checkbox" name="is_datalab" id="is_datalab" value="yes" /> <label for="is_datalab">显示数值</label>  <input type="button" name="" value="确 定" onclick="get_chart_data('业务、工作量统计图表')" /> <span style="margin-right:4px"><input type="reset" name="" value="重 置" onclick="javascript:$('#chart_f_list label').removeClass('blue')" /></span></div>
          </form>
