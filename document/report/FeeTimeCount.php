...
function check_form(actions){
	...
	if (actions == "search"){
		...
		var url="action=fee_report"+times+"&"+$('#form1').serialize();
		//alert(url);
		$.ajax({
	...
	if (actions == "excel") {
		...
		var url="action=fee_report&do_actions=excel"+times+"&"+$('#form1').serialize();
		$.ajax({
             ...
             <table width="100%" border="0" align="center"  cellspacing="0" id="search_list" class="search_table" >
               ...
               <tr>
                 ...
                 <select name="data_type" id="data_type" class="s_option">
                    ...
                    <option value="data_4" chart_x="日期、工号、姓名" title="按日期、工号统计">按日期、工号统计</option>
                    <option value="data_4_month" chart_x="月份、工号、姓名" title="按月份、工号统计">按月份、工号统计</option>
                    <option value="data_5" chart_x="工号、姓名、日期" title="按工号、日期统计">按工号、日期统计</option>
                    <option value="data_5_month" chart_x="工号、姓名、月份" title="按工号、月份统计">按工号、月份统计</option>
                    ...
                    <option value="data_7" chart_x="业务、日期" title="按业务、日期统计">按业务、日期统计</option>
                    <option value="data_7_month" chart_x="业务、月份" title="按业务、月份统计">按业务、月份统计</option>
                    <option value="data_8" chart_x="业务、工号、姓名" title="按业务、工号统计">按业务、工号统计</option>
                    <option value="data_9" chart_x="工号、姓名" title="按工号、业务统计">按工号、业务统计</option>
                     
                  </select>
          
                 </td>
                 <td width="10%" align="right">时长格式：</td>
                 <td>
               ...
               <tr>
                 ...
                 <td colspan="7">
                 <span class="gray">
                 请注意！本报表中出现的时长均为"通话时长",查询结果中的话费项均为参考值，不能作为最终话费结算依据.<br />
                 报表字段释义：1)最长费用:通话时间最长的话单产生话费;2)平均费用:所有接通话单的平均话费;3)平均成功费用:提交为"成功"的话单平均话费;4)成功费用比:提交为"成功"的话费占所有话费的比;5)接通:加入计算了“丢弃”数据;</span></td>
               </tr>
               <tr>
