...
function get_datalist(page_nums,a_ctions,doa_ctions,pagesize){
	...
	var url="action=get_campaign_list&pages="+page_nums+"&actions="+a_ctions+"&do_actions="+doa_ctions+"&sorts="+$('#sorts').val()+"&order="+$('#order').val()+"&pagesize="+pagesize+times+"&"+$('#form1').serialize();
	//alert(url);
	//return false;
		...
		success: function(json){ 
			...
			if(parseInt(json.counts)>0){
				...
				$.each(json.datalist, function(index,con){
					...
					tr_str+="<td align=\"center\"><input name=\"c_id\" type=\"checkbox\" value=\""+con.campaign_id+"\" /></td>";
...
$(document).ready(function(){
	...
	$("#datatable .dataHead th[sort]").map(function(){
 	...
 	$('<input name="a_ctions" type="text" class="dis_none"  id="a_ctions"/> <input name="doa_ctions" type="text" class="dis_none"  id="doa_ctions"/> <input name="recounts" type="text" class="dis_none"  id="recounts"/> <input name="pages" type="text" class="dis_none"  id="pages" value="1"/> <input name="pagecounts" type="text" class="dis_none"  id="pagecounts"/><input name="pagesize" type="text" class="dis_none"  id="pagesize" value="15"/> <input name="sorts" type="text" class="dis_none"  id="sorts" value="campaign_id"/> <input name="order" type="text" class="dis_none"  id="order"/>').appendTo("body");
...
<div class="page_main">
     ...
     <tr>
        <td colspan="2"><a href='javascript:void(0);'  class='zPushBtn' hidefocus='true' tabindex='-1' onselectstart='return false' priv="true" onClick="add_campaign('');" title="新建业务活动"><img src="/images/icons/telephone6.png" style="margin-top:6px"/><b>新建业务活动&nbsp;</b></a><a href='javascript:void(0);'  class='zPushBtn' hidefocus='true' tabindex='-1' onselectstart='return false' priv="true" onClick="copy_campaign();" title="复制现有业务活动配置到新活动"><img src="/images/icons/telephone3.png" style="margin-top:6px" /><b>复制业务活动&nbsp;</b></a></td>
      </tr>
    </table>
     ...
     <table width="99%" border="0" align="center" cellpadding="0" class="blocktable" >
        <tr>
            <td style="">
          <input type="text" class="dis_none"  name="get_dial_method" id="get_dial_method" value="0" />
          <input type="text" class="dis_none"  name="get_auto_dial_level" id="get_auto_dial_level" value="0" />
            ...
            <form action="" onSubmit=""  method="post" name="form1" id="form1">       
             <table width="100%" border="0" align="center"  cellspacing="0" id="search_list" class="search_table" >
               ...
               <tr>
                 ...
                 <td>
                 	<select name="dial_method" class="s_option" id="dial_method" onclick="get_select_opt('','send.php','get_dial_method','dial_method','group_n')">
                    	<option value="">未指定</option>
                    </select>
                 </td>
                 <td width="8%" align="right" id="td">呼叫级别：</td>
                 <td>
                     <select name="auto_dial_level" class="s_option" id="auto_dial_level" onfocus="get_select_opt('','send.php','get_auto_dial_level','auto_dial_level','none')">
                      	<option value="">未指定</option>
                     </select>
                 </td>
            ...
            <table border="0" width="100%" align="center" cellpadding="0" cellspacing="0" class="dataTable" id="datatable" >
                  <thead>
                    <tr align="left" class="dataHead">
                      ...
                      <th sort="dial_statuses" >可拨打状态</th>
