	...
	<table width="100%" border=0 cellpadding=0 cellspacing=2>
		...
		?>
			<tr >
				...
				<td width="" height="24" align="left" class="deepgreen"><label for="agents_item_<?php echo $rs["user_group"]; ?>" onclick="CheckItemsAll('form1','agents_item_<?php echo $rs["user_group"]; ?>');"><input type="checkbox" id="agents_item_<?php echo $rs["user_group"]; ?>" name="agents_item" value="<?php echo $rs["user_group"]; ?>" parentid="agents_<?php echo $rs["user_group"]; ?>" ><?php echo $rs["group_name"]; ?></label></td>
				<td align="left" class="check_items">
                <ul>
		<?php       	
		
			$sqls="select user,full_name from vicidial_users where user_group='".$rs["user_group"]."' order by user_id desc";
			$rows2=mysqli_query($db_conn,$sqls);
			
			if(mysqli_num_rows($rows2)!=0){
				while($rs2= mysqli_fetch_array($rows2)){ 
		 ?>      
			 <li><label for="agents_item_<?php echo $rs["user_group"]; ?>_<?php echo $rs2["user"]; ?>" onclick="CheckItems('form1','agents_<?php echo $rs["user_group"]; ?>','agents_item_<?php echo $rs["user_group"]; ?>');"><input type="checkbox" parentid="agents_item_<?php echo $rs["user_group"]; ?>" id="agents_item_<?php echo $rs["user_group"]; ?>_<?php echo $rs2["user"]; ?>" name="agents" value="<?php echo $rs2["full_name"]; ?>|<?php echo $rs2["user"]; ?>"><?php echo $rs2["full_name"]; ?>[<?php echo $rs2["user"]; ?>]</label></li>
		<?php 
				}
			} 
...
function do_setquality_user_list(){
	...
	if (callstatus_name!=""){callstatus_name=callstatus_name.substr(0,callstatus_name.length-1)}
 	$(_DialogInstance.ParentWindow.document).find("#quality_user_list").val(callstatus_name);
	
 	$(_DialogInstance.ParentWindow.document).find("#quality_user_list").attr("title","双击<?php echo $tits ?>："+callstatus_name);
                      ...
                      <td align="left" valign="top" class="search_text_zone check_items">
                       <ul>
                        ...
                        if(mysqli_num_rows($rows)!=0){
                             while($rs= mysqli_fetch_array($rows)){ 
 						 ?>  
                         <li <?php echo $is_dis;?>><label for="quality_user_<?php echo $rs["userid"] ?>" onclick="CheckItems('form1','quality_user','quality_user_all');"><input type="checkbox" parentid="quality_user_all" id="quality_user_<?php echo $rs["userid"] ?>" name="quality_user" value="<?php echo $rs["full_name"] ?>|<?php echo $rs["userid"] ?>"><span class="<?php echo $class; ?>"><?php echo $rs["full_name"] ?>[<?php echo $rs["userid"] ?>]</span></label></li>
                        <?php 
                            }
                        }else{
                  ...
                  <table width="100%" border="0" align="center" cellpadding=4 cellspacing=0>
                   ...
                   <table width="100%" border="0" align="center" cellpadding=2 cellspacing=0>
                    ...
                    <tr>
                      ...
                      <td height="24"><input name="job_name" type="text" id="job_name" size="60" maxlength="80" value="<?php echo $job_name ?>"/><span class="red">※</span></td>
                    </tr>
                    <tr>
                      <td width="22%" height="" align="right">被叫号码：</td>
                      <td align="left" valign="middle"><textarea name="phones" id="phones" cols="60" rows="5" style="height:130px"><?php echo $phone_numbers; ?></textarea><span class="red">※</span></td>
                    </tr>                    
                  </table>
