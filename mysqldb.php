...
class db {
	...
	function SynRecords($row,$call_type){
		...
		$row1=$this->createCdrEdu($row,$call_type);
		$this->query_unbuffered($str);
		
		//$sql1 = "select call_id from cc_call_history where call_id=".$row['closecallid']."";
		//$resw=$this->row_query_one($sql1);
		//if($resw){
		//	;
		//}else{
		$this->row_insert("cc_call_history",$row1);	
	...
	function createCdrEdu($row,$call_type){
		...
		$sql = "select filename,location from recording_log where vicidial_id=".$row['uniqueid']."";
		$ress=$this->row_query_one($sql);
		echo $ress['filename'];
		if($ress['filename'] == ""){;}
		else{
			$cdrRow['file_name'] = $ress['filename'];
			$cdrRow['location'] = $ress['location'];
			$cdrRow['is_leavemess'] = '1';
		}

		$sql = "select group_name from vicidial_inbound_groups where group_id='IVR_AIA'";
		$ress=$this->row_query_one($sql);
		...
		if($call_type == 0){
			...
			$cdrRow["call_exten"]=$ress["group_name"];
			...
			//$sql = "select group_name from vicidial_inbound_groups where group_id=".$row['campaign_id']."";
			
			//echo $ress['group_name'];
		...
		if(($row['status']=='CG')||($row['status']=='good')){
